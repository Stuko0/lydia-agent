 ...
 * product name (`Lydia`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Lydia'
 *
 * This is a hard failure with no obvious cause for the user — `lydia desktop`
 ...
 * (the mac-unpacked Lydia.app bundle) and Windows (win-unpacked), so we
