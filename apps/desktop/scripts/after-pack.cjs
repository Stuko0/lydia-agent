 ...
 *
 * Stamps the Lydia icon + identity onto the packed Windows Lydia.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `lydia desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Lydia')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Lydia'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Lydia.exe keeps the stock Electron icon`)
  }
}
