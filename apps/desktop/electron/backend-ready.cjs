...
// compile and import the whole `lydia_cli.main` → `web_server` → FastAPI/
...
function waitForDashboardPort(child, timeoutMs = resolvePortAnnounceTimeoutMs()) {
  return new Promise((resolve, reject) => {
    ...
    function onExit(code, signal) {
      cleanup()
      reject(new Error(`Lydia backend: exited before port announcement (${signal || code})`))
    }
    ...
    const timer = setTimeout(() => {
      cleanup()
      reject(new Error(`Timed out waiting for Lydia backend port announcement (${timeoutMs}ms)`))
    }, timeoutMs)
