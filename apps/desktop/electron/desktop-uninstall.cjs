...
/**
 * Map an uninstall mode to the `python -m lydia_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `lydia_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'lydia_cli.uninstall', '--mode', mode]
}
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import lydia_cli`
 ...
 * modes + `pythonPath`=agentRoot so `import lydia_cli` resolves from source
