...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'lydia-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/lydia-runner-test',
    sourceRepoRoot: null,
    lydiaHome: '/tmp/lydia-runner-test',
    logRoot: '/tmp/lydia-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'lydia-agent', 'scripts')
