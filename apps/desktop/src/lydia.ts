import { JsonRpcGatewayClient } from '@lydia/shared'
...
export class LydiaGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Lydia gateway connection closed',
      connectErrorMessage: 'Could not connect to Lydia gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Lydia gateway is not connected',
...
// curation `lydia model` does — for Nous it honors the free/paid tier so a
...
export function checkLydiaUpdate(force = false): Promise<BackendUpdateCheckResponse> {
  return window.lydiaDesktop.api<BackendUpdateCheckResponse>({
    ...profileScoped(),
    path: `/api/lydia/update/check${force ? '?force=true' : ''}`
  })
}
