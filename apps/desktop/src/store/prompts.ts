...
// Git credential askpass — global (not session-keyed) because git prompts
// come from a long-lived subprocess, not from a per-session tool call.
// `requestId` is generated by web_server when the GIT_ASKPASS shim
// long-polls /api/git/askpass. The renderer posts the answer to
// /api/git/askpass/respond (not jsonrpc) and clears the request.
export interface AskpassRequest {
  prompt: string
  requestId: string
}

export const $askpassRequest = atom<AskpassRequest | null>(null)
export function setAskpassRequest(request: AskpassRequest): void {
  $askpassRequest.set(request)
}
export function clearAskpassRequest(): void {
  $askpassRequest.set(null)
}
