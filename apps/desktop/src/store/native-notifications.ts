...
const STORAGE_KEY = 'lydia:native-notifications'
...
// "Backgrounded" = the user isn't on Lydia. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.lydiaDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.lydiaDesktop
