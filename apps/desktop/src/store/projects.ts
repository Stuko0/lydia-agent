...
import type { ProjectInfo, ProjectsPayload } from '@/types/lydia'
...
const PROJECT_SCOPE_KEY = 'lydia.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Lydia gateway is not connected')
  }
...
// repo, returning where Lydia should start working. Git is the source of
