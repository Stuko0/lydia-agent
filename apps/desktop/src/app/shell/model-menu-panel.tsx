...
import { getGlobalModelOptions, getMoaModels } from '@/lydia'
...
import type { MoaConfigResponse, ModelOptionProvider, ModelOptionsResponse } from '@/types/lydia'
...
export function ModelMenuPanel({ gateway, onSelectModel, requestGateway }: ModelMenuPanelProps) {
  ...
  // session (effort/fast), gated by capability. Unset → Lydia defaults.
          ...
          {groups.map(group => (
            <DropdownMenuGroup className="py-0.5" key={group.provider.slug}>
              ...
              {group.families.map(family => {
                ...
                // the active model, otherwise its remembered preset (Lydia
