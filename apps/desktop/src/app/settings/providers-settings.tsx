...
import { disconnectOAuthProvider, listOAuthProviders } from '@/lydia'
import { useI18n } from '@/i18n'
import { Check, ChevronDown, ChevronRight, KeyRound, Loader2, Terminal, Trash2 } from '@/lib/icons'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import { $desktopOnboarding, startManualProviderOAuth } from '@/store/onboarding'
import type { EnvVarInfo, OAuthProvider } from '@/types/lydia'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.lydiaDesktop?.terminal)
...
//      `lydia model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Lydia can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Lydia can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // Lydia never deletes creds another tool owns behind a silent API call.
