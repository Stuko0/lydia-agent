...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: LydiaConfigRecord = { display: { theme: 'mono' } }
  ...
  it('rejects prototype-polluting config paths', () => {
    const config: LydiaConfigRecord = {}
  ...
  describe('providerGroup', () => {
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // LYDIA_QWEN_ shares the LYDIA_ stem with other integrations.
      expect(providerGroup('LYDIA_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
