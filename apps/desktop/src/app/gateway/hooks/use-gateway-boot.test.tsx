...
// bug that drives the REAL useGatewayBoot hook + REAL LydiaGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshLydiaConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { lydiaDesktop?: unknown }).lydiaDesktop = fakeDesktop()
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForLydia) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startLydia()'s remote branch awaits waitForLydia() for 45s before it
    ...
    ;(window as { lydiaDesktop?: unknown }).lydiaDesktop = desktop
    ...
    // After ~45s waitForLydia gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Lydia backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
