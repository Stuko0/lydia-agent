...
const LYDIA_PATHS_MIME = 'application/x-lydia-paths'
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      ...
      letterSpacing: 0,
      lineHeight: 1.12,
      // Full-screen TUIs (lydia --tui, vim) grab the mouse, so a plain drag
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[lydia-terminal] WebGL unavailable; falling back to DOM', err)
      }
