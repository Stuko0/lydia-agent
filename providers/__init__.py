...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with lydia-agent)
2. User plugins: ``$LYDIA_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$LYDIA_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$LYDIA_HOME/plugins/model-providers/`` if it exists."""
    try:
        from lydia_constants import get_lydia_home

        d = get_lydia_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple LYDIA_HOME profiles don't alias each other.
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$LYDIA_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with lydia-agent.
    ...
    # 2. User plugins — under $LYDIA_HOME/plugins/model-providers/<name>/.
