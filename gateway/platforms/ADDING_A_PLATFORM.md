...
There are two ways to add a platform to the Lydia gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.lydia/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Lydia code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `lydia gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `lydia_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Lydia core.
...
And add it to the `lydia-gateway` composite:

```python
"lydia-gateway": {
    "includes": [..., "lydia-your-platform"]
}
```
...
## 12. Status Display (`lydia_cli/status.py`)
...
## 13. Gateway Setup Wizard (`lydia_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ lydia_cli/ toolsets.py \
  --include="*.py" -l | sort -u
