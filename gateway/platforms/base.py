...
# Audio file extensions Lydia recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Lydia' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Lydia-created Telegram private-chat
...
from lydia_constants import get_default_lydia_root, get_lydia_dir, get_lydia_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.lydia/.env manually."
)
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; LydiaAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Lydia root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* LYDIA_HOME's cache, so a gateway running at
    the root (e.g. ``LYDIA_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when LYDIA_HOME is symlinked under a
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Lydia profile and shared Lydia root both contain control
    ...
    # LYDIA_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.lydia
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Lydia
    secrets (``~/.lydia/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    Lydia-managed cache, under an operator-allowlisted root
    ...
    # credential/secret stores under the Lydia root (~/.lydia/.env,
    ...
    # ``MEDIA:~/.lydia/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?lydia\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+lydia[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # Lydia commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Lydia sends
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Lydia home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Lydia home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.lydia/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from lydia_cli.commands import should_bypass_active_session
