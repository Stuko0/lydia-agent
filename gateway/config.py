...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_lydia_home() / "sessions")
    ...
    # gateway behaves exactly as before — single LYDIA_HOME, no profile stamping.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.lydia/config.yaml (primary user-facing config)
    3. ~/.lydia/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_lydia_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through lydia_cli.config.load_config, so without this a managed
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``lydia config set gateway.streaming.*``
