...
The PID file lives at ``{LYDIA_HOME}/gateway.pid``.  LYDIA_HOME defaults to
``~/.lydia`` but can be overridden via the environment variable.  This means
separate LYDIA_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting LYDIA_HOME."""
    home = get_lydia_home()
...
def _get_lock_dir() -> Path:
    ...
    return state_home / "lydia" / _LOCKS_DIRNAME
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Lydia gateway lifecycle subcommand from a command line.
    ...
    test also matched ``lydia_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\lydia-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Lydia's
    ...
    for token in tokens:
        ...
        if basename in ("lydia-gateway", "lydia-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "lydia_cli.main" in joined
        or "lydia_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("lydia", "lydia.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `lydia gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Lydia-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Lydia gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a LYDIA_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.lydia`` or ``$LYDIA_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``lydia_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``LYDIA_HOME=<path>``) on its argv; the default/root gateway runs
    ...
    # a non-matching explicit LYDIA_HOME= on the argv. LYDIA_HOME is usually
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `lydia status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``LYDIA_HOME`` in-process.  Defaults to
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    LYDIA_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different LYDIA_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``lydia.service`` + ``lydia-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # running under a different LYDIA_HOME. When two profile gateway
    # services share the same default ~/.lydia (LYDIA_HOME not set
    ...
    # field is absent in markers written by older Lydia versions; treat
    ...
    if replacer_home is not None and replacer_home != str(get_lydia_home()):
        return False
    ...
    # ``lydia gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(target_pid: int) -> bool:
    ...
    try:
        ...
        record = {
            ...
            "replacer_lydia_home": str(get_lydia_home()),
