#!/usr/bin/env python3
"""
SQLite State Store for Lydia Agent.
...
DEFAULT_DB_PATH = get_lydia_home() / "state.db"
...
# lydia_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``lydia_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see lydia_cli/kanban_db.py) would
...
class SessionDB:
    ...
    # With multiple lydia processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``lydia_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `lydia update` to rebuild the venv with a "
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another Lydia surface and jump
    ...
    def delete_session_if_empty(
        ...
        and ``lydia sessions list`` output. (Pattern ported from
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Lydia, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Lydia session.

        A Lydia session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Lydia session is already bound to any Telegram DM topic.
