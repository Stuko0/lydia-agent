...
# 5. Symlinks the 'lydia' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}🌹 Lydia Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/lydia-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink lydia into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up lydia command..."

LYDIA_BIN="$SCRIPT_DIR/venv/bin/lydia"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$LYDIA_BIN" "$COMMAND_LINK_DIR/lydia"
echo -e "${GREEN}✓${NC} Symlinked lydia → $COMMAND_LINK_DISPLAY_DIR/lydia"
...
# ============================================================================
# Seed bundled skills into ~/.lydia/skills/
# ============================================================================

LYDIA_SKILLS_DIR="${LYDIA_HOME:-$HOME/.lydia}/skills"
mkdir -p "$LYDIA_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.lydia/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$LYDIA_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  lydia status        # Check configuration"
if is_termux; then
    echo "  lydia gateway       # Run gateway in foreground"
else
    echo "  lydia gateway install # Install gateway service (messaging + cron)"
fi
echo "  lydia cron list     # View scheduled jobs"
echo "  lydia doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m lydia_cli.main setup
fi
