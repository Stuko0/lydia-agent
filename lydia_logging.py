"""Centralized logging setup for Lydia Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.lydia/logs/`` (profile-aware via ``get_lydia_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``lydia_cli.web_server``,
    ``lydia_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Lydia (TUI, gateway, ``hy_memory`` server, MCP
...
from lydia_constants import get_config_path, get_lydia_home
...
# Used by _ComponentFilter and exposed for ``lydia logs --component``.
COMPONENT_PREFIXES = {
...
def setup_logging(
    *,
    ...
    """Configure the Lydia logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    lydia_home
        Override for the Lydia home directory.  Falls back to
        ``get_lydia_home()`` (profile-aware).
    log_level
    ...
    home = lydia_home or get_lydia_home()
    log_dir = home / "logs"
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the lydia group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from lydia_cli.config import is_managed
