...
This document is the formal interface between the **Lydia gateway** (Python,
`gateway/relay/`) and the **connector** (Node/TypeScript,
`Stuko/gateway-gateway`). The connector implementer's first action is to
read this file.
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `lydia gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`lydia gateway enroll` → connector `/relay/enroll`, or
...
(`Stuko/gateway-gateway`); this section is the gateway-facing summary.
...
driven by the managed Portal / `lydia` CLI.
