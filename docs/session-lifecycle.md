...
The canonical transcript store is SQLite via `SessionDB` (from `lydia_state`). The
...
Counts consecutive restarts via a JSON file (`{LYDIA_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `lydia update`, `lydia gateway restart`,
or `/restart`.
