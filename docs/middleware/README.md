...
Lydia middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Lydia'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Lydia calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Lydia retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Lydia guardrails,
...
- `telemetry_schema_version`: currently `lydia.observer.v1`
- `middleware_schema_version`: currently `lydia.middleware.v1`
...
Lydia stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Lydia runs
them as a nested chain in registration order. Middleware failures are fail-open:
Lydia logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Lydia applies middleware in this order:
...
For each tool call, Lydia applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Lydia pre-execution path against the effective arguments:
...
```bash
lydia plugins enable <plugin-name>
```

For isolated local testing, use one `LYDIA_HOME` for plugin enablement and the
agent run:

```bash
export LYDIA_HOME=/tmp/lydia-middleware-test
mkdir -p "$LYDIA_HOME"
lydia plugins enable <plugin-name>
lydia chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run lydia plugins enable <plugin-name>
uv run lydia chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["lydia_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/lydia-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Lydia expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Lydia treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Lydia preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Lydia does not convert downstream
