...
Lydia supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.lydia/config.yaml`:
...
Or set the env var: `LYDIA_KANBAN_DISPATCH_IN_GATEWAY=false`
