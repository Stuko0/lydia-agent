# Lydia Observer Hooks

Lydia observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Lydia' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Lydia catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Lydia only builds
