...
When running Lydia inside Docker, the default `network_mode: host` gives the
...
The Lydia [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   LYDIA_UID=$(id -u) LYDIA_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=lydia,lydia-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://lydia-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Lydia trust model and vulnerability reporting
