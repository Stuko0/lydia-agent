...
LYDIA_HOME_DIR="${LYDIA_HOME:-$HOME/.lydia}"
...
# ── 3. Ensure Lydia config has twozero_td MCP entry ──
if [[ ! -f "$LYDIA_CFG" ]]; then
    echo -e " ${FAIL} Lydia config not found at ${LYDIA_CFG}"
    manual_steps+=("Create ${LYDIA_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$LYDIA_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Lydia config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Lydia config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
    ...
    manual_steps+=("Restart Lydia session to pick up config change")
fi
