...
## 5. Lydia Configuration

### Config location

`$LYDIA_HOME/config.yaml` (defaults to `~/.lydia/config.yaml` when `LYDIA_HOME` is unset)
...
Restart the Lydia session for changes to take effect. The MCP connection is
