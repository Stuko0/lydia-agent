# Gitea REST API Cheatsheet

Base URL: `https://\${GITEA_HOST:-gitea.com}/api/v1`

All requests need: `-H "Authorization: token $GITEA_TOKEN"`
...
- Check remaining: `curl -s -H "Authorization: token $GITEA_TOKEN" https://\${GITEA_HOST:-gitea.com}/api/v1/rate_limit`
...
curl -s -H "Authorization: token $GITEA_TOKEN" \
  https://\${GITEA_HOST:-gitea.com}/api/v1/repos/$TEA_OWNER/$TEA_REPO

# POST with JSON body
curl -s -X POST \
  -H "Authorization: token $GITEA_TOKEN" \
  https://\${GITEA_HOST:-gitea.com}/api/v1/repos/$TEA_OWNER/$TEA_REPO/issues \
  -d '{"title": "...", "body": "..."}'

# PATCH (update)
curl -s -X PATCH \
  -H "Authorization: token $GITEA_TOKEN" \
  https://\${GITEA_HOST:-gitea.com}/api/v1/repos/$TEA_OWNER/$TEA_REPO/issues/42 \
  -d '{"state": "closed"}'

# DELETE
curl -s -X DELETE \
  -H "Authorization: token $GITEA_TOKEN" \
  https://\${GITEA_HOST:-gitea.com}/api/v1/repos/$TEA_OWNER/$TEA_REPO/issues/42/labels/bug
