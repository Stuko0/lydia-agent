...
curl -sL -H "Authorization: token $GITEA_TOKEN" \
  https://\${GITEA_HOST:-gitea.com}/api/v1/repos/$TEA_OWNER/$TEA_REPO/actions/runs/<RUN_ID>/logs \
...
2. Check if secrets are configured (missing `GITEA_TOKEN` or custom secrets)

**Common fixes:**
- Add `permissions:` block to workflow YAML
- Verify secrets exist: `tea secret list` or check repo settings
...
# Then monitor
tea pulls ci --watch 2>/dev/null || \
  echo "Poll with: curl -s -H 'Authorization: token ...' https://\${GITEA_HOST:-gitea.com}/api/v1/repos/.../commits/$(git rev-parse HEAD)/status"
```
