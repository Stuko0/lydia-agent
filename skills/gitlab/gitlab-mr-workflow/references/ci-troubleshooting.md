...
curl -sL -H "PRIVATE-TOKEN: $GITLAB_TOKEN" \
  https://\${GITLAB_HOST:-gitlab.com}/api/v4/projects/$GL_OWNER/$GL_REPO/actions/runs/<RUN_ID>/logs \
...
2. Check if secrets are configured (missing `GITLAB_TOKEN` or custom secrets)

**Common fixes:**
- Add `permissions:` block to workflow YAML
- Verify secrets exist: `glab secret list` or check repo settings
...
# Then monitor
glab mr ci --watch 2>/dev/null || \
  echo "Poll with: curl -s -H 'PRIVATE-TOKEN: ...' https://\${GITLAB_HOST:-gitlab.com}/api/v4/projects/.../repository/commits/$(git rev-parse HEAD)/status"
```
