...
def server():
    with patch.dict(
        "sys.modules",
        {
            "lydia_constants": MagicMock(
                get_lydia_home=MagicMock(return_value="/tmp/lydia_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'lydia-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'lydia-release' patched"
    }
