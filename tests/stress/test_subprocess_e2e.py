...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves lydia_cli.kanban_db on its own
  ...
  - worker logs are captured to LYDIA_HOME/kanban/logs/<task>.log
...
def main():
    ...
    # Point the `lydia` CLI child processes will run at the worktree
    # lydia_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "lydia")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m lydia_cli.main "$@"
""")
