...
Run from the PR worktree:
    ...
    cd ~/.lydia/lydia-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.lydia/lydia-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.lydia/lydia-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a lydia-agent checkout"
)
...
# Isolated LYDIA_HOME for the config write.
