...
engine plugins (e.g. lydia-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``lydia plugins`` picker."""
    from lydia_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "lydia_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "lydia_cli.plugins.get_plugin_context_engine",
