...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate LYDIA_HOME under tmp_path."""
    monkeypatch.setenv("LYDIA_HOME", str(tmp_path / ".lydia"))
    (tmp_path / ".lydia").mkdir()

    # Force the constants/image cache helpers to re-read LYDIA_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("lydia_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_lydia_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under LYDIA_HOME — gateway cleanup
