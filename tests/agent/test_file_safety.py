...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_lydia_env(self):
        """Lydia' own .env inside LYDIA_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: lydia internal .env is in ~/.lydia/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.lydia/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.lydia/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Lydia cache files must remain blocked."""
    ...
    def test_hub_index_cache_blocked(self, tmp_path):
        ...
        with patch("agent.file_safety._lydia_home_path", return_value=lydia_home):
            ...
            assert error is not None
            assert "internal Lydia cache" in error
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_lydia_home(self, tmp_path):
        """The env basename guard does not depend on LYDIA_HOME resolution."""
        lydia_home = tmp_path / ".lydia"
        lydia_home.mkdir()
