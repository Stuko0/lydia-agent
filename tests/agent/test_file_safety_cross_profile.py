"""Tests for the cross-Lydia-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another Lydia profile's
...
Reference: May 2026 incident — a lydia-security profile session
accidentally edited skills under both ~/.lydia/profiles/lydia-security/skills/
AND ~/.lydia/skills/ (the default profile's skills), realizing only
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake Lydia root with two profiles, monkeypatch the
...
class TestResolveActiveProfileName:
    ...
    def test_named_profile(self, fake_lydia, monkeypatch):
        ...
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "lydia-security"

    def test_falls_back_to_default_on_resolution_failure(self, fake_lydia, monkeypatch):
        """If LYDIA_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs
...
class TestClassifyCrossProfileTarget:
    ...
    def test_security_writing_default_skill(self, fake_lydia, monkeypatch):
        ...
        assert result["active_profile"] == "lydia-security"
    ...
    def test_default_writing_security_skill(self, fake_lydia, monkeypatch):
        ...
        assert result["target_profile"] == "lydia-security"
