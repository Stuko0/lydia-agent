...
""",
        ...
        from lydia_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        lydia_home = tmp_path / ".lydia"
        lydia_home.mkdir()
        (lydia_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("LYDIA_HOME", str(lydia_home))
        from lydia_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from lydia_cli.plugins import PluginContext, PluginManifest, PluginManager
