...
Companion to ``tests/lydia_cli/test_xai_oauth_writethrough.py``. That file
...
from lydia_cli import auth as A
...
def profile_and_root(tmp_path, monkeypatch):
    ...
    only refuses the *real* user's ``$HOME/.lydia/auth.json``; a tmp_path
