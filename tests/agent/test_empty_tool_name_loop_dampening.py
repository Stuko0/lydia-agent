...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated LYDIA_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="lydia_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".lydia"))
    prev_home = os.environ.get("LYDIA_HOME")
    os.environ["LYDIA_HOME"] = os.path.join(test_home, ".lydia")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("lydia_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["LYDIA_HOME"] = prev_home
