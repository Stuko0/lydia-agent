...
def lydia_home_with_config(tmp_path, monkeypatch):
    """Isolated ``~/.lydia/`` with a config.yaml referencing one external dir."""
    home = tmp_path / ".lydia"
    home.mkdir()
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("LYDIA_HOME", str(home_a))
    assert get_external_skills_dirs() == [ext_a.resolve()]

    monkeypatch.setenv("LYDIA_HOME", str(home_b))
