...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Lydia passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("LYDIA_HOME", "/users/alice/.lydia/profiles/backend-worker")
        monkeypatch.setenv("LYDIA_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "LYDIA_KANBAN_DB",
            "/users/alice/.lydia/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.lydia/kanban/boards/smoke"]'
            in cmd
        )
