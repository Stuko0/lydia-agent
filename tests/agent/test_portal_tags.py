...
def test_lydia_client_tag_includes_current_version():
    """The client tag must reflect lydia_cli.__version__ verbatim."""
    from lydia_cli import __version__
    from agent.portal_tags import lydia_client_tag

    assert lydia_client_tag() == f"client=lydia-client-v{__version__}"


def test_lydia_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import lydia_client_tag

    tag = lydia_client_tag()
    assert tag.startswith("client=lydia-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import lydia_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=lydia-agent" in tags
    assert lydia_client_tag() in tags
    assert len(tags) == 2
