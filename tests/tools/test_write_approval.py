...
and the shared slash-command handlers (lydia_cli/write_approval_commands.py).
...
def lydia_home(monkeypatch):
    ...
    home = os.path.join(d, ".lydia")
    os.makedirs(home)
    monkeypatch.setenv("LYDIA_HOME", home)
    yield home
...
def test_load_on_disk_store_honors_configured_char_limits(lydia_home, monkeypatch):
    ...
    monkeypatch.setattr("lydia_cli.config.load_config", _boom)
...
def test_skill_gate_on_then_apply_writes_file(lydia_home):
    ...
    # this test's LYDIA_HOME to exercise the real on-disk write path.
    import importlib
    import tools.skill_manager_tool as smt
    importlib.reload(smt)
    from tools import write_approval as wa
