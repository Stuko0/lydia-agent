...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._lydia_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(lydia_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(lydia_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_lydia_bin(self):
        """After _INSTALL_FAILED, manual install in LYDIA_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        lydia_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(lydia_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(lydia_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._lydia_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == lydia_bin
            assert _tirith_mod._resolved_path == lydia_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# LYDIA_HOME isolation
# ---------------------------------------------------------------------------

class TestLydiaHomeIsolation:
    def test_lydia_bin_dir_respects_lydia_home(self):
        """_lydia_bin_dir must use LYDIA_HOME, not hardcoded ~/.lydia."""
        from tools.tirith_security import _lydia_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"LYDIA_HOME": tmpdir}):
            result = _lydia_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_lydia_home(self):
        """_failure_marker_path must use LYDIA_HOME, not hardcoded ~/.lydia."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"LYDIA_HOME": "/custom/lydia"}):
            result = _failure_marker_path()
        assert result == "/custom/lydia/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets LYDIA_HOME; verify it's active."""
        lydia_home = os.getenv("LYDIA_HOME")
        assert lydia_home is not None, "LYDIA_HOME should be set by conftest"
        assert "lydia_test" in lydia_home, "Should point to test temp dir"

    def test_get_lydia_home_fallback(self):
        """Without LYDIA_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_lydia_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove LYDIA_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".lydia")
            result = _get_lydia_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._lydia_bin_dir",
