...
class TestExecuteCodeModeIntegration(unittest.TestCase):
    ...
    def test_project_mode_interpreter_is_venv_python(self):
        ...
        # The lydia-agent venv is always active during tests, so this also
    ...
    def test_project_mode_can_still_import_lydia_tools(self):
        """Regression: lydia_tools still importable from non-tmpdir CWD.

        This is the PYTHONPATH fix — without it, switching to session CWD
        breaks `from lydia_tools import terminal`.
        """
        import tempfile
        with tempfile.TemporaryDirectory() as td:
            code = (
