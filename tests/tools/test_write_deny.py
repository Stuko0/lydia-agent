...
class TestWriteDenyExactPaths:
    ...
    def test_lydia_env(self):
        # ``.env`` under the active LYDIA_HOME (profile-aware, not just
        # ``~/.lydia``) must be write-denied. The hermetic test conftest
        # points LYDIA_HOME at a tempdir — resolve via get_lydia_home()
        # to match the denylist.
        from lydia_constants import get_lydia_home
        path = str(get_lydia_home() / ".env")
        assert _is_write_denied(path) is True
    ...
    def test_lydia_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.lydia/.env`` (whose credentials are inherited by every profile)
        ...
        # Sanity check: LYDIA_HOME does point to the profile dir, not the root.
        from lydia_constants import get_lydia_home, get_default_lydia_root
        assert get_lydia_home() == profile_home
