...
def _save_config_env_sync_keys() -> set[str]:
    """terminal config keys bridged by ``lydia config set foo bar``.
...
def test_save_config_set_supports_critical_bridged_keys():
    """``lydia config set terminal.X true`` must propagate to .env for
...
def test_docker_extra_args_is_bridged_everywhere():
    ...
    present in DEFAULT_CONFIG, TERMINAL_CONFIG_ENV_MAP (so ``lydia config
...
def test_docker_volumes_is_bridged_everywhere():
    ...
    ``lydia config set terminal.docker_volumes '["/host:/workspace"]'`` wrote
...
def test_docker_forward_env_is_bridged_everywhere():
    ...
    ``lydia config set terminal.docker_forward_env '["GITHUB_TOKEN"]'`` had no
