...
def _clean_state():
    ...
    saved = {}
    for k in ("LYDIA_INTERACTIVE", "LYDIA_GATEWAY_SESSION", "LYDIA_EXEC_ASK", "LYDIA_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["LYDIA_INTERACTIVE"] = "1"
        session_key = os.getenv("LYDIA_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No LYDIA_INTERACTIVE or LYDIA_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
