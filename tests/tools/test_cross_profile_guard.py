...
def fake_lydia(tmp_path, monkeypatch):
    """Build a two-profile Lydia layout and point LYDIA_HOME at
    the lydia-security profile (matching the original-incident shape).
    """
    root = tmp_path / "fake-lydia"
    (root / "skills" / "shared-skill").mkdir(parents=True)
    (root / "skills" / "shared-skill" / "SKILL.md").write_text(
        "---\nname: shared-skill\ndescription: default copy.\n---\n"
    )

    sec_home = root / "profiles" / "lydia-security"
    (sec_home / "skills").mkdir(parents=True)

    coder_home = root / "profiles" / "coder"
    (coder_home / "skills").mkdir(parents=True)

    monkeypatch.setenv("LYDIA_HOME", str(sec_home))

    import lydia_constants
    monkeypatch.setattr(lydia_constants, "get_default_lydia_root", lambda: root)

    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_lydia_home_path", lambda: sec_home)
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_lydia):
        from tools.file_tools import write_file_tool
        target = fake_lydia["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_lydia):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_lydia["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "lydia-security" in result["error"]
        # File untouched.
        assert target.read_text() == original

    def test_cross_profile_True_bypass(self, fake_lydia):
    ...
    def test_non_lydia_path_unaffected(self, fake_lydia, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_lydia):
    ...
    def test_cross_profile_patch_bypass(self, fake_lydia):
        from tools.file_tools import patch_tool
        target = fake_lydia["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_lydia):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_lydia, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_lydia["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up LYDIA_HOME (set in
        ...
        assert "not found in active profile 'lydia-security'" in err
        assert "default" in err
        assert "cross_profile=True" in err

    def test_error_names_multiple_profiles(self, fake_lydia, monkeypatch):
        """When the skill exists in TWO other profiles, both should be named."""
        self._make_skill_in_profile(fake_lydia["root"], "everywhere-skill")
        self._make_skill_in_profile(fake_lydia["coder_home"], "everywhere-skill")
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.lydia/profiles/<name>/."""
        # Don't set LYDIA_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_lydia_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_lydia_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_lydia):
        """When active profile is 'lydia-security', the prompt warns
        ...
        assert "Active Lydia profile" in src
        assert "cross_profile=True" in src
        assert "~/.lydia/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Lydia profile: default" in src
        assert "Active Lydia profile: {active_profile}" in src
