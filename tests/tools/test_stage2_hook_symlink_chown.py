...
def test_chown_helper_refuses_symlinked_directories(stage2_text: str, tmp_path: Path) -> None:
    ...
    real_home.mkdir()
    symlinked_home = tmp_path / "lydia-home"
    try:
...
def test_chown_helper_refuses_target_under_symlinked_home(
    ...
    assert proc.returncode == 0, proc.stderr
    assert not log_path.exists(), "must not chown through a symlinked LYDIA_HOME"
    assert "refusing recursive chown through symlinked path" in proc.stdout
...
def test_stage2_uses_symlink_safe_helper_for_lydia_home_trees(stage2_text: str) -> None:
    assert 'chown_lydia_tree "$LYDIA_HOME/$sub"' in stage2_text
    assert 'chown_lydia_tree "$LYDIA_HOME/profiles"' in stage2_text
    assert 'chown_lydia_tree "$LYDIA_HOME/cron"' in stage2_text
    assert 'chown -R lydia:lydia "$LYDIA_HOME/$sub"' not in stage2_text
    assert 'chown -R lydia:lydia "$LYDIA_HOME/profiles"' not in stage2_text
    assert 'chown -R lydia:lydia "$LYDIA_HOME/cron"' not in stage2_text


def test_stage2_skips_top_level_chown_for_symlinked_lydia_home(
    stage2_text: str,
) -> None:
    assert 'refuse_symlinked_path "chown" "$LYDIA_HOME"' in stage2_text
