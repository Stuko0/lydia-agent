...
def _restore_tool_modules():
    original_lydia_home = os.environ.get("LYDIA_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_lydia_home is None:
            ...
            os.environ["LYDIA_HOME"] = original_lydia_home
        _reset_modules(("tools", "lydia_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    lydia_cli = types.ModuleType("lydia_cli")
    lydia_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["lydia_cli"] = lydia_cli
    lydia_home = tmp_path / "lydia-home"
    os.environ["LYDIA_HOME"] = str(lydia_home)
    sys.modules["lydia_cli.config"] = types.SimpleNamespace(
        get_lydia_home=lambda: lydia_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="lydia-agent", create_if_missing=create_if_missing)
