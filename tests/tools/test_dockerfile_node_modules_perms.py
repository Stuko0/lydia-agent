...
Older images made /opt/lydia/ui-tui and /opt/lydia/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV LYDIA_TUI_DIR=/opt/lydia/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R lydia:lydia /opt/lydia/ui-tui" not in text
    assert "chown -R lydia:lydia /opt/lydia/node_modules" not in text
