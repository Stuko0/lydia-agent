"""Tests for lydia_constants module."""
...
class TestGetDefaultLydiaRoot:
    ...
    def test_lydia_home_is_docker(self, tmp_path, monkeypatch):
        """When LYDIA_HOME points outside ~/.lydia (Docker), returns LYDIA_HOME."""
    ...
    def test_lydia_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any LYDIA_HOME outside ~/.lydia is treated as the root."""
        custom = tmp_path / "my-lydia-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (LYDIA_HOME=<root>/profiles/<name>),
    ...
    def test_no_lydia_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\lydia, not ~/.lydia."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("LYDIA_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(lydia_constants.sys, "platform", "win32")

        assert get_default_lydia_root() == local_appdata / "lydia"

    def test_no_lydia_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/lydia without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("LYDIA_HOME", raising=False)
        ...
        assert get_default_lydia_root() == home / "AppData" / "Local" / "lydia"


class TestGetLydiaHome:
    """Tests for get_lydia_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When LYDIA_HOME is unset on Windows, use %LOCALAPPDATA%\\lydia."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("LYDIA_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(lydia_constants.sys, "platform", "win32")
        monkeypatch.setattr(lydia_constants, "_profile_fallback_warned", False)

        assert get_lydia_home() == local_appdata / "lydia"
...
class TestLydiaManagedNode:
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(lydia_constants.sys, "platform", "win32")
        monkeypatch.setenv("LYDIA_HOME", str(home))
        monkeypatch.setattr(lydia_constants, "node_tool_runnable", lambda path: True)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("LYDIA_HOME", str(home))
        monkeypatch.setenv("PATH", str(bin_dir))
        monkeypatch.setattr(lydia_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(lydia_constants, "heal_lydia_managed_node", lambda: False)
        monkeypatch.setattr(
            lydia_constants,
            "node_tool_runnable",
            lambda path: False,
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Lydia-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(lydia_constants, "heal_lydia_managed_node", _heal)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(lydia_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(lydia_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".lydia"
...
class TestAgentBrowserRunnable:
    ...
    def test_version_probe_uses_windows_hide_flags(self, tmp_path, monkeypatch):
        ...
        import lydia_cli._subprocess_compat as subprocess_compat
