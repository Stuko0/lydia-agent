"""Shared fixtures for the lydia-agent test suite.
...
2. **Isolated LYDIA_HOME.** LYDIA_HOME points to a per-test tempdir so
   code reading ``~/.lydia/*`` via ``get_lydia_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".lydia"`` instead of the canonical
   ``get_lydia_home()`` is a bug to fix at the callsite.)
...
_LYDIA_BEHAVIORAL_VARS = frozenset({
    ...
    # the real ~/.lydia/kanban.db instead of the per-test LYDIA_HOME.
    "LYDIA_KANBAN_DB",
    "LYDIA_KANBAN_BOARD",
    "LYDIA_KANBAN_HOME",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and LYDIA_HOME to per-test tempdirs so code that
    reads ``~/.lydia/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 3. Redirect LYDIA_HOME to a per-test tempdir. Code that reads
    #    ``~/.lydia/*`` via ``get_lydia_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.lydia/*`` via
    #    ``Path.home() / ".lydia"`` instead of ``get_lydia_home()``
    ...
    monkeypatch.setenv("LYDIA_HOME", str(fake_lydia_home))
    ...
    #    ~/.lydia/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import lydia_cli.plugins as _plugins_mod
...
@pytest.fixture()
def mock_config():
    """Return a minimal lydia config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``lydia-gateway`` process
...
# correlated with a ``tests/lydia_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> lydia-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart lydia-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting lydia/python patterns are also blocked.
    """
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch lydia-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m lydia_cli.main".
                if (
                    "lydia" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live lydia-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting lydia/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `lydia update` (or the
        # equivalent `python -m lydia_cli.main update`).  These commands
        ...
        if "update" in low and (
            # lydia update / lydia update --gateway / setsid bash -c ... lydia update
            ("lydia" in low and "update" in low.split())
            or
            # python -m lydia_cli.main update --gateway
            ("lydia_cli" in low and "update" in low.split())
            or
            # venv/bin/lydia update  (absolute path variant used in tests)
            (".venv/bin/lydia" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`lydia update` against the real checkout, fetching "
