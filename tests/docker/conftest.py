...
Override the image with ``LYDIA_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("LYDIA_TEST_IMAGE", "lydia-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``LYDIA_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("LYDIA_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged lydia user
# ---------------------------------------------------------------------------
#
# Background: every Lydia runtime path inside the container drops to UID
# 10000 (the ``lydia`` user) via ``s6-setuidgid lydia``. ``docker exec``
...
# fails (PermissionError swallowed) for lydia, so a test that ran as root
...
# helpers below run every probe under ``-u lydia`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: lydia).
