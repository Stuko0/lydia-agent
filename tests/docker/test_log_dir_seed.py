"""Runtime smoke test for Docker $LYDIA_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the lydia user after container boot.
...
registered later runs its log service as the dropped lydia user and
...
def test_logs_gateways_seeded_and_lydia_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by lydia after boot."""
    ...
    assert "logs=lydia" in r.stdout, (
        f"logs/ not owned by lydia: {r.stdout}"
    )
    assert "gateways=lydia" in r.stdout, (
        f"logs/gateways/ not owned by lydia: {r.stdout}"
    )
