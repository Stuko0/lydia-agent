...
Build the real image and verify at runtime:
  ...
  1. /opt/lydia is not writable by the lydia user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and LYDIA_DISABLE_LAZY_INSTALLS are set
  3. /opt/lydia/.install_method contains "docker" (code-scoped stamp)
  4. $LYDIA_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $LYDIA_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_lydia(
    ...
    """The lydia user must not be able to modify /opt/lydia.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/lydia as the lydia user
        "touch /opt/lydia/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"lydia user can write to /opt/lydia (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/lydia/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"lydia user can write to /opt/lydia/.venv: {r.stdout}"
    )
...
def test_install_method_stamp_is_code_scoped(
    ...
    /opt/lydia/.install_method (code-scoped), NOT in $LYDIA_HOME."""
    start_container(built_image, container_name)

    # Code-scoped stamp must exist and say "docker"
    r = docker_exec_sh(
        container_name,
        "cat /opt/lydia/.install_method",
        timeout=10,
    )
    assert r.returncode == 0, (
        f"/opt/lydia/.install_method not found: {r.stderr}"
    )
    assert r.stdout.strip() == "docker", (
        f"expected 'docker' stamp, got: {r.stdout.strip()!r}"
    )

    # $LYDIA_HOME must NOT have a 'docker' stamp
    r = docker_exec_sh(
        container_name,
    ...
    assert r.stdout.strip() != "docker", (
        f"$LYDIA_HOME/.install_method is stamped 'docker' - stage2 must "
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $LYDIA_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $LYDIA_HOME was not healed on boot: "
        f"{r.stdout}"
    )
