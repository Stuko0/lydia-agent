...
Build the real image and verify the actual runtime behavior:
  ...
  3. --user <lydia-uid> (10000) is allowed (supported non-root start)
"""
...
def test_user_pinned_to_lydia_uid_works(
    ...
    """docker run --user 10000:10000 (the lydia UID) must be allowed.
    ...
    assert r.returncode == 0, (
        f"--user 10000:10000 (lydia UID) was rejected: {r.stderr[-500:]}"
    )
    assert "OK" in r.stdout