...
The shim (docker/lydia-exec-shim.sh, installed at /opt/lydia/bin/lydia)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> lydia login` would write /opt/data/auth.json as
...
failed: Lydia is not logged into Nous Portal" on every message.

These tests verify:

1. ``docker exec <c> lydia …`` (defaulting to root) gets dropped to the
   lydia user before the real binary runs.
2. ``docker exec --user lydia <c> lydia …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $LYDIA_HOME from a ``docker exec`` session land
   as lydia:lydia — the actual user-visible invariant.
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``01-lydia-setup`` cont-init hook (docker/stage2-hook.sh) has
    finished seeding + ``chown lydia:lydia`` config.yaml and running the
    ...
    clobbered the root-written file's ownership back to lydia:lydia,
    failing ``test_shim_opt_out_keeps_root`` non-deterministically.

    The reliable "cont-init is done" signal is
    ``$LYDIA_HOME/logs/container-boot.log``: it is written by
    ``02-reconcile-profiles`` (lydia_cli.container_boot), which s6 runs
    *strictly after* ``01-lydia-setup`` in lexicographic order. The
...
def test_shim_drops_root_to_lydia_uid(sleep_container: str) -> None:
    ...
    We invoke `lydia` with a Python-style `-c` shim equivalent — there's no
    pure-lydia "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `lydia` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `lydia`
    do anything that writes to disk, then check the file's owner.

    Use `lydia config set` which writes config.yaml under LYDIA_HOME.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "lydia", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by lydia, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "lydia:lydia", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected lydia:lydia. "
        "The shim did not drop privileges before invoking lydia."
    )


def test_shim_short_circuits_for_non_root_exec(sleep_container: str) -> None:
    """docker exec --user lydia already runs as 10000; shim should be a no-op.
    ...
    r = subprocess.run(
        ["docker", "exec", "--user", "lydia", sleep_container,
         "lydia", "config", "set", "_test.shim_short_circuit", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, (
        f"docker exec --user lydia failed: {r.stderr!r} stdout={r.stdout!r}. "
        "If the shim mis-handled the non-root path, this would fail with EPERM."
    )

    # File still ends up lydia:lydia — orthogonally confirms uid.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "stat", "-c", "%U:%G", "/opt/data/config.yaml"],
        capture_output=True, text=True, timeout=10,
...
def test_shim_opt_out_keeps_root(sleep_container: str) -> None:
    ...
    r = subprocess.run(
        ["docker", "exec",
         ...
         sleep_container,
         "lydia", "config", "set", "_test.opt_out", "1"],
        capture_output=True, text=True, timeout=30,
    )
...
def test_shim_opt_out_strict_truthiness(
    ...
    r = subprocess.run(
        ["docker", "exec",
         ...
         sleep_container,
         "lydia", "config", "set", "_test.falsy", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "lydia:lydia", (
        ...
        f"file owner is {r.stdout.strip()!r}, expected lydia:lydia"
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/lydia/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid lydia lydia <args>` which resolves `lydia`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> lydia login`` (root) wrote
    ...
    "Lydia is not logged into Nous Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `lydia config set`-style
    writes — what matters is the *file ownership invariant* downstream
    of `_save_auth_store`. If the shim works, every file the
    `docker exec` path produces is lydia-readable.

    Specifically: pretend the operator ran `lydia login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the lydia UID. We use `lydia auth list` since that touches the
    ...
    # `lydia auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `lydia config set` first to
    # provoke a write into LYDIA_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "lydia", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: {r.stderr}"

    # The supervised UID (10000) must be able to read everything under
    # LYDIA_HOME that docker exec just wrote.
    r = subprocess.run(
        ["docker", "exec", "--user", "lydia", sleep_container,
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the lydia user "
