...
class TestToolsetConsistency:
    ...
    def test_lydia_platforms_share_core_tools(self):
        """All lydia-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on lydia-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["lydia-cli", "lydia-telegram", "lydia-discord", "lydia-whatsapp", "lydia-slack", "lydia-signal", "lydia-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_lydia_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("lydia-whatsapp")

    def test_lydia_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("lydia-api-server")
