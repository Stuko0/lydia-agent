...
from lydia_cli.profiles import _get_default_lydia_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "lydia"
        assert config.workspace_id == "lydia"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.lydia/honcho.json
        monkeypatch.setenv("LYDIA_HOME", str(tmp_path / "isolated"))
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"lydia": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"lydia": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"lydia": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="lydia-agent"
        ):
            result = config.resolve_session_name("/home/user/lydia-agent/subdir")
        assert result == "lydia-agent"
...
class TestResolveConfigPath:
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: LYDIA_HOME points at ~/.lydia/profiles/<name>, so
        # _get_default_lydia_home() must resolve back to ~/.lydia — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".lydia"
    ...
    def test_falls_back_to_global_without_lydia_home_env(self, tmp_path):
        ...
        with patch.dict(os.environ, {}, clear=False), \
            ...
            os.environ.pop("LYDIA_HOME", None)
            result = resolve_config_path()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".lydia".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".lydia"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        ...
        assert profile_host_key("default") == "lydia"

    def test_default_returns_lydia(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("LYDIA_HONCHO_HOST", None)
            os.environ.pop("LYDIA_HOME", None)
            assert resolve_active_host() == "lydia"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"LYDIA_HONCHO_HOST": "lydia.coder"}):
            assert resolve_active_host() == "lydia.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("LYDIA_HONCHO_HOST", None)
            with patch("lydia_cli.profiles.get_active_profile_name", return_value="coder"):
    ...
    def test_custom_profile_returns_lydia(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("LYDIA_HONCHO_HOST", None)
            with patch("lydia_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "lydia"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("LYDIA_HONCHO_HOST", None)
            # Temporarily remove lydia_cli.profiles to simulate import failure
            saved = sys.modules.get("lydia_cli.profiles")
            sys.modules["lydia_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "lydia"
            finally:
                if saved is not None:
                    sys.modules["lydia_cli.profiles"] = saved
                else:
                    sys.modules.pop("lydia_cli.profiles", None)
...
class TestProfileScopedConfig:
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "lydia": {"aiPeer": "lydia", "peerName": "alice"},
                "lydia_coder": {
                    "aiPeer": "lydia_coder",
    ...
    def test_from_global_config_reads_legacy_dot_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "lydia.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        config = HonchoClientConfig.from_global_config(
            host="lydia_dreamer",
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"lydia": {"enabled": True, "aiPeer": "lydia"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"lydia": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_lydia_config_timeout_override_used_when_config_timeout_missing(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("lydia_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_defaults_to_30s_when_no_timeout_configured(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("lydia_cli.config.load_config", return_value={}):
            client = get_honcho_client(cfg)
    ...
    def test_lydia_request_timeout_alias_used(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("lydia_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
