...
The ``lydia update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m lydia_cli.main gateway run`` — image name
    ``pythonw``, not ``lydia.exe`` — so the ``taskkill /IM lydia.exe`` guard
    ...
    # The lydia.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM lydia.exe' in text
