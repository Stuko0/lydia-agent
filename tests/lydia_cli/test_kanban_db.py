"""Tests for the Kanban DB layer (lydia_cli.kanban_db)."""
...
from lydia_cli import kanban_db as kb


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated LYDIA_HOME with an empty kanban DB."""
    home = tmp_path / ".lydia"
    home.mkdir()
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import lydia_cli.kanban_db as _kb
...
def test_dispatch_skips_nonspawnable_into_separate_bucket(kanban_home, monkeypatch):
    ...
    from lydia_cli import profiles
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Lydia profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="lydia-task", assignee="daily")
...
def test_dispatch_worktree_task_persists_materialized_workspace_and_branch(kanban_home, tmp_path, monkeypatch):
    ...
    import lydia_cli.profiles as profiles
...
def test_cleanup_workspace_removes_managed_scratch_dir(kanban_home):
    ...
    with kb.connect() as conn:
        ...
        assert ws.is_dir()
        kb.complete_task(conn, t, result="ok")
    assert not ws.exists(), "Lydia-managed scratch dir should be cleaned up"
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Lydia' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Lydia' DB/metadata/logs on task completion.
    """
...
# spawned with `lydia -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's LYDIA_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's LYDIA_HOME."""
    ...
    def test_default_install_anchors_at_home_dot_lydia(
        ...
        # Standard install: LYDIA_HOME == ~/.lydia, no profile active.
        default_home = tmp_path / ".lydia"
        default_home.mkdir()
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.lydia/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.lydia/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.lydia/kanban.db.
        default_home = tmp_path / ".lydia"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # LYDIA_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `lydia -p coder`).
        monkeypatch.setenv("LYDIA_HOME", str(profile_home))
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: LYDIA_HOME=/opt/lydia/profiles/coder;
        # `get_default_lydia_root()` walks up to /opt/lydia because
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LYDIA_HOME", str(default_home))
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # LYDIA_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile LYDIA_HOME and reads.
        monkeypatch.setenv("LYDIA_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # `-p <profile>` flag rewrites LYDIA_HOME.
        default_home = tmp_path / ".lydia"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see lydia_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    with _patch("lydia_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="lydia_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'lydia kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
#
# Workers spawned by the dispatcher must use a `lydia` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["lydia", ...]`
...
def test_resolve_lydia_argv_prefers_path_shim(monkeypatch):
    """When `lydia` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import lydia_cli.kanban_db as kb

    monkeypatch.delenv("LYDIA_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/lydia")
    argv = kb._resolve_lydia_argv()
    assert argv == ["/usr/local/bin/lydia"]
...
def test_resolve_lydia_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("LYDIA_BIN", ".\\lydia.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_lydia_argv() == [os.path.abspath(".\\lydia.exe")]
...
def test_resolve_lydia_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "lydia.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("LYDIA_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_lydia_argv() == [sys.executable, "-m", "lydia_cli.main"]
...
def test_resolve_lydia_argv_lydia_bin_bare_name_uses_path(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("LYDIA_BIN", "lydia")
...
def test_resolve_lydia_argv_lydia_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    ...
    (tmp_path / "lydia.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_lydia_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m lydia_cli.main`.

    Pins the correct module name (NOT `lydia` — there is no top-level
    `lydia` package). Regression for #23198: the original PR shipped
    `python -m lydia` which fails with `No module named lydia` on every
    invocation.
    """
    import shutil
    import sys
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_lydia_argv()
    assert argv == [sys.executable, "-m", "lydia_cli.main"]
...
def test_resolve_lydia_argv_module_actually_runs():
    ...
    sufficient — if `lydia_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m lydia_cli.main --version`
    ...
    assert "Lydia Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="lydia_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_healthy_commit_no_exception(tmp_path):
    ...
    from lydia_cli.kanban_db import connect, write_txn
    db = tmp_path / "test.db"
...
def test_write_txn_raises_on_truncated_file(tmp_path):
    ...
    with pytest.raises(sqlite3.DatabaseError, match="torn-extend|page count mismatch"):
        with unittest.mock.patch("lydia_cli.kanban_db.os.path.getsize", side_effect=fake_getsize):
            with write_txn(conn) as c:
                c.execute(
                    "INSERT INTO tasks (id, title, assignee, status, priority, created_at) "
...
def test_write_txn_post_commit_check_fires_every_call(tmp_path):
    ...
    from lydia_cli.kanban_db import connect, write_txn
    import lydia_cli.kanban_db as kanban_db_module
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
    call_count = 0
...
def test_connect_sets_wal_autocheckpoint_100(tmp_path):
    ...
    from lydia_cli.kanban_db import connect
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    import struct
    from lydia_cli.kanban_db import connect, _check_file_length_invariant
...
def test_reap_worker_zombies_returns_count():
    ...
    with patch("lydia_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("lydia_cli.kanban_db._record_worker_exit"):
...
def test_reap_worker_zombies_noop_on_windows(monkeypatch):
    ...
    monkeypatch.setattr("lydia_cli.kanban_db.os.name", "nt")
    with patch("lydia_cli.kanban_db.os.waitpid") as mock_waitpid:
...
def test_reap_worker_zombies_noop_no_children():
    ...
    with patch("lydia_cli.kanban_db.os.waitpid", side_effect=ChildProcessError):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_reap_worker_zombies_records_exit_status():
    ...
    with patch("lydia_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch(
            "lydia_cli.kanban_db._record_worker_exit",
...
def test_reap_worker_zombies_handles_waitpid_os_error():
    ...
    with patch("lydia_cli.kanban_db.os.waitpid", side_effect=OSError("test error")):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_zombie_reaper_survives_all_boards_failing():
    ...
    for tick in range(5):
        ...
        with patch(
            "lydia_cli.kanban_db.os.waitpid", side_effect=make_fake_waitpid(pids)
        ):
            with patch("lydia_cli.kanban_db._record_worker_exit"):
                pids = kb.reap_worker_zombies()
...
def test_dispatch_once_still_reaps_via_extracted_fn(kanban_home):
    ...
    with patch("lydia_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("lydia_cli.kanban_db._record_worker_exit"):
            with patch("lydia_cli.kanban_db.os.name", "posix"):
                pids = kb.reap_worker_zombies()
