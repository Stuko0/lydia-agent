"""Tests for lydia_cli/completion.py — shell completion script generation."""
...
from lydia_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real lydia structure."""
    p = argparse.ArgumentParser(prog="lydia")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "complete -F _lydia_completion lydia" in out
...
class TestGenerateZsh:
    ...
    def test_registers_compdef_instead_of_invoking_completion_function(self):
        out = generate_zsh(_make_parser())
        assert 'compdef _lydia lydia' in out
        ...
        try:
            result = subprocess.run(
                [
                    "zsh",
                    "-fc",
                    f"autoload -Uz compinit && compinit -D; source {path}; [[ ${{_comps[lydia]}} == _lydia ]]",
                ],
                capture_output=True,
                text=True,
            )
...
class TestGenerateFish:
    def test_disables_file_completion(self):
        ...
        assert "complete -c lydia -f" in out
...
class TestProfileCompletion:
    ...
    def test_bash_has_profiles_helper(self):
        ...
        assert 'profiles_dir="$HOME/.lydia/profiles"' in out
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'lydia profile use', complete with profile names."""
        out = generate_bash(_make_parser())
    ...
    def test_zsh_has_profiles_helper(self):
        out = generate_zsh(_make_parser())
        assert "_lydia_profiles()" in out
        assert "$HOME/.lydia/profiles" in out
