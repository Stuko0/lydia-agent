"""Tests for lydia backup and import commands."""
...
def _make_lydia_tree(root: Path) -> None:
    """Create a realistic ~/.lydia directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "lydia_state.db").write_bytes(b"fake-state")
    ...
    # lydia-agent repo (should be EXCLUDED)
    (root / "lydia-agent").mkdir(exist_ok=True)
    (root / "lydia-agent" / "run_agent.py").write_text("# big file\n")
    (root / "lydia-agent" / ".git").mkdir()
    (root / "lydia-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_lydia_agent(self):
        from lydia_cli.backup import _should_exclude
        assert _should_exclude(Path("lydia-agent/run_agent.py"))
        assert _should_exclude(Path("lydia-agent/.git/HEAD"))
    ...
    def test_includes_nested_lydia_agent_in_skills(self):
        """skills/autonomous-ai-agents/lydia-agent/ must NOT be excluded —
        only the root-level lydia-agent/ repo is skipped."""
        from lydia_cli.backup import _should_exclude
        assert not _should_exclude(Path("skills/autonomous-ai-agents/lydia-agent/SKILL.md"))
        assert not _should_exclude(Path("skills/autonomous-ai-agents/lydia-agent/sub/item.txt"))
    ...
    def test_excludes_regeneratable_dependency_and_cache_dirs(self, rel):
        """Python dep trees and tool caches under LYDIA_HOME must be skipped —
...
class TestBackup:
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        import lydia_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_excludes_lydia_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include lydia-agent/ directory."""
        lydia_home = tmp_path / ".lydia"
        lydia_home.mkdir()
        _make_lydia_tree(lydia_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "lydia-agent" in n]
            assert agent_files == [], f"lydia-agent files leaked into backup: {agent_files}"

    def test_excludes_dependency_and_cache_trees(self, tmp_path, monkeypatch):
        """A plugin venv / site-packages / pip cache under LYDIA_HOME must be
    ...
    def test_includes_nested_lydia_agent_in_skills(self, tmp_path, monkeypatch):
        """Backup includes skills/.../lydia-agent/ but NOT root lydia-agent/."""
        lydia_home = tmp_path / ".lydia"
        lydia_home.mkdir()
        _make_lydia_tree(lydia_home)

        # Add a nested lydia-agent directory inside skills (like the real layout)
        nested = lydia_home / "skills" / "autonomous-ai-agents" / "lydia-agent"
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            # Root lydia-agent must be excluded
            root_agent = [n for n in names if n.startswith("lydia-agent/")]
            assert root_agent == [], f"root lydia-agent leaked: {root_agent}"
            # Nested skill lydia-agent must be included
            assert "skills/autonomous-ai-agents/lydia-agent/SKILL.md" in names
            assert "skills/autonomous-ai-agents/lydia-agent/sub/item.txt" in names
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/lydia-backup-*.zip."""
        lydia_home = tmp_path / ".lydia"
        lydia_home.mkdir()
        ...
        zips = list(tmp_path.glob("lydia-backup-*.zip"))
        assert len(zips) == 1

    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside LYDIA_HOME."""
        lydia_home = tmp_path / ".lydia"
        lydia_home.mkdir()
        _make_lydia_tree(lydia_home)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Lydia backup."""
        from lydia_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only lydia_state.db (old wrong name) is rejected."""
        from lydia_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["lydia_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into lydia home."""
        lydia_home = tmp_path / ".lydia"
        lydia_home.mkdir()
        monkeypatch.setenv("LYDIA_HOME", str(lydia_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_strips_lydia_prefix(self, tmp_path, monkeypatch):
        """Import strips .lydia/ prefix if all entries share it."""
        lydia_home = tmp_path / ".lydia"
        lydia_home.mkdir()
        monkeypatch.setenv("LYDIA_HOME", str(lydia_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".lydia/config.yaml": "model: test\n",
            ".lydia/skills/a/SKILL.md": "# A\n",
        })
    ...
    def test_rejects_non_lydia_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a lydia backup."""
        lydia_home = tmp_path / ".lydia"
        lydia_home.mkdir()
        monkeypatch.setenv("LYDIA_HOME", str(lydia_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        # traversal file should NOT exist outside lydia home
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".lydia"
        src_home.mkdir(parents=True)
        _make_lydia_tree(src_home)

        monkeypatch.setenv("LYDIA_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from lydia_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".lydia"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("LYDIA_HOME", str(dst_home))
        ...
        # lydia-agent should NOT be present
        assert not (dst_home / "lydia-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from lydia_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without lydia markers fails validation."""
        import io
        from lydia_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_lydia(self):
        """Detects .lydia/ prefix wrapping all entries."""
        import io
        from lydia_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".lydia/config.yaml", "test")
            zf.writestr(".lydia/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".lydia/"
...
class TestBackupEdgeCases:
    def test_nonexistent_lydia_home(self, tmp_path, monkeypatch):
        """Backup exits when lydia home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".lydia"
        monkeypatch.setenv("LYDIA_HOME", str(fake_home))
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("lydia-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_lydia_home(self, tmp_path, monkeypatch):
        """Backup handles empty lydia home (no files to back up)."""
        lydia_home = tmp_path / ".lydia"
        lydia_home.mkdir()
    ...
    def test_skips_output_zip_inside_lydia(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside lydia root."""
        lydia_home = tmp_path / ".lydia"
        lydia_home.mkdir()
        (lydia_home / "config.yaml").write_text("model: test\n")
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert "lydia -p coder" in coder_wrapper
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from lydia_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    ...
    def lydia_home(self, tmp_path):
        """Create a fake LYDIA_HOME with critical state files."""
        home = tmp_path / ".lydia"
        home.mkdir()
    ...
    def test_list_snapshots(self, lydia_home):
        from lydia_cli.backup import create_quick_snapshot, list_quick_snapshots
    ...
    def test_restore_config(self, lydia_home):
        from lydia_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(lydia_home=lydia_home)
    ...
    def test_restore_nonexistent(self, lydia_home):
        from lydia_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", lydia_home=lydia_home) is False

    def test_auto_prune(self, lydia_home):
        from lydia_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
    ...
    def test_manual_prune(self, lydia_home):
        from lydia_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", lydia_home=lydia_home)
...
# ---------------------------------------------------------------------------
# Pre-update backup (lydia update safety net)
# ---------------------------------------------------------------------------
    ...
    def test_restore_rejects_manifest_rel_traversal(self, lydia_home):
        ...
        entry rather than restoring outside LYDIA_HOME."""
        from lydia_cli.backup import create_quick_snapshot, restore_quick_snapshot
        ...
        assert not escape_dst.exists(), (
            f"manifest rel traversal escaped LYDIA_HOME: {escape_dst} exists"
        )
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_in_quick_state_files(self):
        from lydia_cli.backup import _QUICK_STATE_FILES
...
class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``lydia update``
    ...
    def test_backup_contents_match_full_backup(self, lydia_home):
        ...
        ``lydia backup`` would, and should exclude the same directories."""
        from lydia_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(lydia_home=lydia_home)
        assert out is not None
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # lydia-agent repo excluded
        assert not any(n.startswith("lydia-agent/") for n in names)
    ...
    def test_skips_symlinked_files(self, lydia_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside LYDIA_HOME."""
        from lydia_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    def lydia_home(self, tmp_path, monkeypatch):
        ...
        root.mkdir()
        _make_lydia_tree(root)
        # Point LYDIA_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("LYDIA_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for lydia_cli.config + lydia_constants so they pick up LYDIA_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("lydia_cli.config") or mod == "lydia_constants":
    ...
    def test_default_disabled_is_silent(self, lydia_home, capsys):
        """With the default (``pre_update_backup: false``), ``lydia update``
    ...
    def test_config_enabled_creates_backup(self, lydia_home, capsys):
        ...
        for mod in list(_sys.modules.keys()):
            if mod.startswith("lydia_cli.config"):
...
class TestPreMigrationBackup:
    ...
    ``lydia claw migrate`` runs before mutating ~/.lydia/."""
    ...
    def test_creates_backup_under_backups_dir(self, lydia_home):
        from lydia_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(lydia_home=lydia_home)
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `lydia
    ...
    def test_backup_uses_shared_exclusion_rules(self, lydia_home):
        ...
        ``lydia backup`` / ``create_pre_update_backup`` — no drift."""
        from lydia_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(lydia_home=lydia_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_restorable_with_lydia_import(self, lydia_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Lydia
        backup — `lydia import` should accept it."""
        from lydia_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(lydia_home=lydia_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_missing_lydia_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.lydia yet — nothing to back up."""
        from lydia_cli.backup import create_pre_migration_backup
    ...
    def test_does_not_touch_pre_update_backups(self, lydia_home):
        ...
        from lydia_cli.backup import create_pre_update_backup, create_pre_migration_backup
...
class TestRestoreCronJobsIfEmptied:
    """`lydia update` config migration can leave cron/jobs.json valid-but-empty,
...
# location, NOT under LYDIA_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------
...
class TestMemoryProviderExternalPaths:
    ...
    def test_backup_captures_external_paths_under_external_prefix(self, tmp_path, monkeypatch):
        ...
        # External provider state living OUTSIDE LYDIA_HOME.
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under LYDIA_HOME,
        and credential-shaped files get 0600."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        lydia_home = dst_home / ".lydia"
        lydia_home.mkdir()
        ...
        # External state did NOT leak into LYDIA_HOME.
