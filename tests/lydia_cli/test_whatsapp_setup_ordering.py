...
Before the fix, ``lydia whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``lydia gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    lydia = home / ".lydia"
    lydia.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("LYDIA_HOME", str(lydia))
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from lydia_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("lydia_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``lydia whatsapp`` with an existing paired session and
