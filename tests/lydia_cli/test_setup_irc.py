"""Tests for IRC gateway configuration via `lydia setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Lydia install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Lydia install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#lydia")
            monkeypatch.setenv("IRC_NICKNAME", "lydia-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `lydia setup gateway` experience with IRC present."""
