...
def lydia_home(tmp_path, monkeypatch):
    ...
    # /etc/lydia on the dev/CI box can't influence the test.
    monkeypatch.setenv("LYDIA_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(lydia_home, monkeypatch):
    from lydia_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(lydia_home, monkeypatch):
    from lydia_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(lydia_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_lydia_dotenv(lydia_home=str(home))
