"""Tests for lydia_cli/goals.py — persistent cross-turn goals."""
...
def lydia_home(tmp_path, monkeypatch):
    """Isolated LYDIA_HOME so SessionDB.state_meta writes don't clobber the real one."""
    ...
    # Bust the goal-module's DB cache for each test so it re-resolves LYDIA_HOME.
    from lydia_cli import goals
...
class TestParseJudgeResponse:
    def test_clean_json_done(self):
        from lydia_cli.goals import _parse_judge_response
...
class TestGoalManager:
    ...
    def test_continuation_prompt_shape(self, lydia_home):
        ...
        mgr.set("port goal command to lydia")
        prompt = mgr.next_continuation_prompt()
        assert prompt is not None
        assert "port goal command to lydia" in prompt
...
def test_goal_command_dispatches_in_cli_registry_helpers():
    ...
    from lydia_cli.commands import COMMANDS, COMMANDS_BY_CATEGORY
...
class TestJudgeParseFailureAutoPause:
    ...
    def test_auto_pause_after_three_consecutive_parse_failures(self, lydia_home):
        ...
        from lydia_cli import goals
        from lydia_cli.goals import GoalManager, DEFAULT_MAX_CONSECUTIVE_PARSE_FAILURES
    ...
    def test_consecutive_parse_failures_persists_across_goalmanager_reloads(
        ...
        from lydia_cli import goals
        from lydia_cli.goals import GoalManager, load_goal
...
class TestGoalStateSubgoalsBackcompat:
    def test_old_state_meta_row_loads_without_subgoals(self):
        ...
        from lydia_cli.goals import GoalState
...
class TestMigrateGoalToSession:
    ...
    def test_migrates_active_goal_to_child(self, lydia_home):
        from lydia_cli.goals import save_goal, load_goal, migrate_goal_to_session, GoalState
    ...
    def test_no_goal_to_migrate_returns_false(self, lydia_home):
        from lydia_cli.goals import migrate_goal_to_session, load_goal
    ...
    def test_same_id_is_noop(self, lydia_home):
        from lydia_cli.goals import save_goal, migrate_goal_to_session, GoalState
    ...
    def test_cleared_goal_not_migrated(self, lydia_home):
        from lydia_cli.goals import save_goal, clear_goal, migrate_goal_to_session, load_goal, GoalState
...
class TestGoalContractSerialization:
    def test_roundtrip_with_contract(self):
        from lydia_cli.goals import GoalState, GoalContract
    ...
    def test_render_block_omits_empty_fields(self):
        from lydia_cli.goals import GoalContract
...
class TestGoalManagerContract:
    def test_set_with_contract(self, lydia_home):
        from lydia_cli.goals import GoalManager, GoalContract
    ...
    def test_set_contract_after_the_fact(self, lydia_home):
        ...
        assert mgr.has_contract()
        # Survives reload.
        from lydia_cli.goals import GoalManager as GM2
