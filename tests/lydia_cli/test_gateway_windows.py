"""Tests for lydia_cli.gateway_windows."""
...
import lydia_cli.setup as setup
...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("lydia_cli.config.get_lydia_home", lambda: str(lydia_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(base_pythonw), "-m", "lydia_cli.main"]
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_lydia_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".lydia"
        home.mkdir()
        monkeypatch.setattr("lydia_cli.config.get_lydia_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        missing = tmp_path / "missing" / ".lydia"
        project = tmp_path / "checkout"
        monkeypatch.setattr("lydia_cli.config.get_lydia_home", lambda: missing)
...
def test_gateway_cmd_script_uses_pythonw_without_replace_or_start_churn(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda exe: (exe.replace("python.exe", "pythonw.exe"), r"C:\\Lydia\\lydia-agent\\venv", []),
    )

    content = gateway_windows._build_gateway_cmd_script(
        r"C:\\Lydia\\lydia-agent\\venv\\Scripts\\python.exe",
        r"C:\\Lydia\\lydia-agent",
...
def test_elevated_gateway_command_uses_pythonw_hidden_console(monkeypatch):
    ...
    monkeypatch.setattr(gateway_windows.sys, "executable", r"C:\Lydia\venv\Scripts\python.exe")
...
def test_gateway_vbs_script_is_console_less(monkeypatch):
    ...
    assert "pythonw.exe" in content
    assert "lydia_cli.main" in content
    assert "gateway run" in content
    assert ", 0, False" in content  # hidden window, detached/async
    for var in ("LYDIA_HOME", "PYTHONIOENCODING", "LYDIA_GATEWAY_DETACHED", "VIRTUAL_ENV", "PYTHONPATH"):
...
def test_install_access_denied_launches_elevated_install_before_startup_fallback(monkeypatch, tmp_path, capsys):
    ...
    assert "Launched elevated Lydia gateway install prompt" in out
...
def test_install_startup_fallback_does_not_auto_spawn_when_gateway_stopped(monkeypatch, tmp_path, capsys):
    ...
    assert "lydia --profile alice gateway start" in out
...
def test_uninstall_access_denied_prompts_before_elevating(monkeypatch, tmp_path, capsys):
    ...
    assert "UAC is Windows' admin approval prompt" in out
    assert "Launched elevated Lydia gateway uninstall prompt" in out
...
# resume_pending=True) never fires when `lydia gateway stop` kills the
...
def test_stop_writes_planned_stop_marker_before_killing(monkeypatch):
    ...
    monkeypatch.setattr("lydia_cli.gateway.kill_gateway_processes", fake_kill)
    monkeypatch.setattr("lydia_cli.gateway._get_restart_drain_timeout", lambda: 5.0)
...
def test_stop_escalates_to_force_kill_when_drain_times_out(monkeypatch):
    ...
    `lydia gateway start` fails with "another instance is running".
    """
    pid = 77777
    events = []
