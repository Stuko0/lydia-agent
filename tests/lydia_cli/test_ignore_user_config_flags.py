"""Tests for --ignore-user-config and --ignore-rules flags on `lydia chat`.
...
files. In Lydia the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.lydia/config.yaml`` in ``load_cli_config()``
...
argparse → cmd_chat → cli.main() → LydiaCLI → AIAgent call chain.
"""
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``LydiaCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting LYDIA_IGNORE_RULES=1 flips LydiaCLI.ignore_rules True."""
        monkeypatch.setenv("LYDIA_IGNORE_RULES", "1")

        # Import LydiaCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of LydiaCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.LydiaCLI)
        # Replicate the exact logic from cli.py LydiaCLI.__init__:
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``lydia_cli/main.py`` must set
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real lydia parser accepts both flags."""
