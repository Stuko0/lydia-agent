"""Tests for lydia_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import lydia_cli.gui_uninstall as gu


def _make_agent(lydia_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = lydia_home / "lydia-agent"
    (agent_root / "lydia_cli").mkdir(parents=True)
    (agent_root / "lydia_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(lydia_home: Path) -> None:
    """Create the source-built GUI artifacts a `lydia desktop` run produces."""
    desktop = lydia_home / "lydia-agent" / "apps" / "desktop"
    ...
    (lydia_home / "lydia-agent" / "node_modules").mkdir(parents=True)
...
def test_agent_is_installed_venv_only(tmp_path):
    ...
    lydia_home = tmp_path / ".lydia"
    (lydia_home / "lydia-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(lydia_home) is True
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    assert (agent_root / "lydia_cli" / "__init__.py").exists()
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    lydia_home = tmp_path / ".lydia"
    _make_agent(lydia_home)
    userdata = tmp_path / "Lydia-userdata"
    userdata.mkdir()
...
def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "Lydia"."""
    home = Path("/home/tester")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))

    monkeypatch.setattr(gu.sys, "platform", "darwin")
    assert gu.desktop_userdata_dir() == home / "Library" / "Application Support" / "Lydia"

    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    assert gu.desktop_userdata_dir() == home / ".config" / "Lydia"
...
def test_userdata_dir_windows(monkeypatch):
    ...
    assert gu.desktop_userdata_dir() == Path(r"C:\Users\tester\AppData\Roaming") / "Lydia"
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    LYDIA_HOME with all the destructive externals stubbed out.
    """
    import lydia_cli.uninstall as uninstall
    ...
    monkeypatch.setattr(uninstall, "get_lydia_home", lambda: lydia_home)
    ...
    from lydia_cli import gui_uninstall as gu_mod
...
def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole LYDIA_HOME non-interactively."""
    import lydia_cli.uninstall as uninstall

    lydia_home = tmp_path / ".lydia"
    (lydia_home / "lydia-agent" / "lydia_cli").mkdir(parents=True)
    (lydia_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
...
def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m lydia_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_lydia_home", lambda: lydia_home)
    ...
    assert not (desktop / "release").exists()
    assert not (lydia_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "lydia_cli").exists()
    assert (lydia_home / "config.yaml").exists()
