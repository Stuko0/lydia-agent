"""Tests for the top-level `./lydia` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./lydia` should use `lydia_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "lydia"
    called = []

    fake_main_module = types.ModuleType("lydia_cli.main")

    def fake_main():
        called.append("lydia_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "lydia_cli.main", fake_main_module)
