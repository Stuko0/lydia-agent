"""Tests for the `lydia memory reset` CLI command.
...
- Profile-scoped reset (uses LYDIA_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake LYDIA_HOME with memory files."""
    ...
    (memories / "MEMORY.md").write_text(
        "§\nLydia repo is at ~/.lydia/lydia-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `lydia memory reset` is run.
    """
...
class TestMemoryReset:
    """Tests for `lydia memory reset` subcommand."""
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's LYDIA_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_lydia_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
