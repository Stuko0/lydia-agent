...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "lydia gateway stop",
        "lydia gateway start",
        "lydia  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "LYDIA GATEWAY RESTART",           # uppercase
    ])
    def test_lydia_gateway_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.lydia.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.lydia.gateway.plist",
        "launchctl stop ai.lydia.gateway",
        "systemctl restart lydia-gateway",
        "systemctl stop lydia-gateway.service",
        "systemctl start lydia-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill lydia gateway process",
        "pkill -f lydia.*gateway",
    ])
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_lydia_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade lydia then run lydia gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.lydia.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestGatewaySelfTargetingGuard:
    """Verify lydia gateway stop/restart refuse when _LYDIA_GATEWAY=1."""
...
class TestTerminalToolGatewayLifecycleGuard:
    ...
    Issue #37453: systemctl --user restart lydia-gateway runs as a child of the
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart lydia-gateway", force=True
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-lydia systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart lydia-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart lydia-gateway"]
