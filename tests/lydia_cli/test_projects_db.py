"""Tests for the per-profile Projects store (lydia_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Lydia Agent", folders=["/tmp/lydia"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "lydia-agent"
    assert proj.name == "Lydia Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/lydia"
    assert [f.path for f in proj.folders] == ["/tmp/lydia"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "lydia-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Lydia Agent")
    pdb.create_project(conn, name="Lydia Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["lydia-agent", "lydia-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' LYDIA_HOME.
...
def test_db_path_under_lydia_home():
    # Resolves under LYDIA_HOME (set by the autouse isolation fixture).
