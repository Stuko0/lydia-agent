...
profile switcher can target any profile's LYDIA_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via LYDIA_HOME.
"""
import pytest
import yaml
...
class TestProfileScopedMcp:
    ...
    def test_mcp_probe_runs_inside_profile_scope(
        ...
        import lydia_cli.mcp_config as mcp_config
        from lydia_constants import get_lydia_home
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same LYDIA_HOME the rest
        ...
        monkeypatch.setattr(
            "lydia_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid():
            seen_homes.append(str(get_lydia_home()))
            return None
...
class TestProfileScopedChatPty:
    ...
    def test_chat_argv_unscoped_keeps_legacy_env(self, isolated_profiles, monkeypatch):
        ...
        assert env is not None
        assert env.get("LYDIA_HOME") != str(isolated_profiles["worker_beta"])
