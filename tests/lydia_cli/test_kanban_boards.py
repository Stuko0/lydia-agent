"""Tests for the multi-board kanban layer (``lydia kanban boards …``).
...
* CLI surface: ``lydia kanban boards list/create/switch/rm``.
...
def fresh_home(tmp_path, monkeypatch):
    """Isolated LYDIA_HOME with no prior kanban state.

    The autouse hermetic conftest already nukes credentials + TZ; this
    fixture layers a per-test LYDIA_HOME plus a path-init cache reset
    ...
    # Also reset lydia_constants cache so get_default_lydia_root() re-reads.
    try:
        import lydia_constants
        lydia_constants._cached_default_lydia_root = None  # type: ignore[attr-defined]
    except Exception:
        pass
...
class TestSlugValidation:
    @pytest.mark.parametrize("good", [
        "default", "atm10-server", "lydia-agent", "proj_1", "a",
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    """Run ``lydia kanban …`` with PYTHONPATH pinned to the worktree."""
    env = dict(os.environ)
    env["PYTHONPATH"] = str(_WORKTREE)
    if env_extra:
        env.update(env_extra)
    return subprocess.run(
        [sys.executable, "-m", "lydia_cli.main", "kanban"] + args,
        env=env,
        capture_output=True,
        text=True,
        cwd=str(_WORKTREE),
