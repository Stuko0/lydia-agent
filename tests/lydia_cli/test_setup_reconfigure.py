...
- Bare `lydia setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `lydia setup --quick` runs the narrower "fill in missing items" flow.
- `lydia setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("lydia_cli.setup.ensure_lydia_home", {}),
        ("lydia_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("lydia_cli.config.is_managed", {"return_value": False}),
        ("lydia_cli.setup.load_config", {"return_value": {}}),
        ("lydia_cli.setup.save_config", {}),
        ("lydia_cli.setup.get_env_value", {"return_value": None}),
        ("lydia_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("lydia_cli.setup._print_setup_summary", {}),
        ("lydia_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("lydia_cli.setup.load_config", {"return_value": {}}),
        ("lydia_cli.setup.save_config", {}),
        ("lydia_cli.auth.get_active_provider", {"return_value": None}),
        ("lydia_cli.setup.get_env_value", {"return_value": None}),
        ("lydia_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `lydia setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="lydia_cli.setup.prompt_choice",
                quick="lydia_cli.setup._run_quick_setup",
                model="lydia_cli.setup.setup_model_provider",
                terminal="lydia_cli.setup.setup_terminal_backend",
                agent="lydia_cli.setup.setup_agent_settings",
                gateway="lydia_cli.setup.setup_gateway",
                tools="lydia_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`lydia setup --reconfigure` behaves the same as bare `lydia setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("lydia_cli.setup.prompt_choice", {"return_value": 0}),
                first="lydia_cli.setup._run_first_time_quick_setup",
            )
            from lydia_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "lydia_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["lydia", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["lydia", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["lydia", "setup"])
        try:
            main()
        except SystemExit:
            pass
