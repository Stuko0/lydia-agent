"""Tests for lydia_cli.telegram_managed_bot — QR codes, deep links, pairing."""
...
class TestGenerateDeepLink:
    ...
    def test_name_url_encoded(self):
        link = generate_deep_link(
            ...
            suggested_name="Lydia & Friends",
        )
        assert "Lydia+%26+Friends" in link
...
class TestCreatePairing:
    def test_success(self):
        ...
        mock_resp.json.return_value = {
            ...
            "deep_link": "https://t.me/newbot/LydiaSetupBot/lydia_abcdefghijklmnop_bot?name=Lydia+Agent",
            "qr_payload": "https://t.me/newbot/LydiaSetupBot/lydia_abcdefghijklmnop_bot?name=Lydia+Agent",
            "expires_at": "2026-05-18T00:00:00.000Z",
        }

        with patch(
            "lydia_cli.telegram_managed_bot.httpx.post", return_value=mock_resp
        ) as post:
            pairing = create_pairing("https://api.example.com", bot_name="Lydia Agent")
...
class TestPollForToken:
    ...
    def test_immediate_success(self):
        ...
        with patch(
            "lydia_cli.telegram_managed_bot.httpx.get", return_value=mock_resp
        ) as get:
            with patch("lydia_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_setup_result_includes_owner_user_id(self):
        ...
        with patch("lydia_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("lydia_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_invalid_ready_token_returns_none(self):
        ...
        with patch("lydia_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("lydia_cli.telegram_managed_bot.time.sleep"):
                with patch(
                    "lydia_cli.telegram_managed_bot.time.monotonic"
                ) as mock_time:
    ...
    def test_eventual_success(self):
        ...
        with patch("lydia_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("lydia_cli.telegram_managed_bot.time.sleep"):
                token = poll_for_token(
...
class TestSetupTelegramAuto:
    def test_setup_helper_exists(self):
        from lydia_cli.setup import _setup_telegram_auto

        assert callable(_setup_telegram_auto)

    def test_setup_result_passes_profile_name_for_profile_home(self, monkeypatch, tmp_path):
        from lydia_cli import setup

        seen = {}
        profile_home = tmp_path / ".lydia" / "profiles" / "oracle"
        profile_home.mkdir(parents=True)

        monkeypatch.setattr(setup, "get_lydia_home", lambda: profile_home)
        ...
        monkeypatch.setattr(
            "lydia_cli.telegram_managed_bot.auto_setup_telegram_bot_result",
    ...
    def test_profile_name_from_home_path_handles_windows_separators(self):
        from lydia_cli.setup import _profile_name_from_lydia_home

        assert (
            _profile_name_from_lydia_home(
                PureWindowsPath(r"C:\Users\test\AppData\Local\lydia\profiles\oracle")
            )
            == "oracle"
        )
