"""Tests for ``lydia dashboard register``.
...
import lydia_cli.dashboard_register as dr
...
class TestFastFails:
    def test_not_logged_in_exits_1_with_setup_hint(self, capsys):
        ...
        with patch.object(dr, "cmd_dashboard_register", dr.cmd_dashboard_register):
            with patch(
                "lydia_cli.auth.resolve_nous_access_token", side_effect=err
            ), patch("lydia_cli.config.is_managed", return_value=False):
                with pytest.raises(SystemExit) as exc:
                    dr.cmd_dashboard_register(_ns())
        assert exc.value.code == 1
        out = capsys.readouterr().out
        assert "not logged into Nous Portal" in out
        assert "lydia setup" in out
...
class TestHappyPath:
    def _run(self, *, args, account_token="tok_abc", portal="https://portal.nousresearch.com",
        ...
        with patch(
            "lydia_cli.auth.resolve_nous_access_token", return_value=account_token
        ), patch("lydia_cli.config.is_managed", return_value=False), patch.object(
            dr, "_resolve_portal_base_url", return_value=portal
        ), patch(
            "lydia_cli.config.get_env_value", side_effect=fake_get_env
        ), patch(
            "lydia_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
    ...
    def test_custom_redirect_uri_is_forwarded(self, capsys):
        captured: dict = {}
        self._run(
            args=_ns(redirect_uri="https://lydia.example.com/auth/callback"),
            captured=captured,
        )
        assert (
            captured["body"]["custom_redirect_uri"]
            == "https://lydia.example.com/auth/callback"
        )
...
class TestCustomPortalPersistence:
    ...
    def _run(self, *, args, portal, existing_portal):
        ...
        with patch(
            "lydia_cli.auth.resolve_nous_access_token", return_value="tok"
        ), patch("lydia_cli.config.is_managed", return_value=False), patch.dict(
            dr.os.environ, {}, clear=False
        ), patch.object(
            dr, "_resolve_portal_base_url", return_value=portal
        ), patch(
            "lydia_cli.config.get_env_value", side_effect=fake_get_env_value
        ), patch(
            "lydia_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
...
class TestPublicUrlPersistence:
    ...
    https://lydia.example.com/auth/callback). At serve time the dashboard auth
    ...
    def test_origin_derived_from_full_callback_path(self, capsys):
        ...
        assert saved["LYDIA_DASHBOARD_PUBLIC_URL"] == "https://lydia.example.com"
    ...
    def test_origin_preserves_port(self, capsys):
        saved = self._run(
            args=_ns(redirect_uri="https://lydia.example.com:8443/auth/callback"),
            existing_public=None,
        )
        assert saved["LYDIA_DASHBOARD_PUBLIC_URL"] == "https://lydia.example.com:8443"
    ...
    def test_public_url_equal_to_existing_is_noop(self, capsys):
        ...
        saved = self._run(
            args=_ns(redirect_uri="https://lydia.example.com/auth/callback"),
            existing_public="https://lydia.example.com",
        )
    ...
    def test_public_url_persisted_alongside_portal_url(self, capsys):
        ...
        response = {
            ...
            "name": "dreamy_tesla",
            "kind": "SELF_HOSTED",
            "custom_redirect_uri": "https://lydia.example.com/auth/callback",
            "created_at": "2026-06-04T12:00:00.000Z",
        }
        saved: dict = {}
        ...
        with patch(
            ...
            "lydia_cli.config.get_env_value", return_value=None
        ), patch(
            "lydia_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
            dr.urllib.request, "urlopen", return_value=_fake_http_ok(response)
            ...
            dr.cmd_dashboard_register(
                _ns(
                    ...
                    redirect_uri="https://lydia.example.com/auth/callback",
                )
            )
        assert saved["LYDIA_DASHBOARD_PORTAL_URL"] == "https://preview.example.com"
        assert saved["LYDIA_DASHBOARD_PUBLIC_URL"] == "https://lydia.example.com"
