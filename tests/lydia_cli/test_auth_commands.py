...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from lydia_cli.auth_commands import auth_add_command
    ...
    payload = json.loads((tmp_path / "lydia" / "auth.json").read_text())
...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_lydia_oauth_login_pure",
        lambda: {
            "access_token": token,
            "refresh_token": "refresh-token",
    ...
    entry = next(item for item in entries if item["source"] == "manual:lydia_pkce")
    assert entry["label"] == "claude@example.com"
    assert entry["source"] == "manual:lydia_pkce"
    assert entry["refresh_token"] == "refresh-token"
    assert entry["expires_at_ms"] == 1711234567000


def test_auth_add_qwen_oauth_sets_active_provider(tmp_path, monkeypatch):
    """lydia auth add qwen-oauth must set active_provider in auth.json.
    ...
    monkeypatch.setattr(
        "lydia_cli.auth.resolve_qwen_runtime_credentials",
    ...
    # pool entry from pool.add_entry() still present for lydia auth list
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "lydia_cli.auth._nous_device_code_login",
        lambda **kwargs: {
    ...
    # `lydia auth add nous` must also populate providers.nous so the
    ...
    # raises "Lydia is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_minimax_oauth_starts_login_and_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "lydia_cli.auth._minimax_oauth_login",
        lambda **kwargs: {
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`lydia auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "lydia_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``lydia auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``lydia auth list`` showed two labels sharing
    ...
    monkeypatch.setattr("lydia_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from lydia_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from lydia_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """lydia auth add xai-oauth must write providers singleton and set active_provider.
    ...
    monkeypatch.setattr(
        "lydia_cli.auth._xai_oauth_loopback_login",
        lambda **kwargs: {
            "tokens": {
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    from lydia_cli.auth_commands import auth_remove_command
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from lydia_cli.auth_commands import auth_reset_command
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`lydia logout --provider openai-codex` must still clear model.provider.
    ...
    config_text = (lydia_home / "config.yaml").read_text()
...
def test_logout_defaults_to_configured_codex_when_no_active_provider(tmp_path, monkeypatch, capsys):
    """Bare `lydia logout` should target configured Codex if auth has no active provider."""
    lydia_home = tmp_path / "lydia"
    monkeypatch.setenv("LYDIA_HOME", str(lydia_home))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}, "credential_pool": {}})
    (lydia_home / "config.yaml").write_text(
...
def test_logout_clears_stale_active_codex_without_provider_credentials(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Logged out of OpenAI Codex." in out
    auth_payload = json.loads((lydia_home / "auth.json").read_text())
...
def test_reset_config_provider_uses_atomic_yaml_write(tmp_path, monkeypatch):
    ...
    from lydia_cli.auth import _reset_config_provider
    ...
    with patch("lydia_cli.auth.atomic_yaml_write", side_effect=_boom) as mock_write:
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from lydia_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "lydia_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("lydia_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("lydia_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("lydia_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "lydia_cli.auth_commands.time.time",
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    updated = json.loads((lydia_home / "auth.json").read_text())
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from lydia_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from lydia_cli.auth import unsuppress_credential_source
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    """Re-linking codex via `lydia auth add openai-codex` must clear any suppression marker."""
    monkeypatch.setenv("LYDIA_HOME", str(tmp_path / "lydia"))
    lydia_home = tmp_path / "lydia"
    lydia_home.mkdir(parents=True, exist_ok=True)

    # Pre-existing suppression (simulating a prior `lydia auth remove`)
    ...
    payload = json.loads((lydia_home / "auth.json").read_text())
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("lydia_cli.auth._import_codex_cli_tokens", _fake_import)
    ...
    after = json.loads((lydia_home / "auth.json").read_text())
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    """`lydia auth remove xai 1` must stick even when the env var is exported
    by the shell (not written into ~/.lydia/.env).  Before PR for #13371 the
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in ~/.lydia/.env (not the shell), the
    ...
    (lydia_home / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (lydia_home / ".env").read_text().strip() == ""


def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via `lydia auth add <provider>` clears any
    ...
    from types import SimpleNamespace
    from lydia_cli.auth import is_source_suppressed
    from lydia_cli.auth_commands import auth_add_command
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    via `lydia auth remove`.  This is the gate that prevents shell-exported
...
# =============================================================================
# Unified credential-source stickiness — every source Lydia reads from has a
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import lydia_cli.copilot_auth as ca
...
def test_seed_from_singletons_respects_lydia_pkce_suppression(tmp_path, monkeypatch):
    """anthropic lydia_pkce must not re-seed from ~/.lydia/.anthropic_oauth.json when suppressed."""
    lydia_home = tmp_path / "lydia"
    lydia_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("LYDIA_HOME", str(lydia_home))

    import yaml
    (lydia_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (lydia_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["lydia_pkce"]},
    }))

    # Stub the readers so only lydia_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_lydia_oauth_credentials", lambda: {
    ...
    # lydia_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "lydia_pkce" not in active
...
def test_seed_custom_pool_respects_config_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (lydia_home / "config.yaml").write_text(yaml.dump({
        "model": {},
        "custom_providers": [
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    with patch(
        "lydia_cli.copilot_auth.resolve_copilot_token",
        return_value=("ghp_fake", "gh"),
    ), patch(
        "lydia_cli.copilot_auth.get_copilot_api_token",
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via `lydia auth add <provider>` clears ALL
...
def test_auth_remove_codex_manual_device_code_suppresses_canonical(tmp_path, monkeypatch):
    """Removing a manual:device_code entry (from `lydia auth add openai-codex`)
