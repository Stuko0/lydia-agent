...
def container_env(tmp_path, monkeypatch):
    """Set up a fake LYDIA_HOME with .container-mode file."""
    ...
    container_mode.write_text(
        ...
        "backend=podman\n"
        "container_name=lydia-agent\n"
        "exec_user=lydia\n"
        "lydia_bin=/data/current-package/bin/lydia\n"
    )
    return lydia_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("lydia_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "lydia-agent"
    assert info["exec_user"] == "lydia"
    assert info["lydia_bin"] == "/data/current-package/bin/lydia"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("lydia_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("lydia_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_lydia_home": lambda: lydia_home}), \
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=lydia-custom\n"
        "exec_user=myuser\n"
        "lydia_bin=/opt/lydia/bin/lydia\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "lydia-custom"
    assert info["exec_user"] == "myuser"
    assert info["lydia_bin"] == "/opt/lydia/bin/lydia"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from lydia_cli.main import _exec_in_container
