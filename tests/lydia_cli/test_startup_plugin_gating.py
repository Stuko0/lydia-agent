...
``lydia_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``lydia --help``, ``lydia version``,
``lydia logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``lydia --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from lydia_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["lydia", "-w", "--tui"], None),
        (["lydia", "version"], "version"),
        (["lydia", "--tui", "chat"], "chat"),
        (["lydia", "-w", "logs"], "logs"),
        (["lydia", "chat", "hello world"], "chat"),
        (["lydia", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["lydia", "-m", "gpt5", "chat"], "chat"),
        (["lydia", "--model", "gpt5", "chat", "hi"], "chat"),
        (["lydia", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["lydia", "-z", "hello world"], None),
        (["lydia", "-z", "hello", "chat"], "chat"),
        (["lydia", "--model=gpt5", "chat"], "chat"),     # inline form
        (["lydia", "--", "chat"], "chat"),               # -- terminator
        (["lydia", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["lydia", "some-plugin-cmd"], "some-plugin-cmd"),
        (["lydia", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["lydia"],                          # bare → chat
        ["lydia", "--help"],                # top-level help
        ["lydia", "-h"],
        ["lydia", "version"],               # known built-in
        ["lydia", "logs"],
        ["lydia", "gateway", "run"],
        ["lydia", "--tui"],
        ["lydia", "-w", "--tui"],
        ["lydia", "chat", "hi"],
        ["lydia", "help"],                  # accepted built-in-ish
        ["lydia", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["lydia", "meet", "join"],          # potential google_meet plugin
        ["lydia", "honcho", "status"],      # potential memory plugin
        ["lydia", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``lydia help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"lydia_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
