...
the SAME provider universe as ``lydia model`` (the CLI/TUI picker).
...
def test_every_lydia_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `lydia model` universe."""
    ...
    assert not missing, (
        "providers shown in `lydia model` but not configurable in the desktop "
