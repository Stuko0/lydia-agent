...
import lydia_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    def test_systemd_install_repairs_outdated_unit_without_force(self, tmp_path, monkeypatch):
        unit_path = tmp_path / "lydia-gateway.service"
    ...
    def test_systemd_stop_timeout_prints_status_guidance(self, monkeypatch, capsys):
        ...
        assert "lydia gateway status" in output

    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`lydia gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `lydia gateway restart` in the background; when a
    ...
    def test_run_gateway_refreshes_outdated_unit_on_boot(self, tmp_path, monkeypatch):
        ...
        via exit-code-75 (bypassing `lydia gateway restart`)."""
        unit_path = tmp_path / "lydia-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``LYDIA_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../lydia_test`` LYDIA_HOME — silently breaking
        ...
        # Realistic generated unit referencing a pytest tmpdir LYDIA_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="LYDIA_HOME=/tmp/pytest-of-alice/pytest-42/'
    ...
    def test_refresh_refuses_to_bake_any_tempdir_home_into_real_user_unit(
        ...
        """Structural guard: a manual E2E LYDIA_HOME like
        ``/tmp/lydia-e2e-41264`` carries none of the pytest markers but
        poisons the unit identically (seen live 2026-06-11 — an E2E probe ran
        ``lydia gateway restart`` with a /tmp LYDIA_HOME exported; the
        ...
        must refuse ANY temp-dir LYDIA_HOME, not just pytest-shaped ones.
        """
        unit_path = tmp_path / "lydia-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
        ...
        polluted_unit = (
            "[Service]\n"
            'Environment="LYDIA_HOME=/tmp/lydia-e2e-41264"\n'
            "WorkingDirectory=/tmp/lydia-e2e-41264\n"
        )
        monkeypatch.setattr(
            gateway_cli,
            "generate_systemd_unit",
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_var_tmp_home(self):
        unit = '[Service]\nEnvironment="LYDIA_HOME=/var/tmp/lydia-x"\n'
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="LYDIA_HOME={tmp_path}/lydia-home"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is not None

    def test_detects_tmp_home_in_launchd_plist(self):
        plist = (
            "<dict>\n  <key>LYDIA_HOME</key>\n"
            "  <string>/tmp/lydia-e2e-99999</string>\n</dict>\n"
        )
        assert (
    ...
    def test_tmp_prefixed_non_temp_path_is_accepted(self):
        ...
        unit = '[Service]\nEnvironment="LYDIA_HOME=/tmpfs-data/.lydia"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is None
...
class TestRequireServiceInstalled:
    def test_exits_with_install_hint_when_unit_missing(self, tmp_path, monkeypatch, capsys):
        ...
        assert "lydia gateway install" in out
...
class TestGeneratedSystemdUnits:
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "_lydia_home_for_target_user", lambda home: "/home/alice/.lydia")
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
...
class TestLaunchdServiceRecovery:
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.lydia.gateway.plist"
        ...
        # LYDIA_HOME resolves under the (pytest tmp) test LYDIA_HOME.
        monkeypatch.setattr(
            gateway_cli,
            "generate_launchd_plist",
            lambda: (
                "<plist>--replace\n<key>LYDIA_HOME</key>"
                "<string>/Users/alice/.lydia</string></plist>"
            ),
        )
    ...
    def test_launchd_install_falls_back_to_detached_on_bootstrap_5(self, tmp_path, monkeypatch, capsys):
        ...
        # (which would trip on the pytest-tmp test LYDIA_HOME) stays out of
        # the way — this test exercises the bootstrap-error fallback.
        monkeypatch.setattr(
            gateway_cli,
            "generate_launchd_plist",
            lambda: (
                "<plist><key>LYDIA_HOME</key>"
                "<string>/Users/alice/.lydia</string></plist>"
            ),
        )
    ...
    def test_launchd_fallback_exits_when_spawn_fails(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "nohup lydia gateway run" in out
    ...
    def test_launchd_unsupported_marker_write_and_clear(self, tmp_path, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_lydia_home", lambda: tmp_path)
...
class TestGatewaySystemServiceRouting:
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: lydia gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup lydia gateway" in out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.lydia.gateway"])
            ),
        )
...
class TestSystemUnitLydiaHome:
    """LYDIA_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'LYDIA_HOME=/home/alice/.lydia' in unit
        assert '/root/.lydia' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: LYDIA_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("LYDIA_HOME", "/root/.lydia/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'LYDIA_HOME=/home/alice/.lydia/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_lydia_home(self, monkeypatch):
        # Custom LYDIA_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("LYDIA_HOME", "/opt/lydia-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'LYDIA_HOME=/opt/lydia-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's LYDIA_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        lydia_home = str(gateway_cli.get_lydia_home().resolve())
        assert f'LYDIA_HOME={lydia_home}' in unit
...
class TestLydiaHomeForTargetUser:
    ...
    def test_remaps_default_home(self, monkeypatch):
        ...
        assert result == "/home/alice/.lydia"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._lydia_home_for_target_user("/home/alice")
        assert result == "/home/alice/.lydia/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("LYDIA_HOME", "/opt/lydia")

        result = gateway_cli._lydia_home_for_target_user("/home/alice")
        assert result == "/opt/lydia"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "lydia gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_default_lydia_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.lydia should not produce a --profile flag."""
        lydia_home = tmp_path / ".lydia"
        lydia_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LYDIA_HOME", str(lydia_home))
        result = gateway_cli._profile_arg(str(lydia_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.lydia/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".lydia" / "profiles" / "mybot"
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LYDIA_HOME", str(tmp_path / ".lydia"))
    ...
    def test_named_profile_under_target_user_root_returns_flag(self, tmp_path):
        ...
        target_root = tmp_path / "home" / "alice" / ".lydia"
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile LYDIA_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "lydia"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.lydia/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".lydia" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".lydia" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_lydia_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        root_profile = root_home / ".lydia" / "profiles" / "mybot"
        root_profile.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("LYDIA_HOME", str(root_profile))
        monkeypatch.setattr(gateway_cli, "get_lydia_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
        ...
        assert f'LYDIA_HOME={target_home / ".lydia" / "profiles" / "mybot"}' in unit
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".lydia" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.lydia.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".lydia" / "lydia-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".lydia" / "lydia-agent")

    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/lydia", str(tmp_path / "alice"))
        assert result == "/opt/lydia"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".lydia" / "lydia-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        root_home = tmp_path / "root"
        root_home.mkdir()
        project = root_home / ".lydia" / "lydia-agent"
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("LYDIA_HOME", str(root_home / ".lydia"))
        monkeypatch.setattr(gateway_cli, "get_lydia_home", lambda: root_home / ".lydia")
        ...
        # WorkingDirectory is anchored at the target user's LYDIA_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.lydia" in unit
        assert "WorkingDirectory=/home/alice/.lydia/lydia-agent" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "lydia-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "lydia-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'lydia gateway install' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'lydia gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'lydia gateway start' inside Docker exits 0 with container guidance."""
        import pytest
        ...
        out = capsys.readouterr().out
        assert "docker" in out.lower()
        assert "lydia gateway run" in out
...
class TestLegacyLydiaUnitDetection:
    ...
    older install left a ``lydia.service`` unit behind when the service was
    renamed to ``lydia-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``lydia.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``lydia-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=Lydia Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m lydia_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_legacy_lydia_service_in_system_scope(self, tmp_path, monkeypatch):
        ...
        legacy = system_dir / "lydia.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_ignores_profile_unit_lydia_gateway_coder(self, tmp_path, monkeypatch):
        ...
        Teknium's concern — ``lydia-gateway-coder.service`` is our standard
        ...
        for base in (user_dir, system_dir):
            (base / "lydia-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "lydia-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "lydia-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
    ...
    def test_ignores_unrelated_lydia_service(self, tmp_path, monkeypatch):
        """Third-party ``lydia.service`` that isn't ours stays untouched.

        If a user has some other package named ``lydia`` installed as a
        service, we must not flag it.
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "lydia.service").write_text(
            "[Unit]\nDescription=Some Other Lydia\n[Service]\n"
            "ExecStart=/opt/other-lydia/bin/daemon --foreground\n",
            encoding="utf-8",
        )
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        user_dir, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "lydia.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "lydia.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m lydia_cli.main gateway run
          - python path/to/lydia_cli/main.py gateway run
          - lydia gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m lydia_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/lydia/lydia_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/lydia gateway run --replace",
            "ExecStart=/venv/bin/python /opt/lydia/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = f"lydia.service" if i == 0 else f"lydia.service"  # same name
            # Test each variant fresh
            (user_dir / "lydia.service").write_text(
                f"[Unit]\nDescription=Old Lydia\n[Service]\n{execstart}\n",
                encoding="utf-8",
            )
            results = gateway_cli._find_legacy_lydia_units()
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "lydia.service" in out
        assert "lydia gateway migrate-legacy" in out
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "lydia.service"
...
class TestRemoveLegacyLydiaUnits:
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        assert any("--user stop lydia.service" in c for c in cmds_joined)
        assert any("--user disable lydia.service" in c for c in cmds_joined)
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo lydia gateway migrate-legacy" in out
    ...
    def test_system_scope_with_root_removes(self, tmp_path, monkeypatch, capsys):
        ...
        assert any(
            c.startswith("systemctl stop lydia.service") for c in cmds_joined
        )
        assert any(
            c.startswith("systemctl disable lydia.service") for c in cmds_joined
        )
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "lydia.service"
        system_legacy = system_dir / "lydia.service"
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (lydia-gateway-coder.service)
        ...
        profile_unit = user_dir / "lydia-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "lydia-gateway.service"
...
class TestMigrateLegacyCommand:
    """Tests for the `lydia gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import lydia_cli.main as cli_main
        ...
        # public, just confirm that `lydia gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import lydia_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "lydia_cli.main", "gateway", "--help"],
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "lydia_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "lydia-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "lydia-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "lydia-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``lydia gateway start --system``).
        self._setup_units(tmp_path, monkeypatch, system_present=False, user_present=False)
        monkeypatch.setattr(gateway_cli.os, "geteuid", lambda: 1000)
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "lydia-gateway")
        ...
        assert "sudo systemctl start lydia-gateway" in out
        assert "sudo lydia gateway uninstall --system" in out
        assert "lydia gateway install" in out
    ...
    def test_restart_remediation_uses_systemctl_restart(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl restart lydia-gateway" in out
    ...
    def test_stop_remediation_uses_systemctl_stop(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl stop lydia-gateway" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``lydia gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (LYDIA_HOME), never the source checkout / worktree, so a relocated or
    ...
    def test_stable_working_dir_uses_lydia_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".lydia"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_lydia_home", lambda: home)
    ...
    def test_stable_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        # LYDIA_HOME points somewhere that does not exist -> fall back.
        missing = tmp_path / "does-not-exist" / ".lydia"
        monkeypatch.setattr(gateway_cli, "get_lydia_home", lambda: missing)
