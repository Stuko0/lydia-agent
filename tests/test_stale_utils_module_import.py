...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.lydia/lydia-agent/utils.py)
...
2. ``lydia update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``lydia_cli/main.py`` (the ``lydia update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``lydia_constants`` after the
