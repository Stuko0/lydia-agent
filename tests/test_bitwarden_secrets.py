...
binary download are exercised manually by `lydia secrets bitwarden
...
def lydia_home(tmp_path, monkeypatch):
    """Point Lydia at an isolated home directory."""
    home = tmp_path / ".lydia"
    home.mkdir()
    monkeypatch.setenv("LYDIA_HOME", str(home))
    # Some modules cache get_lydia_home; clear if needed.
    import lydia_constants
    if hasattr(lydia_constants, "_LYDIA_HOME_CACHE"):
        lydia_constants._LYDIA_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from lydia_cli.env_loader import _apply_external_secret_sources
