"""Tests for lydia_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Lydia entry point
(lydia, lydia-agent, lydia-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Lydia entry point imports lydia_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported lydia_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("lydia_bootstrap", None)
    import lydia_bootstrap  # noqa: WPS433
    return lydia_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Lydia CLI
...
class TestEntryPointsImportBootstrap:
    """Every Lydia entry point must import lydia_bootstrap as its
    ...
    # Entry points that invoke Lydia as a process.  Each one must
    # import lydia_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "lydia_cli/main.py",   # lydia CLI (console_script)
        "run_agent.py",          # lydia-agent (console_script)
        "acp_adapter/entry.py",  # lydia-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import lydia_bootstrap' and that
        ...
        half-finished ``lydia update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``lydia_bootstrap`` as a top-level module) leaves lydia
        ...
        """
        # Resolve relative to the lydia-agent repo root.  Tests live
        # at tests/test_lydia_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # lydia start even when lydia_bootstrap hasn't been
        ...
        assert first_import_name == "lydia_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'lydia_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import lydia_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Lydia's own top-level modules — covering both
    ...
    def _run(self, hb, path_seed, env=None):
        ...
        try:
            ...
            hb.harden_import_path(src_root="/opt/lydia")
            return sys.path[:]
        finally:
            sys.path[:] = original
    ...
    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/lydia", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Lydia root.  The guard must relocate Lydia to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/lydia"])
        assert result[0] == "/opt/lydia"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Lydia root.
        assert result.index("/opt/lydia") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/lydia", "/opt/lydia", ""])
        assert result.count("/opt/lydia") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["LYDIA_PYTHON_SRC_ROOT"] = "/env/lydia"
            hb.harden_import_path()
            assert sys.path[0] == "/env/lydia"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("LYDIA_PYTHON_SRC_ROOT", None)
