...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    lydia_state = ModuleType("lydia_state")
    lydia_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "lydia_state", lydia_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Lydia"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Lydia"
