...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.lydia."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /lydia, /btw, /stop,
        ...
        for expected in ("/lydia", "/btw", "/stop", "/model", "/help"):
...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@lydia`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@lydia"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@lydia")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    # instead of as /lydia subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /lydia btw."""
        command = {
            "command": "/btw",
    ...
    async def test_legacy_lydia_prefix_still_works(self, adapter):
        """Backward compat: /lydia btw foo must still route to /btw foo.

        Old workspace manifests only declared /lydia as the single slash.
    ...
    async def test_legacy_lydia_freeform_question(self, adapter):
        """/lydia <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/lydia",
...
class TestSlashEphemeralAck:
    ...
    async def test_legacy_lydia_slash_stashes_context(self, adapter):
        """Legacy /lydia <subcommand> also stashes context."""
        command = {
            "command": "/lydia",
            "text": "help",
            "user_id": "U_H",
    ...
    async def test_freeform_lydia_question_does_not_stash_context(self, adapter):
        """Free-form /lydia <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/lydia",
