...
(/opt/lydia/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable LYDIA_HOME location instead.
"""
import importlib
...
def test_writable_install_returns_install_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "lydia_constants.get_lydia_home", lambda: lydia_home
    )

    resolved = whatsapp_common.resolve_whatsapp_bridge_dir()
    assert resolved == install_bridge
    # Nothing mirrored into LYDIA_HOME.
    assert not (lydia_home / "scripts" / "whatsapp-bridge").exists()


def test_readonly_install_mirrors_to_lydia_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable LYDIA_HOME."""
    install_root = tmp_path / "install"
    install_bridge = install_root / "scripts" / "whatsapp-bridge"
    _seed_install_tree(install_bridge)
    ...
    expected = lydia_home / "scripts" / "whatsapp-bridge"
...
def test_readonly_install_reuses_existing_mirror(tmp_path, monkeypatch):
    """If the LYDIA_HOME mirror already exists, return it without re-copying."""
    install_root = tmp_path / "install"
    install_bridge = install_root / "scripts" / "whatsapp-bridge"
    _seed_install_tree(install_bridge)

    lydia_home = tmp_path / "lydia_home"
    mirror = lydia_home / "scripts" / "whatsapp-bridge"
