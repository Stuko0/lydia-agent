...
closure the PR changed, against a real temp ``LYDIA_HOME``.
"""
...
def _fake_switch_result():
    ...
    from lydia_cli.model_switch import ModelSwitchResult
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "lydia_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
    # save_config writes to ``get_lydia_home() / config.yaml`` — point it here.
    monkeypatch.setattr("lydia_constants.get_lydia_home", lambda: lydia_home)
    monkeypatch.setattr("lydia_cli.config.get_lydia_home", lambda: lydia_home)
    return cfg_path
