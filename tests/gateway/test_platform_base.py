...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.lydia/media/'
...
class TestMediaDeliveryPathValidation:
    ...
    def test_recency_trust_allows_pdf_in_project_dir(self, tmp_path, monkeypatch):
        ...
        files outside ~/.lydia/cache/* were silently dropped, leaving the
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_lydia_credentials(self, tmp_path, monkeypatch):
        """~/.lydia/.env and ~/.lydia/auth.json stay blocked even in
    ...
    def test_denylist_blocks_shared_lydia_root_config_for_profiles(self, tmp_path, monkeypatch):
        """Profile-mode gateways must still block the shared Lydia root config."""
        self._patch_roots(monkeypatch)

        fake_home = tmp_path / "home"
        profile_home = fake_home / ".lydia" / "profiles" / "work"
        profile_home.mkdir(parents=True)
        lydia_root = fake_home / ".lydia"
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        """Integration credentials at the LYDIA_HOME root (google_token.json)
    ...
    def test_denylist_blocks_pairing_directory_contents(self, tmp_path, monkeypatch):
        """Files under ~/.lydia/pairing/ (platform pairing tokens) are
    ...
    def test_denylist_blocks_non_cache_file_under_lydia_home(self, tmp_path, monkeypatch):
        """A non-credential file the agent wrote directly under ~/.lydia
    ...
    def test_root_home_lydia_env_still_blocked(self, tmp_path, monkeypatch):
        """``~/.lydia/.env`` stays blocked under the $HOME exception — it is a
    ...
    def test_profile_scoped_cache_delivers_under_symlinked_root(self, tmp_path, monkeypatch):
        """Reopened #31733: a profile gateway whose LYDIA_HOME is symlinked
        under a denied prefix (e.g. /opt/data -> /root/.lydia) emits
...
class _CapturingAdapter(BasePlatformAdapter):
    ...
    chat text. That argument is a host filesystem path inside the Lydia
...
class TestMediaFallbackDoesNotLeakHostPath:
    ...
    a chat message like ``🎬 Video: /home/.../lydia/cache/video/abc.mp4``
    — a host filesystem path with no actionable information.
    """

    SENSITIVE_PATH = "/home/jayne/.lydia/cache/media/sensitive_host_path_abc123.bin"
