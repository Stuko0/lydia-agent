...
The root ``package.json`` is installed by ``lydia update`` on every user,
...
* ``@askjo/camofox-browser`` is NOT eager. It is an explicit opt-in
  ...
  Firefox-fork binary, which silently blocked ``lydia update`` for
...
guidance in the ``lydia-agent-dev`` skill before changing the
expectations.
"""
...
def test_camofox_is_not_in_root_dependencies() -> None:
    ...
    assert "@askjo/camofox-browser" not in deps, (
        ...
        "Camofox post_setup handler in lydia_cli/tools_config.py so it "
...
def test_agent_browser_stays_eager() -> None:
    ...
    assert "agent-browser" in deps, (
        ...
        "after `lydia setup` / `lydia update` without an explicit "
        "post_setup step."
    )
