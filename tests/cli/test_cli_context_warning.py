...
def _isolate(tmp_path, monkeypatch):
    """Isolate LYDIA_HOME so tests don't touch real config."""
    home = tmp_path / ".lydia"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal LydiaCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = LydiaCLI.__new__(LydiaCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["lydia-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Lydia' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Lydia' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Lydia' minimum context length."""
