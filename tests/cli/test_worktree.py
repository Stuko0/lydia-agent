...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("lydia/lydia-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned lydia/* and pr-* branches."""

    def test_prunes_orphaned_lydia_branch(self, git_repo):
        """lydia/lydia-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "lydia/lydia-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "lydia/lydia-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "lydia/lydia-deadbeef" in result.stdout
        ...
        assert "lydia/lydia-deadbeef" in orphaned
        ...
        assert "lydia/lydia-deadbeef" not in result.stdout
