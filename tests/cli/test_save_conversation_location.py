...
file to be discoverable via ``lydia sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.lydia/sessions/saved/`` and prints
...
def lydia_home(tmp_path, monkeypatch):
    ...
    if hasattr(lydia_constants, "_lydia_home_cache"):
        lydia_constants._lydia_home_cache = None
    return home
...
def test_save_conversation_writes_under_lydia_home(lydia_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.lydia/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the LYDIA_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "lydia_constants"]:
    ...
    assert "lydia --resume 20260101_120000_abc123" in out, out
