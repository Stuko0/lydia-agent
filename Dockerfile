...
# Do not write .pyc files at runtime: /opt/lydia is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/lydia/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when lydia
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# lydia process, the dashboard, and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main lydia process, the dashboard,
...
RUN set -eu; \
    ...
    # 'Lydia WebUI' catalog). The image has moved to s6-overlay /init\
...
RUN useradd -u 10000 -m -d /opt/data lydia
...
#
# ui-tui/packages/lydia-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/lydia-ink/ ui-tui/packages/lydia-ink/
# apps/shared/ is copied IN FULL because web/package.json references it as a
# `file:` workspace dependency (same pattern as lydia-ink above).
...
# lock on the @lydia/ink entry, tripped the TUI launcher's
...
# for the same reason: it lazy-installs into /opt/lydia/.venv at first
...
# gives the non-root lydia user read + traverse but no write; root retains
# write so the build steps below don't need chmod u+w dances.
COPY --link --chmod=a+rX,go-w . .

# ---------- Permissions ----------
# Link lydia-agent itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Wire the exec shim and install-method stamp.  Files under /opt/lydia are
# already root-owned (COPY, uv sync, npm install all run as root) and
# read-only for the lydia user (go-w from the --chmod above).

USER root
RUN mkdir -p /opt/lydia/bin && \
    cp /opt/lydia/docker/lydia-exec-shim.sh /opt/lydia/bin/lydia && \
    chmod 0755 /opt/lydia/bin/lydia && \
...
# host install's marker and wrongly block its ``lydia update``. A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the lydia user via
# `s6-setuidgid lydia` in its run script. If LYDIA_UID is unset, services
# run as the default lydia user (UID 10000).

# ---------- Bake build-time git revision ----------
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `lydia dump` reports
...
# /opt/lydia/.lydia_build_sha at build time, and have
# lydia_cli/build_info.py read it at runtime.  Both `lydia dump` and
...
RUN if [ -n "${LYDIA_GIT_SHA}" ]; then \
        printf '%s\n' "${LYDIA_GIT_SHA}" > /opt/lydia/.lydia_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-lydia + dashboard.
...
# `exec lydia`. Wired in as cont-init.d/01- so it
...
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/lydia/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-lydia-setup && \
    chmod +x /etc/cont-init.d/01-lydia-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV LYDIA_WEB_DIST=/opt/lydia/lydia_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/lydia/ui-tui/dist/entry.js`) and skip the
...
ENV LYDIA_TUI_DIR=/opt/lydia/ui-tui
ENV LYDIA_HOME=/opt/data
ENV LYDIA_WRITE_SAFE_ROOT=/opt/data
ENV LYDIA_DISABLE_LAZY_INSTALLS=1
# The published image seals /opt/lydia (root-owned, read-only) so a runtime
...
# is seeded + chowned to the lydia user by docker/stage2-hook.sh and lives
...
# `docker exec <c> lydia ...` they default to root, and any file the
# command writes under $LYDIA_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/lydia/bin/lydia, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid lydia`
# when invoked as root. Non-root callers (supervised processes,
# `--user lydia`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/lydia/.venv/bin/lydia). See the shim source for
...
# bin globally so `docker exec <container> lydia ...` and any
# subprocess that doesn't activate the venv first still find lydia.
#
# /opt/lydia/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/lydia/bin:/opt/lydia/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. lydia
# subcommand vs. no-args), drops to the lydia user via s6-setuidgid,
# and exec's the final program so its exit code becomes the container
# exit code. Without the wrapper-as-ENTRYPOINT, leading-dash args
# like `--version` would be intercepted by /init's POSIX shell.
ENTRYPOINT [ "/init", "/opt/lydia/docker/main-wrapper.sh" ]
CMD [ ]
