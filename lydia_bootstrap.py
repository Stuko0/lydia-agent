"""Windows UTF-8 bootstrap for Lydia entry points.
...
should be imported at the very top of every Lydia entry point
(``lydia``, ``lydia-agent``, ``lydia-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Lydia modules.

    Lydia ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Lydia source root
    ...
    """
    root = src_root or os.environ.get("LYDIA_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``LYDIA_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("LYDIA_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import lydia_bootstrap``
# (or ``from lydia_bootstrap import apply_windows_utf8_bootstrap``) at
