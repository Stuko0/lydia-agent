/**
 * Lydia Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.lydia/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__LYDIA_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const COLUMN_DOT = {
    triage: "lydia-kanban-dot-triage",
    todo: "lydia-kanban-dot-todo",
    ready: "lydia-kanban-dot-ready",
    running: "lydia-kanban-dot-running",
    blocked: "lydia-kanban-dot-blocked",
    done: "lydia-kanban-dot-done",
    archived: "lydia-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-lydia-task";
  ...
  const DOCS_URL = "https://lydia-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://lydia-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "lydia.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="lydia-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])lydia-kanban-md-code\1/i.test(attrs)) {
      return ' class="lydia-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "lydia-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "lydia-kanban-md",
  ...
  // standard `drop` event and our `lydia-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("lydia-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("lydia-kanban-column--drop");
          if (target) target.classList.add("lydia-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("lydia-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("lydia-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("lydia-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__LYDIA_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "lydia-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "lydia-kanban-attention",
        "lydia-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "lydia-kanban-attention-bar" },
        h("span", { className: "lydia-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "lydia-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "lydia-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "lydia-kanban-attention-row",
                  "lydia-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "lydia-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "lydia-kanban-attention-row-id" }, task.id),
                h("span", { className: "lydia-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "lydia-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "lydia-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "lydia-kanban-diag-action-btn",
      action.suggested ? "lydia-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " lydia-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".lydia-kanban-drawer-comment-row input, .lydia-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "lydia-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("lydia-kanban-diag", sevClass) },
      h("div", { className: "lydia-kanban-diag-header" },
        h("span", { className: "lydia-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "lydia-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "lydia-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "lydia-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "lydia-kanban-diag-data-row" },
                  h("span", { className: "lydia-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "lydia-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "lydia-kanban-diag-data-row" },
                h("span", { className: "lydia-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "lydia-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "lydia-kanban-diag-reassign-row" },
            h("span", { className: "lydia-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "lydia-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "lydia-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "lydia-kanban-diag-msg",
              msg.ok ? "lydia-kanban-diag-msg--ok" : "lydia-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "lydia-kanban-section" },
      h("div", { className: "lydia-kanban-section-head-row" },
        h("span", { className: "lydia-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "lydia-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "lydia-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "lydia-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "lydia-kanban-docs-link",
      title: "Open Lydia Kanban docs in a new tab",
      "aria-label": "Lydia Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `lydia kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "lydia-kanban-msg-ok" : "lydia-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "lydia-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "lydia-kanban-boardswitcher" },
      h("div", { className: "lydia-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "lydia-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "lydia-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "lydia-kanban-dialog-title" },
        ...
        h("div", { className: "lydia-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Lydia profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "lydia-kanban-bulk" },
      h("span", { className: "lydia-kanban-bulk-count" },
      ...
      h("div", { className: "lydia-kanban-bulk-priority",
      ...
      h("div", { className: "lydia-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Lydia profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "lydia-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("lydia-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("lydia-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "lydia-kanban-trash",
        dragOver ? "lydia-kanban-trash--drop" : "",
        props.draggingTaskId ? "lydia-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "lydia-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "lydia-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".lydia-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "lydia-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("lydia-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("lydia-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "lydia-kanban-column",
        dragOver ? "lydia-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "lydia-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "lydia-kanban-col-check",
        ...
        h("span", { className: cn("lydia-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "lydia-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "lydia-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "lydia-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "lydia-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "lydia-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "lydia-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "lydia-kanban-lane" },
                  h("div", { className: "lydia-kanban-lane-head" },
                    h("span", { className: "lydia-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "lydia-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "lydia-kanban-card--stale-red";
    if (age >= tier.amber) return "lydia-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".lydia-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "lydia-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "lydia-kanban-card",
        props.selected ? "lydia-kanban-card--selected" : "",
        props.failed ? "lydia-kanban-card--failed" : "",
        props.draggingSource ? "lydia-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "lydia-kanban-card-content" },
          h("div", { className: "lydia-kanban-card-row" },
            h("label", {
              className: "lydia-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "lydia-kanban-card-check",
            ...
            h("span", { className: "lydia-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or lydia kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "lydia-kanban-warning-badge",
                    "lydia-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "lydia-kanban-needs-assignee",
          ...
          h("div", { className: "lydia-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "lydia-kanban-card-row lydia-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "lydia-kanban-assignee",
                            title: `Assigned to Lydia profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "lydia-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "lydia-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "lydia-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "lydia-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Lydia profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Lydia profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "lydia-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "lydia-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "lydia-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "lydia-kanban-drawer-close",
        ...
        data ? h("div", { className: "lydia-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "lydia-kanban-section" },
      h("div", { className: "lydia-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "lydia-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "lydia-kanban-drawer-body" },
      h("div", { className: "lydia-kanban-drawer-title" },
        h("span", { className: cn("lydia-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "lydia-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "lydia-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "lydia-kanban-section" },
        h("div", { className: "lydia-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "lydia-kanban-section" },
        h("div", { className: "lydia-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "lydia-kanban-comment" },
                h("div", { className: "lydia-kanban-comment-head" },
                  h("span", { className: "lydia-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "lydia-kanban-comment-ago" },
      ...
      h("div", { className: "lydia-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "lydia-kanban-event",
              isDiag ? "lydia-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "lydia-kanban-event-header" },
                  h("span", { className: "lydia-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "lydia-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "lydia-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "lydia-kanban-event-header-plain" },
                  h("span", { className: "lydia-kanban-event-kind" }, e.kind),
                  h("span", { className: "lydia-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "lydia-kanban-event-phantom-row" },
                  h("span", { className: "lydia-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "lydia-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "lydia-kanban-event-payload" },
    ...
    return h("div", { className: "lydia-kanban-section" },
      h("div", { className: "lydia-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "lydia-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `lydia-kanban-run--${r.outcome || r.status || "ended"}`
          : "lydia-kanban-run--active";
        return h("div", { key: r.id, className: cn("lydia-kanban-run", outcomeClass) },
          h("div", { className: "lydia-kanban-run-head" },
            h("span", { className: "lydia-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "lydia-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "lydia-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "lydia-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "lydia-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "lydia-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "lydia-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "lydia-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "lydia-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "lydia-kanban-pre lydia-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "lydia-kanban-meta-row" },
      h("span", { className: "lydia-kanban-meta-label" }, props.label),
      h("span", { className: "lydia-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "lydia-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "lydia-kanban-meta-row" },
        h("span", { className: "lydia-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "lydia-kanban-meta-value lydia-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "lydia-kanban-meta-row" },
        h("span", { className: "lydia-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "lydia-kanban-meta-value lydia-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "lydia-kanban-section" },
      h("div", { className: "lydia-kanban-section-head-row" },
        h("span", { className: "lydia-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "lydia-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "lydia-kanban-section" },
      h("div", { className: "lydia-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "lydia-kanban-deps-row" },
        h("span", { className: "lydia-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "lydia-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "lydia-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "lydia-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "lydia-kanban-dep-chip-x",
      ...
      h("div", { className: "lydia-kanban-deps-row" },
        h("span", { className: "lydia-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "lydia-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "lydia-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "lydia-kanban-section" },
      ...
      h("div", { className: "lydia-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "lydia-kanban-home-sub lydia-kanban-home-sub--on"
              : "lydia-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__LYDIA_PLUGINS__ && typeof window.__LYDIA_PLUGINS__.register === "function") {
    window.__LYDIA_PLUGINS__.register("kanban", KanbanPage);
  }
})();
