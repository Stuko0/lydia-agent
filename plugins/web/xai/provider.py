...
``{title, url, description, position}`` rows every other Lydia web
provider produces.
...
prefers Lydia-managed xAI Grok OAuth (via ``lydia auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.lydia/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``lydia native`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `lydia auth` to sign in with "
