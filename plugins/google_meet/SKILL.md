...
lydia meet install                 # pip deps + Chromium (transcribe only)
lydia meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
lydia meet auth                    # optional; skips guest-lobby wait
lydia meet setup                   # preflight checks
```

`lydia meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or LYDIA_MEET_REALTIME_KEY in ~/.lydia/.env
```
...
pip install playwright websockets && python -m playwright install chromium
lydia plugins enable google_meet
lydia meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
lydia meet node approve my-mac ws://<mac-ip>:18789 <token>
lydia meet node ping my-mac                   # confirm reachable
```

Run `lydia meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Lydia Agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `lydia meet auth` avoids this.
...
Local:
```
$LYDIA_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$LYDIA_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `lydia meet node approve`.
