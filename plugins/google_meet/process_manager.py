...
session-scoped state file under ``$LYDIA_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $LYDIA_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_lydia_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this lydia install, leave it first —
    ...
    env = os.environ.copy()
    env["LYDIA_MEET_URL"] = url
    env["LYDIA_MEET_OUT_DIR"] = str(out)
    env["LYDIA_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["LYDIA_MEET_HEADED"] = "1"
    if auth_state:
        env["LYDIA_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["LYDIA_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if LYDIA_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["LYDIA_MEET_MODE"] = mode
    if realtime_model:
        env["LYDIA_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["LYDIA_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["LYDIA_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["LYDIA_MEET_REALTIME_KEY"] = realtime_api_key
