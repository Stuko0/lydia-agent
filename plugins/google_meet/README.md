...
Let the Lydia Agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where lydia runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `lydia meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `lydia meet` CLI |
...
| `cli.py` | `lydia meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$LYDIA_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `lydia meet node {run,list,approve,remove,status,ping}` |
...
lydia meet install                                      # pip + Chromium
lydia meet setup                                        # preflight
lydia meet auth                                         # optional
lydia meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
lydia meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.lydia/.env
lydia meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
lydia meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
lydia meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.lydia/.env
lydia meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, lydia will **not** switch your system audio input automatically — the
...
lydia plugins enable google_meet
lydia meet node run --display-name my-mac --host 0.0.0.0 --port 18789
# prints the bearer token on first run; copy it
```

On the gateway:
```bash
lydia meet node approve my-mac ws://<mac-ip>:18789 <token>
lydia meet node ping my-mac
