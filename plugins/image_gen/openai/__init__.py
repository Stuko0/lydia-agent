...
three virtual model IDs so the ``lydia native`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not os.environ.get("OPENAI_API_KEY"):
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `lydia native` → Image "
                    "Generation → OpenAI to configure, or `lydia setup` "
                    "to add the key."
                ),
