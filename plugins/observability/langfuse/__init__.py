"""langfuse — Lydia plugin for Langfuse observability.

Traces Lydia conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Lydia plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``lydia plugins enable
observability/langfuse`` or ``lydia native → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``lydia native`` or ~/.lydia/.env):
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Lydia plugin system —
...
def _trace_key(
    ...
    *,
    ...
    Older Lydia paths only expose ``task_id``/``session_id``. Newer paths
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Lydia branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Lydia also has a turn-scoped
    ...
    # messages is a list only for legacy Lydia branches that fired
    # pre_llm_call with API messages directly. Current Lydia fires
...
def register(ctx) -> None:
    ...
    # Lydia versions.  pre_api_request / post_api_request fire per API
