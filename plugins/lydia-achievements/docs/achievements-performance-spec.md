# Lydia Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: lydia-achievements plugin
...
### `GET /api/plugins/lydia-achievements/achievements`
...
### `POST /api/plugins/lydia-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/lydia-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/lydia-achievements/overview`
...
- Persist plugin data under:
  - `~/.lydia/plugins/lydia-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.lydia/plugins/lydia-achievements/scan_checkpoint.json`
...
- `~/.lydia/plugins/lydia-achievements/`
