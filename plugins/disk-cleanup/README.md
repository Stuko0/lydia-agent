...
Auto-tracks and cleans up ephemeral files created during Lydia Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$LYDIA_HOME` and `/tmp/lydia-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `LYDIA_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `LYDIA_HOME` or `/tmp/lydia-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$LYDIA_HOME/disk-cleanup/` is itself excluded
- `$LYDIA_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
