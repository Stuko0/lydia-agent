"""disk_cleanup — ephemeral file cleanup for Lydia Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under LYDIA_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly LYDIA_HOME and /tmp/lydia-*
Never touches: ~/.lydia/logs/ or any system directory.
"""
...
try:
    ...
    def get_lydia_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("LYDIA_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".lydia").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$LYDIA_HOME/logs/``."""
    return get_lydia_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$LYDIA_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under LYDIA_HOME or ``/tmp/lydia-*``.
    ...
    try:
        ...
        pass
    # Allow /tmp/lydia-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("lydia-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "lydia-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $LYDIA_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so LYDIA_HOME is resolved
    # exactly once.
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside LYDIA_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under LYDIA_HOME, but never recurse into known
    # durable state trees.  Some installs place the Lydia checkout, venv,
    # and desktop build under LYDIA_HOME; a full rglob over that tree can
...
def guess_category(path: Path) -> Optional[str]:
    ...
    try:
        ...
        # Path isn't under LYDIA_HOME (e.g. /tmp/lydia-*) — fall through.
        pass
