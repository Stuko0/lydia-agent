...
``lydia://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Lydia OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "lydia-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``lydia-desktop`` / ``lydia-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Lydia.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local LYDIA_HOME override can't reach.
