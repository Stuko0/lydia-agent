...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/lydia>
...
```bash
lydia memory setup honcho   # configure Honcho directly (works on a fresh install)
lydia memory setup          # generic picker, choose Honcho from the list
```
...
echo "HONCHO_API_KEY=***" >> ~/.lydia/.env
```

> `lydia honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `lydia memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$LYDIA_HOME/honcho.json` | Profile-local (isolated Lydia instances) |
| 2 | `~/.lydia/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Lydia profile: `lydia` (default) or `lydia_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `lydia honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Lydia session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`lydia-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`lydia`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-lydia-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening lydia in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Lydia session ID (timestamp + hex). Every `lydia` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Lydia profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`lydia`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Lydia profile: `lydia` (default) or `lydia_<profile>` (e.g. `lydia -p coder` -> host key `lydia_coder`). Older `lydia.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `lydia-agent`) |
...
|---------|-------------|
| `lydia memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `lydia honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `lydia memory setup`) |
| `lydia honcho status` | Show resolved config for active profile |
| `lydia honcho enable` / `disable` | Toggle Honcho for active profile |
| `lydia honcho mode <mode>` | Change recall or observation mode |
| `lydia honcho peer --user <name>` | Update user peer name |
| `lydia honcho peer --ai <name>` | Update AI peer name |
| `lydia honcho tokens --context <N>` | Set context token budget |
| `lydia honcho tokens --dialectic <N>` | Set dialectic max chars |
| `lydia honcho map <name>` | Map current directory to a session name |
| `lydia honcho sync` | Create host blocks for all Lydia profiles |
