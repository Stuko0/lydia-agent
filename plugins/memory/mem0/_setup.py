...
def _setup_platform(lydia_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "lydia-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "lydia"},
...
def _setup_oss(lydia_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "lydia-user")
    ...
    if env_writes:
        _write_env(Path(lydia_home) / ".env", env_writes)
    _save_mem0_json(lydia_home, {"mode": "oss", "user_id": user_id, "agent_id": "lydia", "oss": oss_config})
...
_PGVECTOR_CONTAINER = "lydia-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "lydia"
...
def _setup_oss_interactive(lydia_home: str, config: dict) -> None:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'lydia-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "lydia-user")

    agent_id = input("  Agent ID [lydia]: ").strip()
    agent_id = agent_id or "lydia"
...
def post_setup(lydia_home: str, config: dict) -> None:
    """Entry point called by lydia memory setup framework.
