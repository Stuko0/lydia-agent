...
Configuration
-------------
Secret (lives in $LYDIA_HOME/.env or the environment):
  MEM0_API_KEY       — Mem0 Platform API key (required for platform mode)

Behavioral settings (live in $LYDIA_HOME/mem0.json, set via `lydia memory
setup`):
  ...
  agent_id           — Agent identifier (default: lydia)
...
def _load_config() -> dict:
    """Load config from env vars, with $LYDIA_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "lydia"),
        "oss": {},
    }
    ...
    config_path = get_lydia_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, lydia_home):
        """Write config to $LYDIA_HOME/mem0.json."""
        import json
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $LYDIA_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "lydia")
