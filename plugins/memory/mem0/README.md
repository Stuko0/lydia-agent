...
```bash
lydia memory setup    # select "mem0"
```
...
Behavioral settings live in `$LYDIA_HOME/mem0.json` (set them via `lydia memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.lydia/.env`.

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `platform` | `platform` (Mem0 Cloud) or `oss` (self-hosted) |
| `user_id` | `lydia-user` | User identifier on Mem0 |
...
```bash
lydia memory setup mem0 --mode oss \
...
```bash
lydia memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$LYDIA_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.lydia/mem0_qdrant"}}
  }
}
```
...
```bash
lydia memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$LYDIA_HOME/mem0.json`).
