...
- Supermemory API key from [app.supermemory.ai/integrations?connect=lydia](http://app.supermemory.ai/integrations?connect=lydia)
...
Config file: `$LYDIA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `lydia` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `lydia-{identity}` → `lydia-coder`). |
...
All Supermemory API calls send `x-sm-source: lydia`, and document writes stamp
`metadata.sm_source: lydia`. This is a **functional routing key, not telemetry**:
it groups Lydia-written memories into a dedicated "Lydia" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Lydia profile:

```json
{
  "container_tag": "lydia-{identity}"
}
```

For a profile named `coder`, this resolves to `lydia-coder`. The default profile resolves to `lydia-default`. Without `{identity}`, all profiles share the same container.
