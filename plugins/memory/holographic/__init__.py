"""lydia-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $LYDIA_HOME/config.yaml (profile-scoped):
  plugins:
    lydia-memory-store:
      db_path: $LYDIA_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    ...
    try:
        ...
        return cfg_get(all_config, "plugins", "lydia-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, lydia_home):
        """Write config to config.yaml under plugins.lydia-memory-store."""
        ...
        try:
            ...
            existing["plugins"]["lydia-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_lydia_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from lydia_constants import get_lydia_home
        _lydia_home = str(get_lydia_home())
        _default_db = _lydia_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $LYDIA_HOME in user-supplied paths so config values like
        # "$LYDIA_HOME/memory_store.db" or "~/.lydia/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$LYDIA_HOME", _lydia_home)
            db_path = db_path.replace("${LYDIA_HOME}", _lydia_home)
