"""
Email platform adapter for the Lydia gateway.

Allows users to interact with Lydia by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "lydia-agent" "version" "{_lydia_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `lydia config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `lydia gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Lydia Agent")
        ...
        msg_id = f"<lydia-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
...
# lydia_cli/gateway.py, and the _send_email dispatch in
