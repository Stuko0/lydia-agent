...
Credential storage mirrors every other Lydia channel:

    * runtime SDK creds  -> ``~/.lydia/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.lydia/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a lydia dependency
...
# registers Lydia as its own client_id.
...
# Default name of the project Lydia provisions for the operator.
DEFAULT_PROJECT_NAME = "Lydia Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of lydia-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.lydia/auth.json`` honouring the active Lydia profile."""
    try:
        from lydia_constants import get_lydia_home
        return Path(get_lydia_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.lydia")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.lydia/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.lydia/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.lydia/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: lydia_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `lydia photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `lydia photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `lydia photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `lydia photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `lydia photon setup`)"
