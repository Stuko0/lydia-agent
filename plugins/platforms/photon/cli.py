"""
``lydia photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Lydia
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `lydia photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Lydia Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``lydia photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Lydia Agent" project.
    ...
    # 3. Rotate the project secret and persist creds (runtime -> ~/.lydia/.env,
    ...
    print("  Start the gateway:  lydia gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `lydia photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`lydia photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.lydia/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `lydia photon telemetry on` / `lydia photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.lydia/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  lydia gateway restart")
    return 0
...
#
# `lydia gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``lydia photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `lydia gateway setup` wizard."""
