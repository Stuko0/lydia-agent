...
Small Node helper that bridges Lydia Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Lydia is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Lydia-Sidecar-Token`)
...
The Lydia plugin's `lydia photon setup` command runs `npm install`
here automatically.
