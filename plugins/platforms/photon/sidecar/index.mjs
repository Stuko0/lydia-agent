// Lydia Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Lydia): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Lydia -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Lydia-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `lydia photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Lydia can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Lydia'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Lydia allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Lydia, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-lydia-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Lydia supplied them so a known-good
