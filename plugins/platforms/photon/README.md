...
This plugin connects Lydia Agent to iMessage (and other Spectrum
...
TypeScript-only, Lydia runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Lydia-Sidecar-Token`.
...
There is no separate `login` command; like every other Lydia channel,
...
Run `lydia photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.lydia/.env` (the same place every other
...
Management metadata lives in `~/.lydia/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `lydia photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `lydia photon status`, a DM and a group roundtrip,
