"""ntfy platform adapter (Lydia plugin).
...
``plugins/platforms/ntfy/``. The Lydia plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "lydia-in"              # subscribe topic (incoming)
          publish_topic: "lydia-out"     # optional — defaults to topic
...
_ECHO_TAG = "lydia-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``lydia cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Lydia dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `lydia gateway status` without
