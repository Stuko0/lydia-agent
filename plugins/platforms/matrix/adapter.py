...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable Lydia command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    Lydia accepts ``!command`` as a Matrix-friendly alias, but only for
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal Lydia ``/command`` text."""
...
# Uses get_lydia_home() so each profile gets its own Matrix store.
from lydia_constants import get_lydia_dir as _get_lydia_dir
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    # the adapter accepts "!command" as the alias that always reaches Lydia
        ...
        if self._encryption:
            ...
            if self._encryption:
                try:
                    ...
                    _acct_id = self._user_id or "lydia"
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Lydia Agent" become "Agent".
        """
        if not body:
            return ""
...
# + _PLATFORMS["matrix"] static dict in lydia_cli/{setup,gateway}.py, and the
...
def interactive_setup() -> None:
    """Configure Matrix credentials. Replaces lydia_cli/setup.py::_setup_matrix
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print_info("📬 Home Room: where Lydia delivers cron job results and notifications.")
...
def _is_connected(config) -> bool:
    ...
    configured. Read via lydia_cli.gateway.get_env_value so setup-status
