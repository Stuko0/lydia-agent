"""SimpleX Chat platform adapter (Lydia plugin).
...
This adapter ships as a Lydia platform plugin under
``plugins/platforms/simplex/``. The Lydia plugin loader scans the
...
Optional environment variables:
    ...
    LYDIA_SIMPLEX_TEXT_BATCH_DELAY
...
# Imported at module top because they're stdlib-only inside Lydia — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("LYDIA_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    ...
    auto-accept / home channel. Writes to ``~/.lydia/.env`` via
    ``lydia_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "lydia_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.lydia/.env"
        )
        return
