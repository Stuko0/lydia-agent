...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `lydia slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        tokens_file = get_lydia_home() / "slack_tokens.json"
        ...
        try:
            ...
            # Lydia does not currently need to route them into the agent loop.
            ...
            # /model work directly without /lydia prefix). A single regex
            ...
            # manifest (see `lydia slack manifest`). In Socket Mode, Slack
            ...
            from lydia_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/lydia$")

            @self._app.command(_slash_pattern)
            async def handle_lydia_command(ack, command):
            ...
            for _action_id in (
                "lydia_approve_once",
                "lydia_approve_session",
                "lydia_approve_always",
                "lydia_deny",
            ):
            ...
            for _action_id in (
                "lydia_confirm_once",
                "lydia_confirm_always",
                "lydia_confirm_cancel",
            ):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Lydia handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Lydia session — matching the per-thread behavior channels already
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "lydia_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "lydia_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "lydia_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "lydia_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "lydia_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "lydia_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "lydia_confirm_once": "once",
            "lydia_confirm_always": "always",
            "lydia_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "lydia_approve_once": "once",
            "lydia_approve_session": "session",
            "lydia_approve_always": "always",
            "lydia_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/lydia <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/lydia
        ...
        if slash_name in {"lydia", ""}:
            # Legacy /lydia <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from lydia_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/lydia   ``).
        ...
        # "Running /cmd…" ack shown by handle_lydia_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/lydia <question>" must produce public replies so
...
# ``lydia_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``lydia_cli/setup.py::_setup_slack``.
    """
    from pathlib import Path
    from lydia_cli.config import get_env_value, save_env_value
    from lydia_cli.cli_output import (
        prompt,
        prompt_yes_no,
        print_header,
        print_info,
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under LYDIA_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from lydia_cli.slack_cli import _build_full_manifest
            from lydia_constants import get_lydia_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Lydia",
                bot_description="Your Lydia Agent on Slack",
            )
            target = Path(get_lydia_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `lydia slack manifest --write` anytime to refresh after "
                "Lydia adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `lydia update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://lydia-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``lydia_cli.gateway.get_env_value`` at call time (not via the
    ...
    """
    import lydia_cli.gateway as gateway_mod
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'lydia-agent[slack]'",
        # Interactive setup wizard — replaces lydia_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in lydia_cli/gateway.py.
