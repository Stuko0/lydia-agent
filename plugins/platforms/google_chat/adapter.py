...
# ``model_tools`` import time, so ``lydia status``, ``lydia chat``, etc.
...
#     fresh "Lydia is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a lydia MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${LYDIA_HOME}/google_chat_thread_counts.json so
        ...
        # they don't sit in the chat forever as "Lydia is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("LYDIA_HOME", str(_Path.home() / ".lydia"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${LYDIA_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a lydia MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Lydia is thinking…' marker message.
        ...
        "Lydia is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Lydia is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Lydia is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Lydia is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Lydia is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``lydia setup``.

    The setup wizard at ``lydia_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.lydia/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. lydia-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.lydia/.env")
    print_info("Restart the gateway: lydia gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``lydia cron`` running as a
    separate process from ``lydia gateway``).  Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'lydia-agent[google_chat]'",
        ...
        # the "Lydia is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Lydia is thinking…' marker message "
