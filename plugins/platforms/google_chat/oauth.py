...
- Per-user tokens (keyed by sender email):
    ``${LYDIA_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${LYDIA_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${LYDIA_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${LYDIA_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${LYDIA_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's LYDIA_HOME helper so the token follows the user's
# profile (e.g. tests can override via LYDIA_HOME=/tmp/...).
try:
    from lydia_constants import display_lydia_home, get_lydia_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where lydia_constants isn't importable
...
def _lydia_home() -> Path:
    """Resolve LYDIA_HOME at call time (NOT module import).

    Tests and ``LYDIA_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.lydia/google_chat_user_tokens/`` trivial.
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install lydia-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'lydia-agent[google_chat]'")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into LYDIA_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_lydia_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_lydia_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Lydia (native attachment delivery)"
    )
