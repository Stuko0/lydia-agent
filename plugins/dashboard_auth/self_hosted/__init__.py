...
A standards-compliant OpenID Connect Relying Party for the ``lydia dashboard``
...
:class:`~lydia_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``lydia_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~lydia_cli.dashboard_auth.Session`.
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          ...
          # credential — prefer the env var / ~/.lydia/.env over config.yaml.
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Lydia does not call
...
def register(ctx) -> None:
    ...
    # canonical home is the env var / ~/.lydia/.env; config.yaml is supported
