...
This page covers all commands related to [Lydia profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `lydia profile` without a subcommand shows help.
...
```bash
lydia profile use <name>
```

Sets `<name>` as the active profile. All subsequent `lydia` commands (without `-p`) will use this profile.
...
lydia profile use default
```

## `lydia profile create`

```bash
lydia profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `lydia profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `lydia update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.lydia`), use `lydia skills opt-out` / `lydia skills opt-in`. |
...
lydia profile create work2 --clone-from work

# Clone everything from a specific profile
lydia profile create work2-backup --clone-from work --clone-all
```

## `lydia profile describe`

```bash
lydia profile describe [<name>] [options]
```
...
lydia profile describe researcher

# Set it explicitly
lydia profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
lydia profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
lydia profile describe --all --auto
```

## `lydia profile delete`

```bash
lydia profile delete <name> [options]
```
...
```bash
lydia profile delete mybot
lydia profile delete mybot --yes
```
...
```bash
lydia profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Lydia home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ lydia profile show work
Profile: work
Path:    ~/.lydia/profiles/work
...
```bash
lydia profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Lydia installation.
...
```bash
lydia profile alias work
# Creates/updates ~/.local/bin/work

lydia profile alias work --name mywork
# Creates ~/.local/bin/mywork

lydia profile alias work --remove
# Removes the wrapper script
```

## `lydia profile rename`

```bash
lydia profile rename <old-name> <new-name>
```
...
```bash
lydia profile rename mybot assistant
# ~/.lydia/profiles/mybot → ~/.lydia/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `lydia profile export`

```bash
lydia profile export <name> [options]
```
...
```bash
lydia profile export work
# Creates work.tar.gz in the current directory

lydia profile export work -o ./work-2026-03-29.tar.gz
```

## `lydia profile import`

```bash
lydia profile import <archive> [options]
```
...
```bash
lydia profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

lydia profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`lydia profile export` / `import` are still the right commands for
...
### `lydia profile install`

```bash
lydia profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `lydia -p telemetry`). |
...
lydia profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
lydia profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
lydia profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
lydia profile install ./telemetry/
```

### `lydia profile update`

```bash
lydia profile update <name> [--force-config] [--yes]
```
...
```bash
lydia profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Lydia version, author, env var requirements, the source URL/path, and
...
`lydia profile list` also shows the distribution name and version in a
`Distribution` column, and `lydia profile show <name>` / `delete <name>`
...
lydia profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
lydia profile install https://github.com/your-org/internal-assistant.git
```
...
Lydia version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Lydia can clone from.
3. Tell recipients to run `lydia profile install <your-repo-url>`.
...
## `lydia -p` / `lydia --profile`

```bash
lydia -p <name> <command> [options]
lydia --profile <name> <command> [options]
```

Global flag to run any Lydia command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
lydia -p work chat -q "Check the server status"
lydia --profile dev gateway start
lydia -p personal skills list
lydia -p work config edit
```

## `lydia completion`

```bash
lydia completion <shell>
```
...
- `lydia profile <TAB>` — subcommands (list, use, create, etc.)
- `lydia profile use <TAB>` — profile names
- `lydia -p <TAB>` — profile names
