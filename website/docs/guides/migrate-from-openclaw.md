...
`lydia claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into Lydia. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.lydia/` (by default a single restore-point archive is written before apply, under `~/.lydia/backups/pre-migration-*.zip`; restorable with `lydia import`). |
...
| What | OpenClaw source | Lydia destination | Notes |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.lydia/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.lydia/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.lydia/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.lydia/memories/MEMORY.md` | All daily files merged into main memory. |
...
| Source | OpenClaw location | Lydia destination |
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.lydia/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.lydia/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.lydia/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.lydia/skills/openclaw-imports/` |

Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing Lydia skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | Lydia destination | Notes |
|------|---------------------|-------------------|-------|
...
| Provider API keys | `models.providers.*.apiKey` | `~/.lydia/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |

### Agent behavior

| What | OpenClaw config path | Lydia config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | Lydia config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | Lydia field | Notes |
|----------------|-------------|-------|
...
| TTS assets | `~/.lydia/tts/` (file copy) |

### Messaging platforms

| Platform | OpenClaw config path | Lydia `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | Lydia path | Notes |
|------|-------------|-------------|-------|
...
| Gateway auth token | `gateway.auth.token` | `.env` → `LYDIA_GATEWAY_TOKEN` | Requires `--migrate-secrets` |
...
### Archived (no direct Lydia equivalent)

These are saved to `~/.lydia/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in Lydia |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | Lydia has built-in tool instructions |
...
| Cron jobs | `archive/cron-config.json` | Recreate with `lydia cron create` |
| Plugins | `archive/plugins-config.json` | See [plugins guide](/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | Use `lydia webhook` or gateway hooks |
| Memory backend | `archive/memory-backend-config.json` | Configure via `lydia honcho` |
| Skills registry | `archive/skills-registry-config.json` | Use `lydia skills config` |
...
| Multi-agent list | `archive/agents-list.json` | Use Lydia profiles |
...
Imported skills land in `~/.lydia/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.
