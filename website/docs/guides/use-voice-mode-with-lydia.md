---
sidebar_position: 8
title: "Use Voice Mode with Lydia"
description: "A practical guide to setting up and using Lydia voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Lydia
...
- you want Lydia sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Lydia.
...
## Step 1: make sure normal Lydia works first
...
Lydia supports both local and cloud speech stacks.
...
### If you use `lydia setup`

If you choose NeuTTS in the setup wizard, Lydia checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Lydia transcribes and responds
...
- asking Lydia to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Lydia loop.

## Tuning CLI behavior

### Silence threshold

If Lydia starts/stops too aggressively, tune:
...
Lydia stays a normal chat bot, but can speak replies.
...
- you want Lydia to function like a portable research or ops assistant
...
Lydia joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Lydia detects speech boundaries
- transcripts are posted in the associated text channel
- Lydia responds in text and audio
...
1. get text Lydia working
2. install `lydia-agent[voice]`
