...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `lydia send`."
---

# Pipe Script Output to Messaging Platforms

`lydia send` is a small, scriptable CLI that pushes a message to any
messaging platform Lydia is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | lydia send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `lydia
...
lydia send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
lydia send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
lydia send --list

# Filter by platform
lydia send --list telegram
```
...
Any platform Lydia ships adapters for works as a target:
...
`lydia send` resolves the message body in this order:

1. **Positional argument** — `lydia send --to telegram "hi"`
2. **`--file PATH`** — `lydia send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | lydia send --to telegram`

When stdin is a TTY (no pipe), Lydia does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  lydia send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `lydia send` reuses your Lydia config, the same script works on
any host where Lydia is installed — no need to export bot tokens into
...
`lydia send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  lydia send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | lydia send \
...
```bash
./train.py --epochs 200 && \
  lydia send --to telegram "training done" || \
  lydia send --to telegram "training failed (exit $?)"
```
...
lydia send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(lydia send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `lydia send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `lydia send` calls
the platform's REST endpoint directly using credentials from
`~/.lydia/.env` and `~/.lydia/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `lydia gateway start` and retry.
...
# Machine-readable
lydia send --list --json
```

The listing is built from `~/.lydia/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`lydia gateway
...
| Approach | Multi-platform | Reuses Lydia creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `lydia send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`lydia send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `lydia send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `lydia send` shares with cron delivery.
