---
sidebar_position: 7
title: "Use SOUL.md with Lydia"
description: "How to use SOUL.md to shape Lydia Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Lydia

`SOUL.md` is the **primary identity** for your Lydia instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Lydia to feel like the same assistant every time you talk to it — or if you want to replace the Lydia persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Lydia should be
- what Lydia should avoid stylistically
- how Lydia should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Lydia is and how Lydia speaks
...
Lydia now uses only the global SOUL file for the current instance:

```text
~/.lydia/SOUL.md
```

If you run Lydia with a custom home directory, it becomes:
...
Lydia automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Lydia does not overwrite it
- if the file exists but is empty, Lydia adds nothing from it to the prompt

## How Lydia uses it

When Lydia starts a session, it reads `SOUL.md` from `LYDIA_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Lydia feels.
...
Lydia already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Lydia should sound.

# Avoid
What Lydia should not do.

# Defaults
How Lydia should behave when ambiguity appears.
```
...
Then restart Lydia or start a new session.
...
### I edited SOUL.md but Lydia still sounds the same

Check:
- you edited `~/.lydia/SOUL.md` or `$LYDIA_HOME/SOUL.md`
...
### Lydia is ignoring parts of my SOUL.md
