...
This tutorial walks you through setting up a Telegram bot powered by Lydia Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Lydia Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.lydia/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Lydia itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Lydia Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_lydia_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Lydia Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Lydia Gateway...
...
# View live logs
journalctl --user -u lydia-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `lydia gateway install` to update the plist.
:::
...
```bash
lydia gateway stop && lydia gateway start
```
...
lydia pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.lydia/.env`:
...
Control how much detail the bot shows when using tools. In `~/.lydia/config.yaml`:
...
Customize how the bot communicates by editing `~/.lydia/SOUL.md`:

For a full guide, see [Use SOUL.md with Lydia](/guides/use-soul-with-lydia).
...
```bash
# From the CLI
lydia cron list          # View all scheduled jobs
lydia cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u lydia-gateway` (Linux) or `~/.lydia/logs/gateway.log` (macOS) |
| Cron job output | `~/.lydia/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.lydia/cron/jobs.json` |
| Pairing data | `~/.lydia/pairing/` |
| Session history | `~/.lydia/sessions/` |
