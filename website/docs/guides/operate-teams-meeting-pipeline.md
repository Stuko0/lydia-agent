...
```bash
lydia teams-pipeline token-health
lydia teams-pipeline token-health --force-refresh
```
...
```bash
lydia teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
lydia teams-pipeline maintain-subscriptions
lydia teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Lydia cron (recommended if you already run the Lydia gateway)

Lydia ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.lydia/scripts/`. First create the script:

```bash
mkdir -p ~/.lydia/scripts
cat > ~/.lydia/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec lydia teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.lydia/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
lydia cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/lydia-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Lydia Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=lydia
EnvironmentFile=/etc/lydia/env
ExecStart=/usr/local/bin/lydia teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/lydia-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Lydia Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now lydia-teams-pipeline-maintain.timer
systemctl list-timers lydia-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/lydia teams-pipeline maintain-subscriptions >> /var/log/lydia/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.lydia/.env` at the top of a wrapper script that crontab calls.
...
```bash
lydia teams-pipeline subscriptions   # should show expirationDateTime advanced
lydia teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
lydia teams-pipeline list
lydia teams-pipeline list --status failed
lydia teams-pipeline show <job-id>
```

### Replay a stored job

```bash
lydia teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
lydia teams-pipeline fetch --meeting-id <meeting-id>
lydia teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `lydia teams-pipeline maintain-subscriptions --dry-run`
- inspect `lydia teams-pipeline list --status failed`
...
- run `lydia teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `lydia teams-pipeline run`
...
- [ ] `lydia teams-pipeline validate` returns an OK snapshot
- [ ] `lydia teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Lydia cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
