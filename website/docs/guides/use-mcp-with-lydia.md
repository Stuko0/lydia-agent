---
sidebar_position: 6
title: "Use MCP with Lydia"
description: "A practical guide to connecting MCP servers to Lydia Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Lydia

This guide shows how to actually use MCP with Lydia Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Lydia tool
- you want Lydia to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Lydia to internal APIs, databases, or company systems without modifying Lydia core

Do not use MCP when:
- a built-in Lydia tool already solves the job well
...
- Lydia remains the agent
- MCP servers contribute tools
- Lydia discovers those tools at startup or reload time
...
If you installed Lydia with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Lydia banner/status should show MCP integration when configured
- ask Lydia what tools it has available
...
## WSL2: bridge Lydia in WSL to Windows Chrome

This is the practical setup when:

- Lydia runs inside WSL2
...
In this setup, Lydia does **not** connect to Chrome directly. Instead:

- Lydia runs in WSL
- Lydia starts a local stdio MCP server
...
```text
Lydia (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Lydia stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Lydia core browser transport
...
```bash
lydia mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
lydia mcp test chrome-devtools-win
```

Then start a fresh Lydia session or run:
...
Once loaded, Lydia can use the MCP-prefixed browser tools directly. For example:
...
If Lydia runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Lydia from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Lydia from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Lydia:
...
2. Lydia-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Lydia will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Lydia to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Lydia to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Lydia remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
lydia mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
lydia mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `lydia mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Lydia to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Lydia to merge, publish, deploy, or spawn runtimes.
...
Now Lydia can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Lydia core.
...
Because Lydia now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
