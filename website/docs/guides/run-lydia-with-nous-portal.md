...
Open [portal.stuko.dev/manage-subscription](https://portal.stuko.dev/manage-subscription), sign up, and pick a plan.
...
3. Sets `model.provider: nous` in `~/.lydia/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where Lydia is running. Two options:
...
lydia auth add nous --type oauth --manual-paste
...
You should see Lydia call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.
...
### Don't pick Lydia-4 for agent work

Lydia-4-70B and Lydia-4-405B are available on the Portal at deep discounts, but they're **chat/reasoning models**, not tool-call-tuned. They will struggle with multi-step agent loops. Use them via [Nous Chat](https://chat.stuko.dev) for conversation/research work, or through the [subscription proxy](/user-guide/features/subscription-proxy) from non-agent tools. For Lydia Agent itself, stick to the frontier agentic models above.

The Portal's own [info page](https://portal.stuko.dev/info) carries this warning too — it's the official Nous guidance, not just a Lydia-side opinion.
...
These rows appear in `lydia native` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, Lydia runs the Portal login inline (without changing your inference provider or your other tools).
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Lydia will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
```bash
lydia cron create "every day at 9am" \
...
If you use [Lydia profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.lydia/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `lydia portal info` shows "not logged in" after `lydia setup --portal`
...
Re-verify with `lydia portal info`.
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Lydia doesn't replay it endlessly. Just log in again:
...
If a model is genuinely unavailable, [open an issue](https://github.com/Stuko/lydia-agent/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`lydia portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Lydia profiles where you're using the wrong one (check `lydia profile list`)

### Want to revoke and start clean

```bash
lydia auth logout nous       # wipes the local refresh token
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Lydia configurations
