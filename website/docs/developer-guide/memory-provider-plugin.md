...
description: "How to build a memory provider plugin for Lydia Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give Lydia Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `lydia plugins`.
:::
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          lydia_home (str): Active LYDIA_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `lydia memory setup` | **Yes** |
| `save_config(values, lydia_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
`get_config_schema()` returns a list of field descriptors used by `lydia memory setup`:
...
Every field in `get_config_schema()` is prompted during `lydia memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$LYDIA_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::
...
the `messages` parameter; Lydia will continue calling them with the legacy
signature.
...
All storage paths **must** use the `lydia_home` kwarg from `initialize()`, not hardcoded `~/.lydia`:

```python
# CORRECT — profile-scoped
from lydia_constants import get_lydia_home
data_dir = get_lydia_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.lydia/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `lydia my-provider status`, `lydia my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `lydia <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `lydia --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: lydia my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the lydia my-provider argparse tree.
