...
The ACP adapter wraps Lydia' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
lydia acp / lydia-acp / python -m acp_adapter
...
The Zed ACP Registry path launches the same adapter through `uvx --from 'lydia-agent[acp]==<version>' lydia-acp`, pointed at the `lydia-agent` PyPI release.
...
- `allow_once` -> Lydia `once`
- `allow_always` -> Lydia `always`
- reject options -> Lydia `deny`
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["lydia-acp"])
...
Instead it reuses Lydia' runtime resolver:

- `acp_adapter/auth.py`
- `lydia_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Lydia provider/credentials. It also always advertises a terminal setup auth method (`lydia-setup`, args `--setup`) so first-run registry clients can open Lydia' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.lydia/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `lydia-acp` toolset definition
- `lydia_cli/main.py` — `lydia acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `lydia-acp` script
