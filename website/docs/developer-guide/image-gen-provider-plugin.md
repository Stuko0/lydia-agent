...
:::tip
Image-gen is one of several **backend plugins** Lydia supports. The others (with more specialized ABCs) are [Memory Provider Plugins](/developer-guide/memory-provider-plugin), [Context Engine Plugins](/developer-guide/context-engine-plugin), and [Model Provider Plugins](/developer-guide/model-provider-plugin). General tool/hook/CLI plugins live in [Build a Lydia Plugin](/guides/build-a-lydia-plugin).
:::

## How discovery works

Lydia scans for image-gen backends in three places:

1. **Bundled** — `<repo>/plugins/image_gen/<name>/` (auto-loaded with `kind: backend`, always available)
2. **User** — `~/.lydia/plugins/image_gen/<name>/` (opt-in via `plugins.enabled`)
3. **Pip** — packages declaring a `lydia_agent.plugins` entry point
...
A bundled plugin is complete at this point. User plugins at `~/.lydia/plugins/image_gen/<name>/` need to be added to `plugins.enabled` in `config.yaml` (or run `lydia plugins enable <name>`).
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            #   - base64 data: save under $LYDIA_HOME/cache/images/ via save_b64_image()
...
`kind: backend` is what routes the plugin to the image-gen registration path. `requires_env` is prompted during `lydia plugins install`.
...
Some backends return image URLs (fal, Replicate); others return base64 payloads (OpenAI gpt-image-2). For the base64 case, use `save_b64_image()` — it writes to `$LYDIA_HOME/cache/images/<prefix>_<timestamp>_<uuid>.<ext>` and returns the absolute `Path`. Pass that path (as `str`) as `image=` in `success_response()`. Gateway delivery (Telegram photo bubble, Discord attachment) recognizes both URLs and absolute paths.

## User overrides

Drop a user plugin at `~/.lydia/plugins/image_gen/<name>/` with the same `name` property as a bundled one and enable it via `lydia plugins enable <name>` — the registry is last-writer-wins, so your version replaces the built-in. Useful for pointing an `openai` plugin at a private proxy, or swapping in a custom model catalog.

## Testing

```bash
export LYDIA_HOME=/tmp/lydia-imggen-test
mkdir -p $LYDIA_HOME/plugins/image_gen/my-backend
# …copy __init__.py + plugin.yaml into that dir…

export MY_BACKEND_API_KEY=your-test-key
lydia plugins enable my-backend

# Pick it as the active provider
echo "image_gen:" >> $LYDIA_HOME/config.yaml
echo "  provider: my-backend" >> $LYDIA_HOME/config.yaml

# Exercise it
lydia -z "Generate an image of a corgi in a spacesuit"
```
...
`my_backend_imggen_package` must expose a top-level `register` function. See [Distribute via pip](/guides/build-a-lydia-plugin#distribute-via-pip) in the general plugin guide for the full setup.
...
- [Build a Lydia Plugin](/guides/build-a-lydia-plugin) — general tools/hooks/slash commands guide
