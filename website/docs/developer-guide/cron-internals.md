...
description: "How Lydia stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `lydia_cli/cron.py` | CLI `lydia cron` subcommands |
...
Jobs are stored in `~/.lydia/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `lydia cron` commands are run or during active CLI sessions.
...
```python
# ~/.lydia/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.lydia/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `lydia cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `lydia cron` CLI provides direct job management:

```bash
lydia cron list                    # Show all jobs
lydia cron create                  # Interactive job creation (alias: add)
lydia cron edit <job_id>           # Edit job configuration
lydia cron pause <job_id>          # Pause a running job
lydia cron resume <job_id>         # Resume a paused job
lydia cron run <job_id>            # Trigger immediate execution
lydia cron remove <job_id>         # Delete a job
```
