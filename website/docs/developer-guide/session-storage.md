...
Lydia Agent uses a SQLite database (`~/.lydia/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `lydia_state.py`


## Architecture Overview

```
~/.lydia/state.db (SQLite, WAL mode)
...
Multiple lydia processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.lydia/state.db
...
Default path: `~/.lydia/state.db`

This is derived from `lydia_constants.get_lydia_home()` which resolves to
`~/.lydia/` by default, or the value of `LYDIA_HOME` environment variable.
