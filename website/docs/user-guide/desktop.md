...
sidebar_position: 3
title: "Desktop App"
description: "The native Lydia desktop app — a polished experience for chatting with Lydia, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Desktop App

The Lydia desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Lydia Agent core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `lydia` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
Lydia has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`lydia`) and **[TUI](./tui.md)** (`lydia --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`lydia dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Follow the [installation instructions for Lydia Desktop](../getting-started/installation.md).

If you already have Lydia installed, simply run
...
- **The same conversation history** as every other Lydia surface — sessions started here resume in the CLI/TUI and vice versa.
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `lydia desktop --cwd <path>` (or the `LYDIA_DESKTOP_CWD` environment variable).

### Voice

Talk to Lydia and hear it back, the same [voice mode](./features/voice-mode.md) available elsewhere. On macOS the OS will prompt once for microphone access.
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `lydia model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
The app also surfaces the broader Lydia management surface so you don't have to drop to a terminal:

- **Skills** — browse, install, and manage [skills](./features/skills.md).
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#lydia-cron).
- **Profiles** — switch between [Lydia profiles](./profiles.md) (isolated config/skills/sessions).
...
The [manual update process](https://lydia-agent.nousresearch.com/docs/getting-started/updating) also works with the GUI.

## Uninstalling

Open **Settings → About → Danger zone** and pick how much to remove:

- **Uninstall Chat GUI only** — removes the desktop app and its data; the Lydia Agent, your config, and your chats stay. (Same as `lydia uninstall --gui`.)
- **Uninstall GUI + agent, keep my data** — removes the app and the agent but keeps config, chats, and secrets for a future reinstall. (Same as `lydia uninstall`.)
- **Uninstall everything** — removes the app, the agent, and all user data. (Same as `lydia uninstall --full`.)
...
You can do the same from the terminal — `lydia uninstall --gui` for the GUI alone, or `lydia uninstall` / `lydia uninstall --full` for the agent too.

:::note
Running `lydia uninstall --gui` from a **source checkout** (a `lydia desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `lydia desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `lydia desktop`

To launch via the CLI, simply run `lydia desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.
...
| `--build-only`       | Build the desktop app but do not launch it (used by `lydia update`)                      |
...
| `--lydia-root PATH` | Override the Lydia source root the app uses (sets `LYDIA_DESKTOP_LYDIA_ROOT`)          |
| `--ignore-existing`  | Force the app to ignore any `lydia` CLI already on `PATH` during backend resolution      |
| `--fake-boot`        | Enable deterministic boot delays for validating the startup UI                            |

## How it works

The packaged app ships the Electron shell and a native React chat surface. On first launch it can install the Lydia Agent runtime into `LYDIA_HOME` (`~/.lydia`, or `%LOCALAPPDATA%\lydia` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. Backend resolution first honours `LYDIA_DESKTOP_LYDIA_ROOT`, then a completed managed install, then a probed `lydia` on `PATH` (unless `--ignore-existing` / `LYDIA_DESKTOP_IGNORE_EXISTING=1` is set), and finally an explicit `LYDIA_DESKTOP_LYDIA` command override for packagers such as Nix. The React renderer talks to a headless backend the app launches for you — a `lydia serve` process that serves the `tui_gateway` JSON-RPC/WebSocket API — and reuses the agent runtime rather than embedding `lydia --tui`. The desktop app is **self-contained**: it runs its own `lydia serve` backend and never opens or requires the [web dashboard](./features/web-dashboard.md). (Runtimes older than the `serve` command fall back to a headless `dashboard --no-open` automatically, so an app update never outruns its backend.) Install, backend-resolution, and self-update logic live in the Electron main process.

## Connecting to a remote backend

By default the app starts and manages its own **local** backend. You can instead point it at a Lydia backend running on another machine — a VPS, a home server, or a Mini behind Tailscale.

:::info The remote backend is a running `lydia serve` process
"Remote backend" means a **`lydia serve`** server running on the remote machine — that is the process the desktop app connects to. Nothing in this section works unless that backend is actually up and reachable. The desktop app does not start it for you; you (or a `systemd` service) keep `lydia serve` running on the remote host, and the app attaches to it. If you also use messaging channels (Telegram, Discord, etc.), the **gateway** is a *separate* long-running process you start independently — see the note after the setup steps.
:::
...
Set a username and password, then start the backend bound to a reachable address. The credentials live in `~/.lydia/.env` (the secrets file, mode 0600):
...
lydia serve --host 0.0.0.0 --port 9119
```

Keep that `lydia serve` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.

Separately, make sure the **gateway is running** on the remote host if you rely on messaging channels — the `lydia serve` backend is what the desktop app talks to, but your Telegram/Discord/Slack gateway sessions are a different process that you start and keep running on their own. See [Messaging](./messaging/index.md) for gateway setup.
...
Running the backend as a systemd service? Give the unit `EnvironmentFile=%h/.lydia/.env` so the credentials are in the environment at boot.
...
1. **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/lydia` work if you front it with a reverse proxy)
...
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.lydia/.env` and that the dashboard process actually loaded them.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Lydia Desktop to a remote backend](./features/web-dashboard.md#connecting-lydia-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Lydia Desktop](../reference/environment-variables.md#web-dashboard--lydia-desktop).

## Troubleshooting

Boot logs land in `LYDIA_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.lydia/lydia-agent/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
...
- [TUI](./tui.md) — the modern terminal UI used by `lydia --tui` and the dashboard chat tab
