...
Connect Lydia to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.lydia/.env`:
...
All standard Lydia commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.lydia/logs/gateway.log`
...
4. Consider enabling verbose logging: `LYDIA_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Lydia automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
LYDIA_LOG_LEVEL=debug lydia gateway
```
