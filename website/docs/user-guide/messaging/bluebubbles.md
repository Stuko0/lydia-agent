...
Connect Lydia to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Lydia and the BlueBubbles server
...
Or set environment variables directly in `~/.lydia/.env`:
...
By default, Lydia responds to every authorized BlueBubbles/iMessage DM or group message. To make group chats opt-in, enable mention gating:
...
With `require_mention: true`, DMs still work normally, but group-chat messages are ignored unless they match a mention pattern. If you do not configure custom patterns, Lydia uses conservative defaults for `Lydia` and `@Lydia Agent` variants.
...
When someone messages your iMessage, Lydia automatically sends them a pairing code. Approve it with:
```bash
lydia pairing approve bluebubbles <CODE>
```
Use `lydia pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.lydia/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.lydia/.env`):
```bash
...
Lydia will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Lydia
Lydia → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Lydia sends messages via the BlueBubbles REST API.
...
| `BLUEBUBBLES_MENTION_PATTERNS` | No | Lydia wake words | JSON array, newline-separated, or comma-separated regex patterns for group mention matching |

Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.lydia/config.yaml` (default: `true`). There is no corresponding environment variable.
...
### Chat Addressing
You can address chats by email or phone number — Lydia resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `lydia logs gateway` for webhook errors (or `lydia logs -f` to follow in real-time)
