...
Connect Lydia to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Lydia — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Lydia runs it in a
...
lydia photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Lydia Agent` project on your account.
...
Runtime credentials are written to `~/.lydia/.env`
...
(device token, dashboard project id) lives in `~/.lydia/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Lydia
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Lydia replies with a pairing code. Approve it with:

```bash
lydia pairing approve photon <CODE>
```

Use `lydia pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.lydia/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.lydia/.env`):
...
By default Lydia responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Lydia` and
`@Lydia Agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Lydia will reply.
...
assigned iMessage line Lydia uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run lydia photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `lydia photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `lydia photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `lydia photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Lydia sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Lydia wake words  | JSON list / comma / newline regex patterns for group mentions |
