...
Lydia can connect to WhatsApp through Meta's **official** WhatsApp Business Cloud API. This is the production-grade path: no Node.js bridge subprocess, no QR codes, no account-ban risk.
...
- The Lydia gateway needs a **public HTTPS URL** so Meta can deliver inbound messages via webhook.
- Replies more than 24 hours after the user's last message require a pre-approved **template** (this is Meta's "customer service window" rule, not a Lydia limit).
...
4. **Optional but recommended**: ffmpeg on `PATH` so outbound voice messages render as native WhatsApp voice-note bubbles (green waveform) instead of MP3 audio attachments. Lydia degrades gracefully if absent.
...
2. **Add** → name (e.g. `lydia-bot`) → role: **Admin**.
...
6. Copy the token → update `WHATSAPP_CLOUD_ACCESS_TOKEN` in `~/.lydia/.env` → restart the gateway.

System User tokens don't expire unless you explicitly revoke them.

---

## Exposing Lydia to the internet

The Cloud API delivers inbound messages by HTTPS POST to your webhook URL — that means the Lydia gateway has to be reachable from Meta's servers.  Three common ways:
...
2. Generate a **Verify Token** — the wizard does this for you with `secrets.token_urlsafe(32)`; if you're configuring manually, run:
   ...
   ```
   Save it as `WHATSAPP_CLOUD_VERIFY_TOKEN` in `~/.lydia/.env`.
3. Start the Lydia gateway: `lydia gateway`.
...
## Allowlist (Lydia-side)

In addition to Meta's recipient whitelist, Lydia has its own per-platform allowlist that controls **which incoming messages the agent processes**.  Add to `~/.lydia/.env`:
...
The `lydia whatsapp-cloud` wizard prints these links at the end of setup. None of this is required for the bot to work — it's pure polish for how your bot appears to users.

---

## Configuration reference

All settings live in `~/.lydia/.env`.  Required values are in **bold**.
...
When the agent invokes any of these flows, Lydia uses WhatsApp's native interactive messages — tap-to-answer buttons instead of "reply with the number" prompts:
...
Lydia acknowledges inbound messages immediately:
...
Lydia TTS produces MP3. Two paths:
...
Lydia warns the agent about this window in its system prompt, so the model knows to mention it when scheduling delayed messages.

Message-template support (the workaround for outside-window sends) is not yet implemented in Lydia. If you need it, please [open an issue](https://github.com/NousResearch/lydia-agent/issues) — it's planned but waiting on a clear demand signal.

### Group chats

The Cloud API has limited group support (capability-tier gated by Meta).  Lydia's `whatsapp_cloud` adapter currently handles **direct messages only** in v1.  If you need group chats, use the Baileys bridge.

### Outbound rate limit

Meta's default throughput is **80 messages/second per business phone number**, with upgrades available.  Lydia doesn't currently enforce this client-side — extremely high-volume sends could hit Meta's limit.
...
- **Verify token mismatch** — the token in `~/.lydia/.env`'s `WHATSAPP_CLOUD_VERIFY_TOKEN` must match exactly what you typed into Meta's dashboard.  Run the curl probe above to confirm the gateway's verify handshake works locally first.
- **Gateway not running** — check `lydia gateway` is up.
- **App Secret not set** — without it, Lydia refuses inbound POSTs with 503.  Meta interprets that as "can't validate."
...
- Wait for template support to land in Lydia.
...
Common cause: the toolset configured for `whatsapp_cloud` is missing the tools the agent wants to call.  Check `lydia native list` and verify the platform is using `lydia-whatsapp` (the default Cloud adapter toolset, same as Baileys).

If the model emits tool-call-shaped text instead of a structured call, it usually means the toolset was effectively empty.  See `lydia_cli/platforms.py` for the platform → default toolset mapping.
...
- **Treat the App Secret like a password** — anyone with it can forge webhook payloads that Lydia will accept as authentic.
...
| | Baileys (`lydia whatsapp`) | Cloud API (`lydia whatsapp-cloud`) |
|---|---|---|
...
Most users running Lydia for personal projects prefer Baileys. Most users running customer-facing bots prefer Cloud API.
