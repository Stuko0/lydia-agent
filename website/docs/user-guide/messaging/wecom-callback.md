...
Connect Lydia to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Lydia supports two WeCom integration modes:
...
> Run `lydia gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Lydia decrypts the message, queues it for the agent
...
(Use `lydia gateway start` only after `lydia gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Lydia and the token the
...
in `~/.lydia/.env` values around `=` will also break signature checks. After
fixing, restart `lydia gateway run`.
...
Check `lydia gateway run` logs for the bound host/port. If the adapter bound to
