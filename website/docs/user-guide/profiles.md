...
A profile is a separate Lydia home directory. Each profile gets its own directory containing its own `config.yaml`, `.env`, `SOUL.md`, memories, sessions, skills, cron jobs, and state database. Profiles let you run separate agents for different purposes — a coding assistant, a personal bot, a research agent — without mixing up Lydia state.
...
```bash
lydia profile create coder       # creates profile + "coder" command alias
coder setup                       # configure API keys and model
coder chat                        # start chatting
```

That's it. `coder` is now its own Lydia profile with its own config, memory, and state.
...
```bash
lydia profile create researcher --description "Reads source code and external docs, writes findings."
```

You can also set or auto-generate the description later with `lydia profile describe` — see the [Kanban guide](./features/kanban#auto-vs-manual-orchestration) for the full routing model.
...
Copies your current profile's `config.yaml`, `.env`, `SOUL.md`, and skills into the new profile. Same API keys, model, and capabilities, but fresh sessions and memory. Edit `~/.lydia/profiles/work/.env` for different API keys, or `~/.lydia/profiles/work/SOUL.md` for a different personality.

### Clone everything (`--clone-all`)

```bash
lydia profile create backup --clone-all
```

Copies **everything** — config, API keys, personality, all memories, skills, cron jobs, plugins. A complete working snapshot. Per-profile history is excluded (session history, `state.db`, `backups/`, `state-snapshots/`, `checkpoints/`) — these belong to the source profile and can reach tens of GB. For a full backup including history, use `lydia profile export` or `lydia backup` instead.

### Clone from a specific profile

```bash
lydia profile create work --clone-from coder
```
...
```bash
lydia profile create work-backup --clone-from coder --clone-all
```
...
The alias works with every lydia subcommand — it's just `lydia -p <name>` under the hood.
...
```bash
lydia -p coder chat
lydia --profile=coder doctor
lydia chat -p coder -q "hello"    # works in any position
```

### Sticky default (`lydia profile use`)

```bash
lydia profile use coder
lydia chat                   # now targets coder
lydia native                  # configures coder's tools
lydia profile use default    # switch back
```

Sets a default so plain `lydia` commands target that profile. Like `kubectl config use-context`.
...
- **`lydia profile`**: Shows current profile name, path, model, gateway status

## Profiles vs workspaces vs sandboxing

Profiles are often confused with workspaces or sandboxes, but they are different things:

- A **profile** gives Lydia its own state directory: `config.yaml`, `.env`, `SOUL.md`, sessions, memory, logs, cron jobs, and gateway state.
...
Using `cwd: "."` on the local backend means "the directory Lydia was launched from", not "the profile directory".
...
```bash
# Edit coder's tokens
nano ~/.lydia/profiles/coder/.env

# Edit assistant's tokens
nano ~/.lydia/profiles/assistant/.env
```
...
```bash
coder gateway install         # creates lydia-gateway-coder systemd/launchd service
assistant gateway install     # creates lydia-gateway-assistant service
```

Each profile gets its own service name. They run independently.

:::note Inside the official Docker image
Per-profile gateways are supervised by [s6-overlay](https://github.com/just-containers/s6-overlay) (PID 1 in the container), so `lydia profile create <name>` automatically registers an s6 service slot at `/run/service/gateway-<name>/`. `lydia -p <name> gateway start/stop/restart` dispatches to `s6-svc` instead of spawning a bare process — crashes are auto-restarted and `docker restart` preserves the previously-running set of gateways. See [Per-profile gateway supervision](/user-guide/docker#per-profile-gateway-supervision) for details.
:::
...
```bash
coder config set model.default anthropic/claude-sonnet-4
echo "You are a focused coding assistant." > ~/.lydia/profiles/coder/SOUL.md
```
...
default for **future CLI/gateway runs** (same as `lydia profile use`) —
...
```bash
lydia profile list           # show all profiles with status
lydia profile show coder     # detailed info for one profile
lydia profile rename coder dev-bot   # rename (updates alias + service)
lydia profile export coder   # export to coder.tar.gz
lydia profile import coder.tar.gz   # import from archive
```

## Deleting a profile

```bash
lydia profile delete coder
```

This stops the gateway, removes the systemd/launchd service, removes the command alias, and deletes all profile data. You'll be asked to type the profile name to confirm.

Use `--yes` to skip confirmation: `lydia profile delete coder --yes`

:::note
You cannot delete the default profile (`~/.lydia`). To remove everything, use `lydia uninstall`.
:::
...
```bash
# Bash
eval "$(lydia completion bash)"

# Zsh
eval "$(lydia completion zsh)"
```
...
Profiles use the `LYDIA_HOME` environment variable. When you run `coder chat`, the wrapper script sets `LYDIA_HOME=~/.lydia/profiles/coder` before launching lydia. Since 119+ files in the codebase resolve paths via `get_lydia_home()`, Lydia state automatically scopes to the profile's directory — config, sessions, memory, skills, state database, gateway PID, logs, and cron jobs.

This is separate from terminal working directory. Tool execution starts from `terminal.cwd` (or the launch directory when `cwd: "."` on the local backend), not automatically from `LYDIA_HOME`.
...
isolated by `LYDIA_HOME`, not by changing `HOME`. Container backends still use
`{LYDIA_HOME}/home` for persistent tool state, and host users who need strict
per-profile tool config can opt in with `terminal.home_mode: profile`.

This means two things that are easy to mix up:

- `LYDIA_HOME` is the profile boundary. It controls Lydia config, `.env`,
  memory, sessions, skills, logs, cron jobs, gateway state, and other Lydia
  data.
- `HOME` is the operating-system/user home that external CLIs expect. On host
  installs, Lydia keeps it as the real user home by default so tools like
...
profile` in that profile's `config.yaml`. In that mode Lydia launches tool
subprocesses with `HOME={LYDIA_HOME}/home`; you then need to initialize or link
...
Lydia also exposes `LYDIA_REAL_HOME` to subprocesses so scripts can still find
the actual account home when `home_mode: profile` is active.

The default profile is simply `~/.lydia` itself. No migration needed — existing installs work identically.
