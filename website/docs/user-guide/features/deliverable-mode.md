...
When Lydia Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.lydia/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.lydia/config.yaml` (switchable per session
via `/personality`).
...
If you use Lydia' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.lydia/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Lydia Agent's deliverable
