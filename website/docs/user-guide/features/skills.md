...
All skills live in **`~/.lydia/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Lydia at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
By default every profile is seeded with the bundled skill catalog, and each `lydia update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.lydia` profile):

```bash
curl -fsSL https://lydia-agent.nousresearch.com/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
lydia profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
lydia skills opt-out            # stop future seeding — nothing on disk is touched
lydia skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
lydia skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `lydia update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `lydia skills opt-in`) to re-enable.

:::note Safe by default
`lydia skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Lydia installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Lydia to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.lydia/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
lydia chat --toolsets skills -q "What skills do you have?"
lydia chat --toolsets skills -q "Show me the axolotl skill"
```
...
description, the standard section order, Lydia-tool framing, no invented
commands).
...
/home/user/.lydia/cache/chart-q4-2025.png
...
When a missing value is encountered, Lydia asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `lydia setup` or `~/.lydia/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `lydia config migrate` prompts for unconfigured settings, and `lydia config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.lydia/skills/                  # Single source of truth
...
If you maintain skills outside of Lydia — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Lydia to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.lydia/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.lydia/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Lydia process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Lydia ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.lydia/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.lydia/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `lydia bundles list`.
...
lydia bundles create research

# Overwrite an existing bundle
lydia bundles create backend-dev --skill ... --force

# Delete a bundle
lydia bundles delete backend-dev

# Re-scan ~/.lydia/skill-bundles/ and report changes
lydia bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.lydia/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.lydia/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.lydia/pending/skills/` and are
...
```bash
lydia skills browse                              # Browse all hub skills (official first)
lydia skills browse --source official            # Browse only official optional skills
lydia skills search kubernetes                   # Search all sources
lydia skills search react --source skills-sh     # Search the skills.sh directory
lydia skills search https://mintlify.com/docs --source well-known
lydia skills inspect openai/skills/k8s           # Preview before installing
lydia skills install openai/skills/k8s           # Install with security scan
lydia skills install official/security/1password
lydia skills install skills-sh/vercel-labs/json-render/json-render-react --force
lydia skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
lydia skills list --source hub                   # List hub-installed skills
lydia skills check                               # Check installed hub skills for upstream updates
lydia skills update                              # Reinstall hub skills with upstream changes when needed
lydia skills audit                               # Re-scan all hub skills for security
lydia skills uninstall k8s                       # Remove a hub skill
lydia skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
lydia skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
lydia skills publish skills/my-skill --to github --repo owner/repo
lydia skills snapshot export setup.json          # Export skill config
lydia skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Lydia. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `lydia skills search <query> --source skills-sh`. Lydia resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Lydia currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Lydia repository itself and install with built-in trust.
...
This is Vercel's public skills directory. Lydia can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
lydia skills search https://mintlify.com/docs --source well-known
lydia skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
lydia skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Lydia can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
lydia skills install openai/skills/k8s
lydia skills tap add myorg/skills-repo
```
...
[Skills Hub](https://lydia-agent.nousresearch.com/docs) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no Lydia-side changes required.
...
- Lydia source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Lydia supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Lydia source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Lydia can search and convert agent entries from LobeHub's public catalog into installable Lydia skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Lydia source id: `lobehub`

#### 8. browse.sh (`browse-sh`)

Lydia integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Lydia' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Lydia source id: `browse-sh`
- Trust level: `community`

```bash
lydia skills search airbnb --source browse-sh
lydia skills inspect browse-sh/airbnb.com/search-listings-ddgioa
lydia skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Lydia fetches the URL, parses the YAML frontmatter, security-scans it, and installs.
...
```bash
lydia skills install https://sharethis.chat/SKILL.md
lydia skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
lydia skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `lydia skills update` re-fetches from the same URL automatically when you want to refresh.
...
`lydia skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Lydia | Always trusted |
...
```bash
lydia skills check          # Report which installed hub skills changed upstream
lydia skills update         # Reinstall only the skills with updates available
lydia skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Lydia users add with `lydia skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.lydia.tags`, `version`, `author`, `platforms`, `metadata.lydia.config`).
...
Lydia discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any Lydia user can subscribe and install:

```bash
lydia skills tap add my-org/lydia-skills
lydia skills search deploy
lydia skills install my-org/lydia-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.lydia/.hub/taps.json`:
...
The `lydia skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `lydia skills tap list` shows the effective path per tap.
...
```bash
lydia skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.lydia/.hub/taps.json` (created on demand).

## Bundled skill updates (`lydia skills reset`)

Lydia ships with a set of bundled skills in `skills/` inside the repo. On install and on every `lydia update`, a sync pass copies those into `~/.lydia/skills/` and records a manifest at `~/.lydia/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Lydia recomputes the hash of your local copy and compares it to the origin hash:
...
lydia skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `LYDIA_HOME`, so `lydia -p coder skills reset <name>` only affects that profile.
:::
