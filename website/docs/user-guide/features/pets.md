...
Lydia can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<LYDIA_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Lydia renders the sprite at full fidelity when your
...
lydia pets install boba --select
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `lydia pets list [query] [--limit N]` |
| List installed pets | `lydia pets list --installed` |
| Install a pet | `lydia pets install <slug> [--select] [--force]` |
| Set the active pet | `lydia pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `lydia pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `lydia pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `lydia pets off` |
| Remove an installed pet | `lydia pets remove <slug>` |
| Diagnose setup | `lydia pets doctor` |

`lydia pets show` flags:
...
Beyond installing pre-made pets from the gallery, Lydia can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `lydia pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `lydia native` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
...
always-on-top desktop window. Out there it stays visible while Lydia is
...
Run `lydia pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Lydia uses its own
  profile-scoped `<LYDIA_HOME>/pets/` instead — install through `lydia pets`.
