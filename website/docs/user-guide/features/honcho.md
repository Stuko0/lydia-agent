...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Lydia's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Lydia instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
lydia memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `lydia` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Lydia syncs them back at session init.
...
The `lydia honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `lydia memory setup honcho` (or run `lydia memory setup` and pick it from the list); the `lydia honcho` subcommand then appears on the next invocation.

```bash
lydia memory setup honcho    # Configure Honcho directly (works before activation)
lydia honcho status          # Connection status, config, and key settings
lydia honcho setup           # Redirects to `lydia memory setup` (post-activation alias)
lydia honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
lydia honcho peer            # Show or update peer names + dialectic reasoning level
lydia honcho mode            # Show or set recall mode (hybrid/context/tools)
lydia honcho tokens          # Show or set token budget for context and dialectic
lydia honcho identity        # Seed or show the AI peer's Honcho identity
lydia honcho sync            # Sync Honcho config to all existing profiles
lydia honcho peers           # Show peer identities across all profiles
lydia honcho sessions        # List known Honcho session mappings
lydia honcho map             # Map current directory to a Honcho session name
lydia honcho enable          # Enable Honcho for the active profile
lydia honcho disable         # Disable Honcho for the active profile
lydia honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `lydia honcho`

If you previously used the standalone `lydia honcho setup`:
...
No re-login or re-setup needed. Run `lydia memory setup` and select "honcho" — the wizard detects your existing config.
