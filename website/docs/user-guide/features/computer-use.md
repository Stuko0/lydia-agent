...
```
lydia computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `lydia computer-use
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Lydia app). `lydia computer-use doctor` will tell you which permission is missing. |
...
```
lydia -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.lydia/config.yaml`.

## `lydia computer-use doctor` — your first triage stop

`lydia computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ lydia computer-use doctor
...
agent is acting here." Each Lydia run declares its own cua-driver
**session id** (something like `lydia-3a7b9c14d2e8`); the cursor's
...
Lydia intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the Lydia-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Lydia skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Lydia
...
Lydia applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.lydia/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Lydia applies four layers of optimisation:
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Lydia warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Lydia from inside the RDP / console
...
```
LYDIA_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
LYDIA_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Lydia disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Lydia sets
...
When it's on, `lydia computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Lydia at a binary you built from source instead
of the published release. Lydia resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`LYDIA_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Lydia testing (see notes).
...
### Option B — point Lydia straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`LYDIA_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
LYDIA_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
LYDIA_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Lydia is using your build

- `lydia computer-use status` prints the resolved binary path and
  version.
- `lydia computer-use doctor` confirms the binary is reachable and
...
- **Lydia spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Lydia
  change needed either way.
- **Local builds skip the version check.** Lydia warns when the
...
**First action when anything's off: run `lydia computer-use doctor`.**
...
Run `lydia computer-use install` to fetch the cua-driver binary, or
...
Wayland without an XWayland bridge. `lydia computer-use doctor` will
...
- **Lydia-side skill** — `skills/computer-use/SKILL.md` — teaches the
  Lydia `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Lydia (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Lydia among them)
