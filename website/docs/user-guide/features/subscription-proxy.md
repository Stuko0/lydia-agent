...
chat completions — use your Lydia-managed provider subscription as their
...
| Use case | "Use Lydia as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Lydia stores
the refresh token in `~/.lydia/auth.json` — the same place all Lydia
...
Model:      Lydia-4-70B    # or Lydia-4.3-36B, Lydia-4-405B
```
...
interface in `lydia_cli/proxy/adapters/`.
...
If you see `not logged in`, run `lydia portal`. If you see
...
`lydia_cli/proxy/adapters/<provider>.py` and registering it in
