...
2. **An LLM provider configured** — run `lydia model` or set your preferred provider credentials in `~/.hermes/.env`
...
Voice mode is available in both the **classic CLI** (`lydia chat`) and the **TUI** (`hermes --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.hermes/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
