...
```bash
lydia memory setup      # interactive picker + configuration
lydia memory status     # check what's active
lydia memory off        # disable external provider
```

You can also select the active memory provider via `lydia plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.lydia/config.yaml`:
...
When a memory provider is active, Lydia automatically:
...
**Setup Wizard:**
```bash
lydia memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `lydia honcho setup` command still works (it now redirects to `lydia memory setup`), but is only registered after Honcho is selected as the active memory provider.

**Config:** `$LYDIA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$LYDIA_HOME/honcho.json` > `~/.lydia/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://10.1.200.116:3000/arquant-admin/NewLydia/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/lydia).
...
:::tip Migrating from `lydia honcho`
If you previously used `lydia honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Lydia profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Lydia profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Lydia profile. Host key `lydia` → default; `lydia.<profile>` for others. |
...
```bash
lydia profile create coder --clone
```

`--clone` creates a `lydia.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Lydia profile, creates host blocks for any profile without one, inherits settings from the default `lydia` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `lydia memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://10.1.200.116:3000/arquant-admin/NewLydia/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/lydia).
...
lydia memory setup    # select "openviking"
# Or manually:
lydia config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.lydia/.env
# Authenticated servers should use a user/admin API key:
echo "OPENVIKING_API_KEY=..." >> ~/.lydia/.env
```
...
`OPENVIKING_AGENT` is Lydia' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
lydia memory setup    # select "mem0" → "Platform"
# Or manually:
lydia config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.lydia/.env
```

**Setup (OSS):**
```bash
lydia memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
lydia memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
lydia memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Config:** `$LYDIA_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.lydia/.env`.

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `platform` | `platform` (Mem0 Cloud) or `oss` (self-hosted) |
| `user_id` | `lydia-user` | User identifier |
| `agent_id` | `lydia` | Agent identifier |
...
**Switching modes:** Re-run `lydia memory setup mem0 --mode <platform|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
lydia memory setup    # select "hindsight"
# Or manually:
lydia config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.lydia/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p lydia ui start`

**Config:** `$LYDIA_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `lydia` | Memory bank identifier |
...
**Config:** `config.yaml` under `plugins.lydia-memory-store`
...
# Then configure Lydia
lydia memory setup    # select "byterover"
# Or manually:
...
- Knowledge tree stored at `$LYDIA_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [API key](http://app.supermemory.ai/integrations?connect=lydia) |
...
**Setup:**
```bash
lydia memory setup    # select "supermemory"
# Or manually:
lydia config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.lydia/.env
```

**Config:** `$LYDIA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `lydia` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `lydia-{identity}` → `lydia-coder`) to isolate memories per Lydia profile
...
| **Requires** | `pip install lydia-memori` + `lydia-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install lydia-memori
lydia-memori install
...
| **Memori** | Cloud | Free/Paid | 5 | `lydia-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$LYDIA_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$LYDIA_HOME/` so each profile has its own credentials
