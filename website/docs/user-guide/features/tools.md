...
description: "Overview of Lydia Agent's tools — what's available, how toolsets work, and terminal backends"
---
...
| **X Search** | `x_search` | Search X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool — gated on xAI credentials (SuperGrok OAuth or `XAI_API_KEY`); off by default, opt in via `lydia native` → 🐦 X (Twitter) Search. |
...
Paid [Nous Portal](https://portal.nousresearch.com) subscribers can use web search, image generation, TTS, and browser automation through the **[Tool Gateway](tool-gateway.md)** — no separate API keys needed. Run `lydia model` to enable it, or configure individual tools with `lydia native`.
:::
