...
1. **SQLite database** (`~/.lydia/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Lydia stores session history so it can resume conversations, but it does not
...
Lydia explicitly injects for that turn.
...
For example, if a user sends an image and asks Lydia to make a meme from it,
Lydia may inspect that image once with vision and run an image-processing
...
`lydia sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`lydia` or `lydia chat`) |
...
lydia -c "my project"   # → resumes "my project #3"
```
...
lydia --resume 20250305_091523_a1b2c3d4
lydia -r 20250305_091523_a1b2c3d4

# Resume by title
lydia --resume "refactoring auth"

# Or with the chat subcommand
lydia chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `lydia sessions list`.

### Conversation Recap on Resume

When you resume a session, Lydia displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Lydia session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.lydia/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `lydia -r "<title>"` from the shell) and pick up where the platform left off.
...
Lydia automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `lydia sessions list` or `lydia sessions browse`.
...
```bash
lydia sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Lydia creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`lydia -c "my project"`), it automatically picks the most recent session in the lineage.
...
Lydia provides a full set of session management commands via `lydia sessions`:
...
# Filter by platform
lydia sessions list --source telegram

# Show more sessions
lydia sessions list --limit 50
```
...
lydia sessions export backup.jsonl

# Export sessions from a specific platform
lydia sessions export telegram-history.jsonl --source telegram

# Export a single session
lydia sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
lydia sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
lydia sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
lydia sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
lydia sessions prune

# Custom age threshold
lydia sessions prune --older-than 30

# Only prune sessions from a specific platform
lydia sessions prune --source telegram --older-than 60

# Skip confirmation
lydia sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`lydia insights`](/reference/cli-commands#lydia-insights).
...
When Lydia cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Lydia uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Lydia in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.lydia/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.lydia/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.lydia/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |

The SQLite database uses WAL mode for concurrent readers and a single writer, which suits the gateway's multi-platform architecture well.

:::warning `sessions.json` is not the session list
`~/.lydia/sessions/sessions.json` is the **gateway routing index** — it maps
...
`lydia sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.lydia/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `lydia sessions list`, the cause is
`state.db` not receiving them — run `lydia sessions repair` and watch for a
...
`*.jsonl` files in `~/.lydia/sessions/`. They are no longer written or
read by Lydia. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Lydia process in the same `LYDIA_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.lydia/config.yaml`:
...
lydia sessions delete <session_id>
