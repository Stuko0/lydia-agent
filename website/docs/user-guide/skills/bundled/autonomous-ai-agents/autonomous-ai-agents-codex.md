...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`lydia-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-lydia-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Lydia terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Lydia itself, `model.provider: openai-codex` uses Lydia-managed Codex
OAuth from `~/.lydia/auth.json` after `lydia auth add openai-codex`. For the
...
## Lydia Gateway Caveat

When invoking the Codex CLI from a Lydia gateway/service context (for example,
