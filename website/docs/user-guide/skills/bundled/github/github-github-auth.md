...
elif _lydia_env="${LYDIA_HOME:-$HOME/.lydia}/.env"; [ -f "$_lydia_env" ] && grep -q "^GITHUB_TOKEN=" "$_lydia_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_lydia_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
