...
description: "Operate the Teams meeting summary pipeline via Lydia CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions"
---
...
Operate the Teams meeting summary pipeline via Lydia CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions.
...
Everything operator-facing is a `lydia teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${LYDIA_HOME:-~/.lydia}/.env`:
...
```bash
lydia teams-pipeline validate              # config snapshot — run first after any change
lydia teams-pipeline token-health          # Graph token status
lydia teams-pipeline token-health --force-refresh   # force a fresh token acquisition
lydia teams-pipeline list                  # recent meeting jobs
lydia teams-pipeline list --status failed  # only failed jobs
lydia teams-pipeline show <job-id>         # full detail of one job
lydia teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
lydia teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
lydia teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
lydia teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
lydia teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

lydia teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
lydia teams-pipeline delete-subscription <sub-id>
lydia teams-pipeline maintain-subscriptions            # renew near-expiry ones
lydia teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `lydia teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `lydia cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
