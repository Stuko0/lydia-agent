...
This skill is separate from the Lydia Email gateway adapter. The gateway
adapter lets people email the agent and uses Lydia' built-in IMAP/SMTP
...
To reply non-interactively from Lydia, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from Lydia)** — pipe the message via stdin:
