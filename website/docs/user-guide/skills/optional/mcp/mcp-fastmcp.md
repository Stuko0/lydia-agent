...
| | |
|---|---|
| Source | Optional — install with `lydia skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Lydia or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Lydia. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.lydia/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.lydia/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Lydia integration, either:

- configure the server in `~/.lydia/config.yaml` using the `native-mcp` skill, or
...
### Lydia cannot see the deployed server

The server-building part may be correct while the Lydia config is not. Load the `native-mcp` skill and configure the server in `~/.lydia/config.yaml`, then restart Lydia.
