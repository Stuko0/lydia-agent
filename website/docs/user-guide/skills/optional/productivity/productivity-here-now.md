...
| | |
|---|---|
| Source | Optional — install with `lydia skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${LYDIA_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${LYDIA_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
| `--client {name}`      | Agent name for attribution (e.g. `lydia`)    |
