...
| | |
|---|---|
| Source | Optional — install with `lydia skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain/hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), Lydia Agent |
| License | MIT |
...
The script reads `${LYDIA_HOME:-~/.lydia}/.env` for two optional defaults:
...
Helper script: `~/.lydia/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${LYDIA_HOME:-~/.lydia}/.env`.
