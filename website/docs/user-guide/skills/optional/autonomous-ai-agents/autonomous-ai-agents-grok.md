...
| | |
|---|---|
| Source | Optional — install with `lydia skills install official/autonomous-ai-agents/grok` |
...
| Related skills | [`codex`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`lydia-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-lydia-agent) |
...
# Grok Build CLI — Lydia Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Lydia terminal. Grok
...
2. **Don't conflate Lydia' xAI auth with the `grok` CLI's auth.** Lydia'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Lydia' xAI auth carries
   over.
