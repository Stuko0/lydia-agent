...
description: "Configure and use Honcho memory with Lydia -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Lydia -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `lydia skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Lydia

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Lydia profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/lydia#running-honcho-locally-with-lydia

### Verify

```bash
lydia honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Lydia profile's AI peer
...
Honcho models conversations as interactions between **peers**. Lydia creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Lydia instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Lydia run |
| `global` | Single session across all directories |

Manual override: `lydia honcho map my-project-name`
...
Each Lydia profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block lydia.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `lydia.coder` host block in `honcho.json`
...
```bash
lydia honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Lydia when Honcho memory is active.
...
Config file: `$LYDIA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `lydia honcho setup`. Ensure `memory.provider: honcho` is in `~/.lydia/config.yaml`.

### Memory not persisting across sessions
Check `lydia honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `lydia profile create <name> --clone`. For existing profiles: `lydia honcho sync`.
...
| Command | Description |
|---------|-------------|
| `lydia honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `lydia honcho status` | Show resolved config, connection test, peer info for active profile |
| `lydia honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `lydia honcho disable` | Disable Honcho for the active profile |
| `lydia honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `lydia honcho peers` | Show peer identities across all profiles |
| `lydia honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `lydia honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `lydia honcho sessions` | List known directory-to-session-name mappings |
| `lydia honcho map <name>` | Map current working directory to a Honcho session name |
| `lydia honcho identity` | Seed AI peer identity or show both peer representations |
| `lydia honcho sync` | Create host blocks for all Lydia profiles that don't have one yet |
| `lydia honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Lydia + Honcho |
| `lydia memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `lydia memory status` | Show active memory provider and config |
| `lydia memory off` | Disable external memory provider |
