...
This page shows how to combine worktrees with Lydia so each session has a clean, isolated working directory.

## Why Use Worktrees with Lydia?

Lydia treats the **current working directory** as the project root:

- CLI: the directory where you run `lydia` or `lydia chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.lydia/config.yaml`
...
git worktree add ../repo-feature feature/lydia-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/lydia-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Lydia there:

```bash
cd ../repo-feature

# Start Lydia in the worktree
lydia
```
...
git worktree add ../repo-experiment-a feature/lydia-a
git worktree add ../repo-experiment-b feature/lydia-b
```
...
- Works on its own branch (`feature/lydia-a` vs `feature/lydia-b`).
...
- Lydia checkpoint data under `~/.lydia/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Lydia experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/lydia-checkpoints-docs`, `feature/lydia-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Lydia from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `lydia -w` (Automatic Worktree Mode)

Lydia has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `lydia/lydia-<hash>`).
...
```bash
lydia -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `lydia -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Lydia session its own clean checkout.
