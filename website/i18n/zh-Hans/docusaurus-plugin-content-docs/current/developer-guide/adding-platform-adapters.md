...
Plugin 元数据。`requires_env` 和 `optional_env` 块会自动填充 `lydia config` UI 条目（参见下方[在 lydia config 中暴露环境变量](#surfacing-env-vars-in-hermes-config)）。
...
| `lydia native` / `hermes skills` | Plugin 平台出现在每平台配置中 |
...
## 在 `lydia config` 中暴露环境变量 {#surfacing-env-vars-in-hermes-config}
