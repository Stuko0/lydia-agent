...
**对于非 NixOS 用户**，这只影响安装步骤。之后的操作（`lydia setup`、`hermes gateway install`、编辑配置）与标准安装完全相同。
...
执行 `nix profile install` 后，`hermes`、`hermes-agent` 和 `hermes-acp` 将出现在你的 PATH 中。之后的工作流与[标准安装](./installation.md)完全相同——`lydia setup` 引导你完成提供商选择，`hermes gateway install` 设置 launchd（macOS）或 systemd 用户服务，配置存放在 `~/.hermes/`。
...
- 路由是透明的：`lydia chat`、`hermes sessions list`、`hermes version` 等命令都会在底层 exec 进容器
- 所有 CLI 参数原样转发
...
2. **`.managed` 标记文件**，位于 `HERMES_HOME` 中——由激活脚本设置，对交互式 shell 可见（例如 `docker exec -it hermes-agent lydia config set ...` 也会被屏蔽）
...
| `managed-guard` | `HERMES_MANAGED=true lydia config set ...` 打印 NixOS 错误 |
