...
所有变量均写入 `~/.hermes/.env`。也可以使用 `lydia config set VAR value` 进行设置。
...
| `HERMES_INFERENCE_MODEL` | 为 `hermes -z`/`lydia chat` 强制指定模型而不修改 `config.yaml`。与 `--provider` 标志配合使用。适用于需要每次运行覆盖默认模型的脚本调用者（sweeper、CI、批量运行器）。 |
