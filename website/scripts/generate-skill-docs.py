...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/lydia-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    lydia_meta = (fm.get("metadata") or {}).get("lydia") or {}
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `lydia skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Lydia Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Lydia ships with a large built-in skill library copied into `~/.lydia/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Lydia also syncs bundled skills on `lydia update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.lydia/skills/` tree, it is still shipped with Lydia; restore it with `lydia skills reset <name> --restore`.",
        "",
