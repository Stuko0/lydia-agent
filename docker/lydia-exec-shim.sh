#!/bin/sh
# shellcheck shell=sh
# /opt/lydia/bin/lydia — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `lydia` user (UID 10000). When an operator runs `docker exec <c> lydia ...`
...
# user runs `docker exec <c> lydia login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: Lydia is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> lydia
...
# Fix
# ---
# This shim sits at /opt/lydia/bin/lydia and is placed earliest on PATH.
# When invoked as root, it drops to the lydia user (via s6-setuidgid)
...
# `docker exec --user lydia`, kanban subagents, etc. — it short-circuits
...
# (/opt/lydia/.venv/bin/lydia), so the second hop cannot re-enter this
...
# state via the lydia CLI. Default is to drop.

set -e

REAL=/opt/lydia/.venv/bin/lydia

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "lydia-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user lydia`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi
...
# Root, no opt-out. Drop to the lydia user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "lydia-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "lydia-shim: re-run with --user lydia or set LYDIA_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the lydia user's home before dropping privileges. Without
