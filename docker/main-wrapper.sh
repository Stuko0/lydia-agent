...
# shellcheck shell=sh
# /opt/lydia/docker/main-wrapper.sh — wraps the container's CMD with
...
# from the Dockerfile never reaches `lydia`. with-contenv repopulates
# the env from /run/s6/container_environment before exec'ing, which is
# what s6-supervised services use too (see main-lydia/run).
#
# Routing:
#   no args                       → exec `lydia` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `lydia <args>` (subcommand passthrough)
#
# Drop to lydia via s6-setuidgid, but skip it when already non-root.
set -e

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid lydia "$@"; exec "$@"; }
...
# an arbitrary non-root, non-lydia UID, the bootstrap was skipped and the
# baked image dirs (owned by the lydia build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u lydia)" ]; then
    cat >&2 <<EOF
[lydia] ERROR: container started with --user $cur_uid (an arbitrary, non-lydia UID) — not supported.
...
# to the lydia user's home before dropping privileges so libraries that
...
# the user's command so `lydia chat` starts in the Docker -w
