...
#
# Wired into the image as /etc/cont-init.d/01-lydia-setup by the
...
INSTALL_DIR="/opt/lydia"

# Drop to lydia via s6-setuidgid, but skip it when already non-root.
as_lydia() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid lydia "$@"; }
...
# starts non-root. The baked install tree under /opt/lydia is intentionally
...
# `id -u lydia` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# lydia UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u lydia)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-lydia UID).
...
and the baked /opt/lydia install tree is intentionally root-owned and
...
The image remaps the lydia user to that UID/GID at boot and chowns the data
...
# `s6-setuidgid lydia mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom LYDIA_HOME paths whose parents only
# root can create (e.g. `LYDIA_HOME=/home/lydia/.lydia` in a Compose
...
if [ -n "${LYDIA_UID:-}" ] && validate_uid_gid "$LYDIA_UID" && [ "$LYDIA_UID" != "$(id -u lydia)" ]; then
    echo "[stage2] Changing lydia UID to $LYDIA_UID"
    usermod -u "$LYDIA_UID" lydia
fi
if [ -n "${LYDIA_GID:-}" ] && validate_uid_gid "$LYDIA_GID" && [ "$LYDIA_GID" != "$(id -g lydia)" ]; then
    echo "[stage2] Changing lydia GID to $LYDIA_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$LYDIA_GID" lydia 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised lydia user (UID 10000)
...
# lydia process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes lydia. Idempotent across container
...
#   - lydia is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G lydia 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] lydia already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" lydia 2>/dev/null; then
        echo "[stage2] Added lydia to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group lydia failed; docker backend may fail with EACCES"
    fi
    break
done
...
# the runtime lydia UID, restore ownership to lydia — but ONLY for the
# directories lydia actually writes to. The full $LYDIA_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of lydia-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_lydia_uid=$(id -u lydia)
...
chown_lydia_tree() {
    ...
    chown -R lydia:lydia "$target" 2>/dev/null || \
...
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $LYDIA_HOME (targeted) to lydia ($actual_lydia_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $LYDIA_HOME: chown the directory itself (not its contents)
    # so lydia can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$LYDIA_HOME"; then
        :
    else
        chown lydia:lydia "$LYDIA_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $LYDIA_HOME failed (rootless container?) — continuing"
    fi
    # Lydia-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by lydia (see the s6-setuidgid mkdir
...
# lydia user. Hosted/container instances keep mutable state under
# $LYDIA_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# LYDIA_DISABLE_LAZY_INSTALLS=1. Keeping /opt/lydia root-owned and
...
# even though installs are re-enabled. The dir is seeded + chowned to lydia
...
# Always reset ownership of $LYDIA_HOME/profiles to lydia on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> lydia …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as lydia and walks
...
# (jobs.json) must stay readable by the unprivileged lydia runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$LYDIA_HOME/cron" ]; then
    chown_lydia_tree "$LYDIA_HOME/cron"
fi

# Reset ownership of lydia-owned top-level state files on every boot.
# The targeted data-volume chown above only covers lydia-owned
# *subdirectories*; loose state files living directly under $LYDIA_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> lydia …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged lydia runtime then hits
...
# entries of lydia_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    if [ -e "$LYDIA_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$LYDIA_HOME/$f"; then
            ...
            chown lydia:lydia "$LYDIA_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the lydia runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$LYDIA_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$LYDIA_HOME/config.yaml"; then
        :
    else
        chown lydia:lydia "$LYDIA_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$LYDIA_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as lydia user ---
# Run as lydia via s6-setuidgid so dirs end up owned correctly (matters
...
# /opt/lydia/.install_method (see Dockerfile), NOT written here into
...
# bind-mounted from the host (~/.lydia:/opt/data) and sometimes shared with a
...
# 'lydia update'. To heal homes already poisoned by older images, remove a
...
# the baked /opt/lydia stamp, so deleting the data-dir copy is safe).
...
if [ -f "$LYDIA_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$LYDIA_HOME/.env"; then
        ...
        chown lydia:lydia "$LYDIA_HOME/.env" 2>/dev/null || true
...
# config-schema migrations that `lydia update` runs for non-Docker installs,
...
if [ -f "$LYDIA_HOME/config.yaml" ]; then
    s6-setuidgid lydia "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$LYDIA_HOME/auth.json" ] && [ -n "${LYDIA_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$LYDIA_HOME/auth.json"; then
        ...
        chown lydia:lydia "$LYDIA_HOME/auth.json" 2>/dev/null || true
...
if [ ! -f "$LYDIA_HOME/gateway_state.json" ] && \
    ...
    if refuse_symlinked_path "seed" "$LYDIA_HOME/gateway_state.json"; then
        ...
        chown lydia:lydia "$LYDIA_HOME/gateway_state.json" 2>/dev/null || true
...
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/lydia/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Lydia spawns for the
...
# main-wrapper.sh propagates it into the supervised ``lydia`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-lydia, dashboard, etc.).
