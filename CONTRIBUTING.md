# Contributing to Lydia Agent

Thank you for contributing to Lydia Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Lydia to work everywhere.
...
Related: #38284 covers the agent-side analog — Lydia itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Lydia install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `lydia skills browse` (labeled "official") and install it with `lydia skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `lydia skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.lydia/plugins/` (or via a pip entry point).
...
- Integrate with `lydia memory setup` via `post_setup()` — no need to touch core code
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. Lydia ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.lydia/plugins/`, project `.lydia/plugins/`, or a pip entry point) — see [Build a Lydia Plugin](https://lydia-agent.nousresearch.com/docs/guides/build-a-lydia-plugin)
...
The installer creates the Lydia venv, wires the `lydia` command, stamps the
install method for `lydia update`, and clones the full git project into
`$LYDIA_HOME/lydia-agent` (usually `~/.lydia/lydia-agent`). That keeps your
...
Use this only if you intentionally do not want Lydia' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `lydia` entrypoint from this venv; running the
system `python3 -m lydia_cli.main` can pick up unrelated system Python
packages.
...
export VIRTUAL_ENV="$HOME/.lydia/venvs/lydia-dev"
...
```bash
mkdir -p ~/.lydia/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.lydia/config.yaml
touch ~/.lydia/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.lydia/.env
```

### Run

```bash
# The standard installer already put `lydia` on PATH.
lydia doctor
lydia chat -q "Hello"
```

If you used the manual clone fallback, run `./lydia` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/lydia" ~/.local/bin/lydia
```
...
├── cli.py                    # LydiaCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (lydia-cli, lydia-telegram, etc.)
├── lydia_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── lydia_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.lydia/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (lydia-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.lydia/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.lydia/`)

| Path | Purpose |
|------|---------|
| `~/.lydia/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.lydia/.env` | API keys and secrets |
| `~/.lydia/auth.json` | OAuth credentials (Nous Portal) |
| `~/.lydia/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.lydia/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.lydia/state.db` | SQLite session database |
| `~/.lydia/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.lydia/cron/` | Scheduled job data |
| `~/.lydia/whatsapp/session/` | WhatsApp bridge credentials |
...
Four fields are supported under `metadata.lydia`:
...
The user may skip setup and keep loading the skill. Lydia only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `lydia setup` or update `~/.lydia/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Lydia native or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Lydia Agent" is the secondary collaborator. If the contributor's commit shows "Lydia Agent" as author because they used Lydia to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Lydia native (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `lydia --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Lydia uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.lydia/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `lydia_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `lydia_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Lydia runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the lydia wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `lydia_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `lydia_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `lydia_cli/gateway_windows.py::
...
Lydia has terminal access. Security matters.
...
2. **Test manually**: Run `lydia` and exercise the code path you changed
...
- Include: OS, Python version, Lydia version (`lydia version`), full error traceback
