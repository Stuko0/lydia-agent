...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `lydia setup` or update `~/.hermes/.env` locally.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Lydia Agent" is the secondary collaborator. If the contributor's commit shows "Lydia Agent" as author because they used Hermes to draft the skill, replace it with their actual name — credit the human, not the tool.
