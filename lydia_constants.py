...
def set_lydia_home_override(path: str | Path | None) -> Token:
    """Set a context-local Lydia home override and return its reset token.
...
def reset_lydia_home_override(token: Token) -> None:
    """Restore the previous context-local Lydia home override."""
    _LYDIA_HOME_OVERRIDE.reset(token)


def get_lydia_home_override() -> str | None:
    """Return the active context-local Lydia home override, if any."""
...
def _get_platform_default_lydia_home() -> Path:
    """Return the platform-native default Lydia home path.

    Prefers ``~/.lydia`` (POSIX) / ``%LOCALAPPDATA%\\lydia`` (Windows) for
    new installs.  If ``~/.hermes`` exists with content but ``~/.lydia`` does
    not, automatically migrates (renames) the legacy directory.
    """
    if sys.platform == "win32":
        ...
        if not new_path.exists() and legacy_path.exists():
            _migrate_legacy_home(legacy_path, new_path)
        return new_path
...
def _migrate_legacy_home(legacy: Path, target: Path) -> None:
    """Automatically migrate the legacy ~/.hermes directory to ~/.lydia.

    Renames the directory in-place. If the rename fails (e.g. cross-device),
    logs a warning to stderr and continues — the user can migrate manually.
    """
    try:
        legacy.rename(target)
        sys.stderr.write(
            f"[Lydia] Migrated legacy home {legacy} → {target}\n"
        )
        sys.stderr.flush()
    except OSError as exc:
        sys.stderr.write(
            f"[Lydia] WARNING: Could not migrate {legacy} → {target}: {exc}\n"
            f"[Lydia] Please rename the directory manually.\n"
        )
        sys.stderr.flush()


def get_lydia_home() -> Path:
    """Return the Lydia home directory (default: platform-native path).

    Reads LYDIA_HOME env var, falls back to the platform-native default.
    This is the single source of truth — all other copies should import this.

    When ``LYDIA_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``LYDIA_HOME`` explicitly (see the systemd
    template in ``lydia_cli/gateway.py`` and the kanban dispatcher in
    ``lydia_cli/kanban_db.py``).  See https://10.1.200.116:3000/arquant-admin/NewLydia/issues/18594.
    """
    ...
    if not _profile_fallback_warned:
        ...
        if active and active != "default":
            ...
            msg = (
                f"[LYDIA_HOME fallback] LYDIA_HOME is unset but active "
                f"profile is {active!r}. Falling back to {fallback_home}, which "
                f"is the DEFAULT profile — not {active!r}. Any data this "
                f"process writes will land in the wrong profile. The "
                f"subprocess spawner should pass LYDIA_HOME explicitly "
                f"(see issue #18594)."
            )
            try:
...
def get_default_lydia_root() -> Path:
    """Return the root Lydia directory for profile-level operations.

    In standard deployments this is the platform-native Lydia home
    (``~/.lydia`` on POSIX, ``%LOCALAPPDATA%\\lydia`` on native Windows).

    In Docker or custom deployments where ``LYDIA_HOME`` points outside
    ``~/.lydia`` (e.g. ``/opt/data``), returns ``LYDIA_HOME`` directly
    — that IS the root.

    In profile mode where ``LYDIA_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.lydia/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_lydia_home()
    env_home = os.environ.get("LYDIA_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # LYDIA_HOME is under ~/.lydia (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — LYDIA_HOME itself is the root
    return env_path
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Lydia is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    if default is not None:
        return default
    return get_lydia_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    if default is not None:
        return default
    return get_lydia_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        ...
        4. ``<LYDIA_HOME>/skills`` last-resort
    """
    ...
    if default is not None:
        return default
    return get_lydia_home() / "skills"


def get_lydia_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a Lydia subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to LYDIA_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to LYDIA_HOME (e.g. ``"image_cache"``).
...
def iter_lydia_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Lydia-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\lydia\\node``. POSIX installs use
    ``$LYDIA_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_lydia_home()
    dirs = [root / "node"]
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Lydia-managed Node trees live under ``$LYDIA_HOME/node`` (or a profile's
    ``LYDIA_HOME``). A partial upgrade or interrupted install can leave
    ...
    used to trust file presence alone, so ``lydia update`` would pick that
...
def lydia_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Lydia-managed node/npm/npx shim exists on disk."""
    names = set()
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%LYDIA_HOME%\\node`` on Windows."""
    import re
    import tempfile
...
def heal_lydia_managed_node() -> bool:
    """Redownload Lydia-managed Node when the tree exists but is broken.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "LYDIA_HOME": str(get_lydia_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def find_lydia_node_executable(command: str) -> str | None:
    """Return a Lydia-managed Node/npm executable path, healing broken trees."""
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Lydia-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Lydia-managed installs.

    This is for Lydia-owned subprocesses that should not be broken by a bad,
...
def with_lydia_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Lydia-managed Node directories prepended to PATH."""
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``lydia update`` — leaving a **dangling symlink** that ``which`` still
...
def display_lydia_home() -> str:
    """Return a user-friendly display string for the current LYDIA_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.lydia`` (or ``~/.lydia`` for legacy installs)
        profile:  ``~/.lydia/profiles/coder``
        custom:   ``/opt/lydia-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    a path.  For code that needs a real ``Path``, use
    :func:`get_lydia_home` instead.
    """
    home = get_lydia_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``LYDIA_HOME`` or other path
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{LYDIA_HOME}/home`` when the profile-home directory exists."""
    lydia_home = get_lydia_home_override() or (env or {}).get("LYDIA_HOME") or os.getenv("LYDIA_HOME")
    if not lydia_home:
        return None
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Lydia profile HOME.

    ``LYDIA_HOME`` scopes Lydia state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={LYDIA_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{LYDIA_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{LYDIA_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Lydia' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under LYDIA_HOME.

    Replaces the ``get_lydia_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, lydia_logging.py, lydia_time.py, etc.).
    """
    return get_lydia_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under LYDIA_HOME."""
    return get_lydia_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under LYDIA_HOME."""
