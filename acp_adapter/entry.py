"""CLI entry point for the lydia-agent ACP adapter.

Loads environment variables from ``~/.lydia/.env``, configures logging
...
try:
    ...
    # shadowing Lydia's own modules — ``lydia acp`` can be started from any
    # cwd, including a project that has same-named packages on its path.
    lydia_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from LYDIA_HOME (default ``~/.lydia``)."""
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="lydia-acp",
        description="Run Lydia Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Lydia version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Lydia provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.lydia/node/ "
...
def _run_setup() -> None:
    ...
    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "lydia", "model"]
        lydia_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``lydia postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting lydia-agent ACP adapter")
