"""ACP agent server — exposes Lydia Agent via the Agent Client Protocol."""
...
try:
    from lydia_cli import __version__ as LYDIA_VERSION
except Exception:
    LYDIA_VERSION = "0.0.0"
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Lydia running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Lydia cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Lydia/OpenAI-compatible user content payload."""
...
class LydiaACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Lydia AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Lydia version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Lydia maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from lydia_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from lydia_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Lydia estimates ``used`` from the same
    ...
    def _provenance_meta(
        ...
        """Best-effort ``_meta.lydia.sessionProvenance`` for an ACP session."""
        try:
    ...
    async def _send_session_info_update(
        ...
        *,
        ...
        """Send ACP native session metadata after Lydia changes it.

        When the internal Lydia head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_lydia_session_id`` so the
        attached ``_meta.lydia.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # lydia_state.py schema — only started_at/ended_at). Use "now" as
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["lydia-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="lydia-agent", version=LYDIA_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Lydia' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Lydia setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Lydia
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Lydia on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Lydia' local kawaii waiting/status
        ...
        try:
            # Snapshot the internal Lydia DB session id before the turn so we
        ...
        # _meta.lydia.sessionProvenance so ACP clients can render the boundary
        ...
        # Lydia' local "waiting for model response" interrupt status is metadata,
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Lydia Agent v{LYDIA_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Lydia has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
