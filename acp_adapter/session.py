"""ACP session manager — maps ACP sessions to Lydia AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.lydia/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Lydia itself is running in WSL.

    Windows ACP clients can launch ``lydia acp`` inside WSL while still sending
    ...
    """
    from lydia_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Lydia from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["lydia-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Lydia Agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Lydia AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Lydia runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.lydia/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``LYDIA_HOME`` dynamically rather than relying on
    ...
    def _make_agent(
        ...
        *,
        ...
        # editor/session cwd instead of the Lydia daemon's process cwd.
