"""ACP auth helpers — detect and advertise Lydia authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "lydia-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Lydia runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Lydia can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Lydia.
    ...
    not have Lydia provider credentials configured yet, so Lydia always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Lydia using the currently configured "
