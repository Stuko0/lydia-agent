# OpenAI-Compatible API Server for Lydia Agent
...
SSE streaming. By exposing this endpoint, lydia-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  lydia-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.lydia/.env`
2. Run `lydia gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with lydia-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns lydia-agent as a model) |
...
Authorization: Bearer lydia-api-key-here
...
But lydia-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "lydia-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured lydia-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of lydia-agent's core prompt
...
Once implemented, lydia-agent works as a drop-in backend for:
