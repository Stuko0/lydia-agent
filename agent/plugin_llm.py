...
Plugins built on Lydia Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Lydia
...
:class:`~lydia_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Lydia' main config
...
handles every provider, fallback chain, and per-task override Lydia
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`lydia_cli.plugins.PluginContext`
