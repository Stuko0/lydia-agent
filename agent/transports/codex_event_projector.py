"""Projects codex app-server events into Lydia' messages list.

The translator that lets Lydia' memory/skill review keep working under the
...
Lydia' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Lydia
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Lydia' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Lydia stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Lydia already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Lydia' messages store text only.
        text_parts: list[str] = []
