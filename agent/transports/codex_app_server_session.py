...
Owns one Codex thread per Lydia session. Drives `turn/start`, consumes
...
# Lydia' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_LYDIA_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Lydia/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Lydia session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _LYDIA_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("LYDIA_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Lydia' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Lydia' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own lydia-tools callback we
            # auto-accept — the user already approved Lydia' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Lydia approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Lydia returns 'once', 'session', 'always', or 'deny'.
...
def _get_lydia_version() -> str:
    """Best-effort Lydia version string for codex's userAgent line."""
    try:
