...
Storage
-------
Bundles live in ``~/.lydia/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``LYDIA_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``lydia bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under LYDIA_HOME.

    Honors ``LYDIA_BUNDLES_DIR`` for tests; falls back to
    ``<LYDIA_HOME>/skill-bundles``.
    """
    override = os.environ.get("LYDIA_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_lydia_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `lydia bundles` CLI subcommand.
# ---------------------------------------------------------------------------
