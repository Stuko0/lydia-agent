...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `lydia backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - lydia_home (str): The active LYDIA_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.lydia``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "lydia").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'lydia memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], lydia_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'lydia memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``lydia_home`` is the active LYDIA_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE LYDIA_HOME.

        ``lydia backup`` only walks LYDIA_HOME, so any provider state kept
        ...
        of the archive, and ``lydia import`` restores them to their original
