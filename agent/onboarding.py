...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Lydia starts and finds ``~/.openclaw/``.

    Points users at ``lydia claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``lydia claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Lydia, run "
        "`lydia claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `lydia claw cleanup` (renames it to ~/.openclaw.pre-migration — "
