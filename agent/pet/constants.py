...
row taxonomy is inferred from the atlas shape so Lydia can render both legacy
...
# rows (1536x1872), while older Lydia/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Lydia' activity state names. They are not always identical to the
...
# Legacy Lydia/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Lydia activity names -> accepted row-name aliases in descending
