...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.lydia/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Lydia ↔ models.dev
# ---------------------------------------------------------------------------

# Lydia provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Lydia (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_lydia_home() / "models_dev_cache.json"
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Lydia provider ID to its models dict from models.dev.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a Lydia provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Lydia ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Lydia or models.dev provider ID.  Tries exact match then
