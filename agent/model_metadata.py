...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Lydia also honours LYDIA_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("LYDIA_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _get_model_metadata_cache_path() -> Path:
    ...
    return get_lydia_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run Lydia Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Lydia from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for Lydia' system prompt, but mapping
...
def _get_context_cache_path() -> Path:
    ...
    from lydia_constants import get_lydia_home
    return get_lydia_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Lydia grow conversations past the runtime limit and Ollama
...
def get_model_context_length(
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from lydia_cli.models import get_copilot_model_context
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Lydia sends to providers:
