...
from lydia_constants import get_lydia_home, get_skills_dir, is_wsl
...
_LYDIA_MD_NAMES = (".lydia.md", "LYDIA.md")


def _find_lydia_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.lydia.md`` or ``LYDIA.md``.
    ...
    # could pick up a .lydia.md planted in /tmp, /home, etc.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Lydia Agent, an intelligent AI assistant created by Nous Research. "
...
LYDIA_AGENT_HELP_GUIDANCE = (
    "You run on Lydia Agent (by Nous Research). When the user needs help with "
    "Lydia itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://lydia-agent.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `lydia-agent` skill with skill_view(name='lydia-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.lydia/kanban.db`. Your task id is in "
    ...
    "every assignee in a real profile (`lydia profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `lydia kanban <verb>` for board operations. Use "
...
#
# The lydia-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to lydia-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `lydia computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Lydia "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Lydia terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "webui": (
        "You are in the Lydia WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Lydia itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Lydia runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Lydia itself is running. The host OS, home, and cwd "
                f"of the Lydia process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Lydia "
    ...
    # Lydia desktop GUI — any agent running under the desktop app should know
    # it. LYDIA_DESKTOP marks the backend powering the chat; LYDIA_DESKTOP_TERMINAL
    # marks a lydia launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    ...
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Lydia desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
    ...
    # Embedder-supplied environment description. Lets a host that wraps Lydia
...
def _skills_prompt_snapshot_path() -> Path:
    return get_lydia_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.lydia/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Lydia Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `lydia-agent` skill "
            "first. It has the actual commands (e.g. `lydia config set …`, `lydia native`, "
            "`lydia setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: lydia setup, lydia setup tools, lydia setup terminal, lydia status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from LYDIA_HOME and return its content, or None.
    ...
    """
    try:
        from lydia_cli.config import ensure_lydia_home
        ensure_lydia_home()
    except Exception as e:
        logger.debug("Could not ensure LYDIA_HOME before loading SOUL.md: %s", e)

    soul_path = get_lydia_home() / "SOUL.md"
...
def _load_lydia_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.lydia.md / LYDIA.md — walk to git root."""
    ...
    try:
        ...
        return _truncate_content(
            result, ".lydia.md", context_length=context_length,
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .lydia.md / LYDIA.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from LYDIA_HOME is independent and always included when present.
    ...
    # SOUL.md from LYDIA_HOME only — skip when already loaded as identity
    if not skip_soul:
