...
Every Lydia request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Lydia Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=lydia-agent",
        "client=lydia-client-v<__version__>",
    ]

The version is sourced live from ``lydia_cli.__version__`` so it auto-aligns
...
``lydia_cli.__version__`` is the canonical source of truth.
"""
...
def _lydia_version() -> str:
    """Return the current Lydia release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``lydia_cli`` cannot be imported (should
...
def lydia_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=lydia-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=lydia-client-v{_lydia_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=lydia-agent", lydia_client_tag()]
