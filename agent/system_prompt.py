...
upstream prefix cache warm.  See ``lydia-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Lydia never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want LYDIA_HOME persona while keeping
    ...
    # Pointer to the lydia-agent skill + docs for user questions about Lydia itself.
    stable_parts.append(LYDIA_AGENT_HELP_GUIDANCE)
    ...
    # LYDIA_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Lydia, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Lydia profile the agent is running
    # under so it doesn't conflate ~/.lydia/skills/ (default profile) with
    # ~/.lydia/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Lydia profile: default. Other profiles (if any) live "
            "under ~/.lydia/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Lydia profile: {active_profile}. This session reads "
            f"and writes ~/.lydia/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.lydia/skills/, ~/.lydia/plugins/, "
            f"~/.lydia/cron/, ~/.lydia/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Lydia never rebuilds or reinjects parts of it
