...
from lydia_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # The Lydia Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``lydia_cli.config`` on the
...
# which becomes the dominant cost of ``lydia`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.lydia/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``lydia`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against LYDIA_HOME, not cwd
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.lydia/skills/`` first, then external.
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Lydia can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    lydia = metadata.get("lydia") or {}
    if not isinstance(lydia, dict):
        lydia = {}
    return {
        "fallback_for_toolsets": lydia.get("fallback_for_toolsets", []),
        "requires_toolsets": lydia.get("requires_toolsets", []),
        "fallback_for_tools": lydia.get("fallback_for_tools", []),
        "requires_tools": lydia.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    lydia = metadata.get("lydia")
    if not isinstance(lydia, dict):
        return []
    raw = lydia.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Lydia metadata, VCS, virtualenv/dependency, cache, and skill
