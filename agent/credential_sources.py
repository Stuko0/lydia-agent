"""Unified removal contract for every credential source Lydia reads from.

Lydia seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.lydia/.env
    claude_code   — ~/.claude/.credentials.json
    lydia_pkce   — ~/.lydia/.anthropic_oauth.json
    ...
    manual        — user ran `lydia auth add`
...
unify here is **removal**:
    ...
    ``lydia auth remove <provider> <N>`` must make the pool entry stay gone.
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.lydia/.env  → clear it
    ...
    """
    from lydia_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Lydia.",
            f"  The pool entry is now suppressed — Lydia will ignore "
            f"{env_var} until you run `lydia auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Lydia stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `lydia auth add anthropic` to re-enable if needed.",
    ])


def _remove_lydia_pkce(provider: str, removed) -> RemovalResult:
    """~/.lydia/.anthropic_oauth.json is ours — delete it outright."""
    from lydia_constants import get_lydia_home

    result = RemovalResult()
    oauth_file = get_lydia_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Lydia Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    `lydia auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `lydia auth add nous` to re-engage, which is the
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``lydia auth remove xai-oauth <N>`` silently undoes
    ...
    Belt-and-braces against the manual entry path: ``lydia auth add
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Lydia auth store is not enough — _seed_from_singletons() would
    ...
    via ``lydia auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `lydia auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `lydia auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Lydia stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `lydia auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        ...
        description="~/.lydia/.anthropic_oauth.json",
    ))
    register(RemovalStep(
