...
def _lydia_home_path() -> Path:
    """Resolve the active LYDIA_HOME (profile-aware) without circular imports."""
    try:
        from lydia_constants import get_lydia_home  # local import to avoid cycles
        return get_lydia_home()
    except Exception:
        return Path(os.path.expanduser("~/.lydia"))


def _lydia_root_path() -> Path:
    """Resolve the Lydia root dir (always the parent of any profile, never per-profile)."""
    try:
        from lydia_constants import get_default_lydia_root  # local import to avoid cycles
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Lydia path.

    Three categories are blocked:

      * Internal Lydia cache files under ``LYDIA_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under LYDIA_HOME and the global Lydia
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.lydia/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active LYDIA_HOME (profile-aware) AND the global
    # Lydia root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (LYDIA_HOME points at
    ...
    for hd in lydia_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Lydia cache file "
    ...
    for hd in lydia_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Lydia credential store "
    ...
    for hd in lydia_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Lydia MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Lydia MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Lydia profiles are separate LYDIA_HOME dirs under
...
#
# Reference: May 2026 incident where a lydia-security profile session
# edited skills under both ``~/.lydia/profiles/lydia-security/skills/``
# AND ``~/.lydia/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under LYDIA_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from LYDIA_HOME.
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Lydia scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Lydia entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Lydia profile {info['target_profile']!r}, but the "
...
#
#   <LYDIA_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.lydia/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.lydia/…`` segment and warns
# regardless of which Lydia profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.lydia/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.lydia`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.lydia/…`` and returns the
    index where the inner Lydia-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .lydia / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".lydia":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Lydia state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.lydia``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.lydia`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Lydia resolver to
    succeed, so it works correctly even when called from contexts where
    LYDIA_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Lydia process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real LYDIA_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.lydia/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.lydia/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host LYDIA_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Lydia state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Lydia "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real LYDIA_HOME. Use the "
