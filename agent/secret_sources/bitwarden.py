...
Lydia pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.lydia/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<lydia_home>/bin/bws`` on
  first use.  Lydia pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.lydia/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``lydia`` invocations don't
  hammer the API.
* Failures NEVER block Lydia startup.  Missing binary, no network,
...
# In-process cache so repeated load_lydia_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, "_CachedFetch"] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `lydia chat -q ...`
...
# never the access token. It's plaintext-equivalent to ~/.lydia/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    ...
    `home_path` is what `load_lydia_dotenv()` already resolved; falling back
    to `$LYDIA_HOME` / `~/.lydia` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("LYDIA_HOME", Path.home() / ".lydia"))
...
def _lydia_bin_dir() -> Path:
    """Where Lydia stores its managed binaries.  Profile-aware."""
    from lydia_constants import get_lydia_home

    return get_lydia_home() / "bin"
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``lydia secrets bitwarden setup``
    ...
    with tempfile.TemporaryDirectory(prefix="lydia-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "lydia-agent"})
    try:
...
# ---------------------------------------------------------------------------
# Public entry point — called from lydia_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_lydia_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `lydia secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `lydia secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `lydia secrets bitwarden setup` to install."
        )
        return result
