# Lydia Agent - Development Guide

Instructions for AI coding assistants and developers working on the lydia-agent codebase.

**Never give up on the right solution.**

## What Lydia Is

Lydia is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Lydia ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`lydia native`, `lydia setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `LYDIA_HOME`. Mocks hide integration bugs.
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `lydia native`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.lydia/plugins/` (or via a
  pip entry point), and promote them in the Stuko0 Discord
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `lydia <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `lydia webhook`, `lydia cron`, `lydia native`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.lydia/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.lydia/lydia-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── cli.py                # LydiaCLI class — interactive CLI orchestrator (~11k LOC)
├── lydia_state.py       # SessionDB — SQLite session store (FTS5 search)
├── lydia_constants.py   # get_lydia_home(), display_lydia_home() — profile-aware paths
├── lydia_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── lydia_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── lydia-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `lydia --tui`
...
**User config:** `~/.lydia/config.yaml` (settings), `~/.lydia/.env` (API keys only).
**Logs:** `~/.lydia/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_lydia_home()`.
Browse with `lydia logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Lydia: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`lydia_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `LydiaCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.lydia/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`lydia_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/lydia` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `lydia_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `LydiaCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `lydia --tui` or `LYDIA_TUI=1`.
...
npm run dev       # watch mode (rebuilds lydia-ink + tsx --watch)
npm start         # production
npm run build     # full build (lydia-ink + tsc)
...
### TUI in the Dashboard (`lydia dashboard` → `/chat`)

The dashboard embeds the real `lydia --tui` — **not** a rewrite.  See `lydia_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `lydia_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `lydia --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `lydia --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). The WebSocket/JSON-RPC transport lives in the framework-agnostic `apps/shared` package (`@lydia/shared` — `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard (`web/`) also consumes; **desktop has no build/runtime dependency on the dashboard frontend** — it spawns a headless `lydia serve` backend server (the same gateway `dashboard` serves, minus the browser UI). `dashboard` and `serve` share `cmd_dashboard`/`start_server` but are independent surfaces — neither launches the other. The one exception is a backward-compat *fallback*: `serve` is newer, so the desktop spawn (`electron/backend-command.cjs` + `backendSupportsServe()` in `main.cjs`) detects whether the resolved runtime registers `serve` and, only when it does not (an older managed install / PATH `lydia` the app hasn't updated yet), rewrites the argv to the legacy `dashboard --no-open`. Without that, a new app against an un-upgraded runtime would crash on an unknown subcommand and brick every mid-upgrade user. It does NOT embed `lydia --tui` — it has its own composer, transcript, and slash-command pipeline. Route desktop bugs to the `lydia-desktop-app-work` skill, not `lydia-dashboard-work`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMANDS` (the ~19 built-ins shown in the palette) plus block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Lydia built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Lydia core.
Use the plugin route instead: create `~/.lydia/plugins/<name>/plugin.yaml`
and `~/.lydia/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Lydia tool that should ship in the base system.
...
**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_lydia_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `LYDIA_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_lydia_home()` for the base directory — never `Path.home() / ".lydia"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `lydia_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `lydia_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `lydia native`, `lydia setup`, most CLI subcommands | `lydia_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`lydia_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
lydia_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.lydia/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
Add to `_BUILTIN_SKINS` dict in `lydia_cli/skin_engine.py`:
...
Users create `~/.lydia/skins/<name>.yaml`:
...
Lydia has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.lydia/plugins/` and pip-installed entry points.

### General plugins (`lydia_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.lydia/plugins/`, `./.lydia/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `lydia` at startup so
  `lydia <pluginname> <subcmd>` works with no change to `main.py`
...
it at argparse setup time and wires it into `lydia <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `lydia --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `lydia_cli/main.py`, etc.).
...
into `~/.lydia/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `lydia memory setup` / `post_setup()` without
...
`~/.lydia/plugins/` (or via pip entry points). They register through
...
don't own. Promote standalone plugins in the Stuko0 Discord
...
2. User: `$LYDIA_HOME/plugins/model-providers/<name>/`
...
[`lydia-example-plugins`](https://github.com/NousResearch/lydia-example-plugins)
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `lydia skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.lydia.tags`, `metadata.lydia.category`,
`metadata.lydia.related_skills`, `metadata.lydia.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.lydia.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Lydia native or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Lydia Agent" is the secondary collaborator. If the
   contributor's commit shows "Lydia Agent" as author (because they
   used Lydia to draft the skill), replace it with their actual name
...
lives in the `lydia-agent-dev` skill at
...
Enable/disable per platform via `lydia native` (the curses UI) or the
...
Lydia returns a delegation id immediately and the result re-enters the
...
go to `~/.lydia/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `lydia_cli/curator.py` wires `lydia curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.lydia/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `lydia cron <verb>`
...
- File lock at `~/.lydia/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `lydia kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `lydia_cli/kanban.py` wires `lydia kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`lydia-kanban-dispatcher.service` for
...
Lydia-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
Lydia supports **profiles** — multiple fully isolated instances, each with its own
`LYDIA_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `lydia_cli/main.py` sets
`LYDIA_HOME` before any module imports. All `get_lydia_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_lydia_home()` for all LYDIA_HOME paths.** Import from `lydia_constants`.
   NEVER hardcode `~/.lydia` or `Path.home() / ".lydia"` in code that reads/writes state.
   ```python
   # GOOD
   ...
   # BAD — breaks profiles
   config_path = Path.home() / ".lydia" / "config.yaml"
   ```

2. **Use `display_lydia_home()` for user-facing messages.** Import from `lydia_constants`.
   This returns `~/.lydia` for default or `~/.lydia/profiles/<name>` for profiles.
   ```python
   # GOOD
   from lydia_constants import display_lydia_home
   print(f"Config saved to {display_lydia_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.lydia/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_lydia_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_lydia_home()`,
   not `Path.home() / ".lydia"`.

4. **Tests that mock `Path.home()` must also set `LYDIA_HOME`** — since code now uses
   `get_lydia_home()` (reads env var), not `Path.home() / ".lydia"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"LYDIA_HOME": str(tmp_path / ".lydia")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not LYDIA_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".lydia" / "profiles"`, NOT `get_lydia_home() / "profiles"`.
   This is intentional — it lets `lydia -p coder profile list` see all profiles regardless
   of which one is active.

## Known Pitfalls

### DO NOT hardcode `~/.lydia` paths
Use `get_lydia_home()` from `lydia_constants` for code paths. Use `display_lydia_home()`
for user-facing print/log messages. Hardcoding `~/.lydia` breaks profiles — each profile
has its own `LYDIA_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### DO NOT introduce new `simple_term_menu` usage
Existing call sites in `lydia_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `lydia_cli/curses_ui.py` — see
`lydia_cli/tools_config.py` for the canonical pattern.
...
with actual imports (not mocks) against a temp `LYDIA_HOME`.

### Tests must not write to `~/.lydia/`
The `_isolate_lydia_home` autouse fixture in `tests/conftest.py` redirects `LYDIA_HOME` to a temp dir. Never hardcode `~/.lydia/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_lydia_home()` resolve within the temp dir.
Use the pattern from `tests/lydia_cli/test_profiles.py`:
```python
@pytest.fixture
def profile_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("LYDIA_HOME", str(home))
    return home
```
...
| HOME / `~/.lydia/` | Your real config+auth.json                  | Temp dir per test                         |
