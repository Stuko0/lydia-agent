...
React + Ink terminal UI for Lydia. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `LYDIA_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `lydia setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.lydia/.lydia_history` or under `LYDIA_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `lydia setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/lydia-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external lydia setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.lydia/tui-parent.log
    ...
    types/
      lydia-ink.d.ts            type declarations for @lydia/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent LydiaCLI subprocess for slash commands
```