...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.LYDIA_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Lydia on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via LYDIA_TUI_TRUECOLOR=1.
