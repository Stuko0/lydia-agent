import type { RunExternalProcess } from '@lydia/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`lydia ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching lydia: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`lydia ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
