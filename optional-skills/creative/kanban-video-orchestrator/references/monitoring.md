...
lydia kanban watch --tenant <project-slug>

# Snapshot of the board
lydia kanban list --tenant <project-slug>
lydia kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
lydia kanban stats --tenant <project-slug>

# Visual dashboard (browser)
lydia dashboard

# Inspect a specific task (includes comments + events)
lydia kanban show <task-id>

# Follow a single task's event stream
lydia kanban tail <task-id>
```

Verify available subcommands with `lydia kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `lydia kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `lydia kanban block <id>` then `lydia kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `lydia kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `lydia kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `lydia profile list` (profile exists?), `lydia status` (gateway/dispatcher up?), and verify tenant. |
...
lydia kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
lydia kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(lydia kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
lydia kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
lydia kanban block <task-id>
lydia kanban archive <task-id>

# Diagnose what happened
lydia kanban show <task-id>      # task body, comments, recent events
lydia kanban tail <task-id>      # follow the live event stream
lydia kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `lydia kanban create` for the director
...
while true; do
    clear
    lydia kanban list --tenant <slug>
    echo "---"
    lydia kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `lydia kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `lydia profile list` and start
