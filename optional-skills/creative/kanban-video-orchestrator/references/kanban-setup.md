...
the actual `setup.sh` that creates the project workspace, configures Lydia
...
2. **Create profiles** — `lydia profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.lydia/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `lydia kanban create` assigned to the director
...
```bash
lydia profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.lydia/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.lydia/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Lydia
...
Each profile gets a `SOUL.md` at `~/.lydia/profiles/<name>/SOUL.md` that
...
```bash
lydia kanban create "Direct production of <video title>" \
    --assignee director \
...
the Lydia `.env` (`${LYDIA_HOME:-$HOME/.lydia}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _lydia_env="${LYDIA_HOME:-$HOME/.lydia}/.env"
...
check_key ELEVENLABS_API_KEY lydia ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY lydia OPENROUTER_API_KEY || exit 1
# ...
```
