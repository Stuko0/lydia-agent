...
Usage (via cron with --no-agent):
    ...
    lydia cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$LYDIA_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Lydia-Watcher/1.0"})
