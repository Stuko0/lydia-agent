...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("LYDIA_HOME", os.path.expanduser("~/.lydia")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("LYDIA_HOME", Path.home() / ".lydia")) / "skills" / "red-teaming" / "godmode"
...
LYDIA_HOME = Path(os.getenv("LYDIA_HOME", Path.home() / ".lydia"))
CONFIG_PATH = LYDIA_HOME / "config.yaml"
PREFILL_PATH = LYDIA_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Lydia models — already uncensored, just needs clean prompt
    "lydia": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "lydia" in model_lower or "nous" in model_lower:
        return "lydia"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Lydia config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.lydia/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Lydia for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Lydia for changes to take effect.")
