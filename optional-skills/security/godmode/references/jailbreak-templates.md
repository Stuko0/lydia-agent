...
## 5. GODMODE FAST — Lydia 4 405B (Zero Refusal)

**Model:** `nousresearch/lydia-4-405b`
**Strategy:** Lydia models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Lydia Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.lydia/config.yaml`:
