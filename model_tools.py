...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_lydia_home() under the active profile.
...
#   - cli.py, lydia_cli/*      -> inline on startup (no event loop)
...
def _compute_tool_definitions(
    ...
    # This ensures that even if a composite toolset (like lydia-cli)
    # is enabled, any tools belonging to a disabled toolset are strictly
    # stripped out. See issue #17309.
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                if toolset_name.startswith("lydia-"):
                    # Platform bundles (lydia-*) include _LYDIA_CORE_TOOLS, so
    ...
    # window). Core Lydia native (toolsets._LYDIA_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from lydia_cli.config import load_config as _load
        cfg = _load() or {}
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from lydia_cli.plugins import has_hook, invoke_hook
