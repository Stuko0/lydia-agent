...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `lydia native` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    # Coding posture (base Lydia — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in lydia_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Lydia nativeets (CLI + messaging platforms)
    #
    ...
    # the `lydia send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "lydia-cron": {
        # Mirrors lydia-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `lydia native` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as lydia-cli; gated by `lydia native`",
        "tools": _LYDIA_CORE_TOOLS,
        "includes": []
    },
    ...
    "lydia-email": {
        "description": "Email bot toolset - interact with Lydia via email (IMAP/SMTP)",
        "tools": _LYDIA_CORE_TOOLS,
        "includes": []
    },
    ...
    "lydia-sms": {
        "description": "SMS bot toolset - interact with Lydia via SMS (Twilio)",
        "tools": _LYDIA_CORE_TOOLS,
        "includes": []
    },
    ...
    "lydia-gateway": {
        ...
        "tools": [],
        "includes": ["lydia-telegram", "lydia-discord", "lydia-whatsapp", "lydia-slack", "lydia-signal", "lydia-bluebubbles", "lydia-homeassistant", "lydia-email", "lydia-sms", "lydia-mattermost", "lydia-matrix", "lydia-dingtalk", "lydia-feishu", "lydia-wecom", "lydia-wecom-callback", "lydia-weixin", "lydia-qqbot", "lydia-webhook", "lydia-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``lydia-*`` bundle's platform-specific tools, excluding core.
    ...
    Bundle nesting is one level deep in practice (only ``lydia-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (lydia-<name>).
        # Gives them _LYDIA_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("lydia-"):
            platform_name = name[len("lydia-"):]
            try:
