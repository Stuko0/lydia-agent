...
Uses a file-based lock (~/.lydia/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `lydia update` reloads
# the module) fail with ModuleNotFoundError for lydia_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from lydia_constants import get_lydia_home
from lydia_cli._subprocess_compat import windows_hide_flags
from lydia_cli.config import load_config, _expand_env_vars
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy lydia_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from lydia_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``lydia native`` config for the ``cron`` platform.
    ...
    try:
        from lydia_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _get_lydia_home() -> Path:
    """Resolve Lydia home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active LYDIA_HOME at call time
    ...
    """
    return _lydia_home or get_lydia_home()
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within LYDIA_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Lydia-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against LYDIA_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    # escape — scripts MUST reside within LYDIA_HOME/scripts/.
    try:
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    try:
        ...
        agent = AIAgent(
            ...
            # LYDIA_HOME. When a workdir is configured, also inject project
