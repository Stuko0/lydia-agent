...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``lydia://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``lydia://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"lydia://blueprint/{quote(blueprint.key)}{qs}"
