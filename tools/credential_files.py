...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.lydia",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``LYDIA_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside LYDIA_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    lydia_home = _resolve_lydia_home()

    # Reject absolute paths — they bypass the LYDIA_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to LYDIA_HOME)",
            relative_path,
        )
        return False
    ...
    # that traversal like ``../. ssh/id_rsa`` cannot escape LYDIA_HOME.
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.lydia/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="lydia-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching lydia_constants.get_lydia_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_lydia_dir()`` for backward compatibility with old directory layouts.
    """
    from lydia_constants import get_lydia_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_lydia_dir(new_subpath, old_name)
        if host_dir.is_dir():
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.lydia``,
    SSH ``<remote_home>/.lydia``, etc.) and whether translation is wanted.
