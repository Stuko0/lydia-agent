...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``lydia_constants.get_subprocess_home()`` so that ``~`` resolves
    ...
    if not path or "~" not in path:
        return path
    try:
        from lydia_constants import get_subprocess_home
...
def _get_lydia_config_resolved() -> str | None:
    """Return the resolved absolute path of the Lydia config file (cached)."""
    ...
    try:
        from lydia_cli.config import get_config_path
        _lydia_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _lydia_config_resolved = str(Path(_expand_tilde("~/.lydia/config.yaml")).resolve())
        except Exception:
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Lydia config file directly.
    ...
    if lydia_config and (resolved == lydia_config or normalized == lydia_config):
        return (
            f"Refusing to write to Lydia config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.lydia/config.yaml directly or use 'lydia config' instead."
        )
    return None


def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Lydia mirror prefix for Docker file tools."""
    try:
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another Lydia
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of Lydia state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.lydia/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      Lydia process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.lydia/…`` path.

    Returns ``None`` when the write is in-scope or outside Lydia scope.
    ...
    # in a session that cd'd into ``~/.lydia/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Lydia internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under LYDIA_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == LYDIA_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-Lydia-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-Lydia-profile guard for
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Lydia profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Lydia profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with lydia_tools.write_file() for very "
            "large files."
        )
