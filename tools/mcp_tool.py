...
transport, discovers their tools, and registers them into the lydia-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.lydia/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.lydia/logs/mcp-stderr.log), tagged with the
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            lydia_home = os.path.expanduser(
                os.getenv(
                    "LYDIA_HOME", os.path.join(os.path.expanduser("~"), ".lydia")
                )
            )
            candidates = [
                ...
                # image (which the Lydia Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Lydia MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Lydia gateways know how to render.
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Lydia' existing approval
...
class MCPServerTask:
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "  pip install 'lydia-agent[mcp]'\n"
                "or (full install):\n"
                "  pip install 'lydia-agent[all]'"
            )
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Lydia' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Lydia is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `lydia mcp login {server_name}` (or delete the tokens "
            f"file under ~/.lydia/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local LYDIA_HOME override into ``coro``.
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local LYDIA_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_lydia_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Lydia config file.
    ...
    ``os.environ`` (which includes ``~/.lydia/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Lydia' MEDIA: tag convention and out to messaging
            ...
            # Lydia' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Lydia's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Lydia registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``lydia native`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Lydia registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple lydia
