...
Backend is selected during ``lydia native`` setup (web.backend in config.yaml).
When available, Lydia can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Lydia config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Lydia' config/.env layer (``lydia config set``, ``lydia native``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.lydia/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``lydia native``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `lydia native` repaint.
        try:
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `lydia native` to set one up."
                ),
            }
        else:
            logger.info(
