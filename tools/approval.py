...
# via shell expansions like $HOME or $LYDIA_HOME, or by the resolved absolute
# active profile home path such as /home/lydia/.lydia/config.yaml. The
# resolved-absolute form is folded into the ~/.lydia/ patterns at detection
...
# go stale when LYDIA_HOME is set after this module is imported, e.g. under the
...
# ~/.lydia/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _LYDIA_ENV_PATH; matches the LYDIA_HOME override form as
# well as ~/.lydia/.
_LYDIA_CONFIG_PATH = (
    r'(?:~\/\.lydia/|'
    r'(?:\$home|\$\{home\})/\.lydia/|'
...
DANGEROUS_PATTERNS = [
    ...
    # `lydia` and `gateway` (e.g. `lydia -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\blydia\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart lydia gateway (kills running agents)"),
    (r'\blydia\s+update\b', "lydia update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `lydia gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart lydia-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart lydia-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(lydia|gateway|cli\.py)\b', "kill lydia/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill lydia` but not
    # `kill -9 $(pgrep -f lydia)` because the substitution is opaque
    ...
    # the `lydia gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.lydia.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(lydia|ai\.lydia)\b', "stop/restart lydia launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Lydia file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.lydia/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Lydia-managed security file (~/.lydia/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_LYDIA_CONFIG_PATH}|{_LYDIA_ENV_PATH})', "in-place edit of Lydia config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_LYDIA_CONFIG_PATH}|{_LYDIA_ENV_PATH})', "in-place edit of Lydia config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_LYDIA_CONFIG_PATH}|{_LYDIA_ENV_PATH})', "in-place edit of Lydia config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.lydia/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / LYDIA_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Lydia home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\lydia), so folding the user home
    # first would eat the prefix the Lydia-home fold needs.
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / LYDIA_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.lydia``); the matched
...
def _rewrite_resolved_lydia_home(command: str) -> str:
    """Rewrite the resolved absolute Lydia home prefix to ``~/.lydia/``.

    Resolves the active ``LYDIA_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.lydia/`` so the static ``_LYDIA_CONFIG_PATH`` / ``_LYDIA_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/lydia/.lydia/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$LYDIA_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from lydia_constants import get_lydia_home
        home = get_lydia_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.lydia")
