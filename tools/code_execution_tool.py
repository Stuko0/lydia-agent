...
Lets the LLM write a Python script that calls Lydia native via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `lydia_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `lydia_tools.py` with file-based RPC stubs
...
# available on every platform Lydia itself runs on.
...
# are non-secret runtime-location flags (the same set lydia_cli treats as the
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Lydia-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Lydia-profile soft guard."""',
...
def generate_lydia_tools_module(enabled_tools: List[str],
    ...
    """
    Build the source code for the lydia_tools.py stub module.
...
"""Auto-generated Lydia native RPC stubs."""
...
"""Auto-generated Lydia native RPC stubs (file-based transport)."""
...
def _execute_remote(
    ...
    The script and the generated lydia_tools.py module are shipped to
    ...
    try:
        ...
        _ship_file_to_remote(env, f"{sandbox_dir}/lydia_tools.py", tools_src)
...
def execute_code(
    ...
    # --- Set up temp directory with lydia_tools.py and script.py ---
    ...
    try:
        # Write the auto-generated lydia_tools module.
        ...
        with open(os.path.join(tmpdir, "lydia_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        # Ensure the lydia-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from lydia_tools import ...`` resolves even
        ...
        # LYDIA_TIMEZONE is an internal Lydia setting and must not leak
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.lydia/.env')).
...
def _load_config() -> dict:
    ...
    ``lydia --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (lydia-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``lydia native`` (e.g. web is turned off),
    ...
    # temp-dir staging and lydia-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.lydia/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Lydia native programmatically. "
        ...
        f"Available via `from lydia_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into lydia_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from lydia_tools import {import_str}` "
