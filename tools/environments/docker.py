...
def _load_lydia_env_vars() -> dict[str, str]:
    """Load ~/.lydia/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Lydia profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale lydia-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=lydia-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Lydia process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=lydia-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a lydia process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Lydia
    process leaves its container behind permanently because there's no
    subsequent Lydia process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=lydia-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=lydia-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``LYDIA_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("LYDIA_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using LYDIA_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'lydia'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``lydia-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.lydia/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. lydia-agent:latest) already use /init as PID 1
        ...
        container_name = f"lydia-{uuid.uuid4().hex[:8]}"
        # Labels make lydia-created containers identifiable to:
        #   * the orphan reaper (`lydia-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`lydia-task-id`, `lydia-profile`)
        #   * operators running `docker ps --filter label=lydia-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Lydia profile is captured at
        ...
        label_args = [
            "--label", "lydia-agent=1",
            "--label", f"lydia-task-id={task_label}",
            "--label", f"lydia-profile={profile_name}",
        ]
        ...
        self._labels = {
            "lydia-agent": "1",
            "lydia-task-id": task_label,
            "lydia-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Lydia process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Lydia secret blocklist. Only implicit passthrough
        # keys are filtered.
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("lydia-task-id", "")
        profile_label = self._labels.get("lydia-profile", "")
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"lydia-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``lydia-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=lydia-agent=1",
                    "--filter", f"label=lydia-task-id={task_label}",
                    "--filter", f"label=lydia-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
        ...
        # container) but can happen if a previous Lydia process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Lydia exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Lydia process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Lydia startup. That covers the
        ...
        # outlives Lydia processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"lydia-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``lydia /quit``
