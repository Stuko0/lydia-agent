...
# Lydia-internal env vars that should NOT leak into terminal subprocesses.
_LYDIA_PROVIDER_ENV_FORCE_PREFIX = "_LYDIA_FORCE_"

# Lydia-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Lydia inference secret exactly
...
# project's dependencies into the Lydia venv path instead of the project's own
# ``.venv`` — silently clobbering the Lydia environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Lydia venv stays reachable via PATH (its bin dir is first), so stripping
...
def _inject_context_lydia_home(env: dict) -> None:
    """Bridge the context-local Lydia home override into subprocess env."""
    try:
        from lydia_constants import get_lydia_home_override

        value = get_lydia_home_override()
        if value:
            env["LYDIA_HOME"] = value
    except Exception:
        pass


def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Lydia-managed secrets from a subprocess environment."""
    try:
    ...
    from lydia_constants import apply_subprocess_home_env
...
# legitimate child Lydia spawns needs them, and they are the highest-value
...
def lydia_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Lydia spawns legitimately needs them.
...
def _find_bash() -> str:
    ...
    #   PortableGit: %LOCALAPPDATA%\lydia\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\lydia\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _lydia_portable_git = os.path.join(_local_appdata, "lydia", "git") if _local_appdata else ""
    ...
    raise RuntimeError(
        "Git Bash not found. Lydia Agent requires Git for Windows on Windows.\n"
...
# Cached directory containing the ``lydia`` console-script.
...
def _resolve_lydia_bin_dir() -> str | None:
    """Return the directory holding the ``lydia`` console-script, or None.
    ...
    lydia install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``lydia``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``lydia`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``lydia`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("lydia")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``lydia`` executable (covers nix-store / venv wrappers).
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "lydia" or base.startswith("lydia."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "lydia.exe" if _IS_WINDOWS else "lydia"
...
def _prepend_lydia_bin_dir(existing_path: str) -> str:
    """Prepend the lydia install dir to ``existing_path`` if it's missing.
...
def _make_run_env(env: dict) -> dict:
    ...
    if path_key is not None:
        ...
        # Ensure the lydia install dir is reachable so plugins can shell out
        # to bare ``lydia`` via the terminal tool even when the gateway was
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Lydia trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``LYDIA_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under LYDIA_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_lydia_home() / "cache" / "terminal"
            except Exception:
