...
def iter_sync_files(container_base: str = "/root/.lydia") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.lydia to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync_back(self, lydia_home: Path | None = None) -> None:
        ...
        Downloads the remote ``.lydia/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="lydia-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.lydia/skills/a.md`` →
        ``~/.lydia/skills/a.md``, a new remote file at
        ``/root/.lydia/skills/b.md`` maps to ``~/.lydia/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
