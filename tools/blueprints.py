...
`lydia skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.lydia.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.lydia.blueprint`` is a mapping containing
    ...
    lydia = meta.get("lydia") if isinstance(meta, dict) else None
    blueprint = lydia.get("blueprint") if isinstance(lydia, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.lydia.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.lydia.blueprint`` block) they can hand
    to ``lydia skills publish`` to share. ``body`` is the plain-language
