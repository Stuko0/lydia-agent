...
def _is_lydia_provider_credential(name: str) -> bool:
    """True if ``name`` is a Lydia-managed provider credential (API key,
    ...
    Non-Lydia API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Lydia credential into the execute_code child.
    """
    try:
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Lydia-managed provider credentials (from
    ``_LYDIA_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Lydia-managed
    ...
    Non-Lydia third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_lydia_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Lydia provider "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Lydia-managed provider credentials must not be passed
                ...
                if _is_lydia_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Lydia "
