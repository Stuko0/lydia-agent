...
selectable via ``lydia native`` → Image Generation; the active model is
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                ...
                f"  • Pick a different model via `lydia native` → Image Generation."
                f"{gateway_message}"
            ) from exc
        raise
    ...
    try:
        ...
        if source_images and not edit_endpoint:
            raise ValueError(
                ...
                f"via `lydia native` → Image Generation."
            )
        ...
        if "image_url" in kwargs or "reference_image_urls" in kwargs:
            ...
            return json.dumps({
                ...
                "error": (
                    ...
                    f"supports editing via `lydia native` → Image Generation."
                ),
...
# model/provider via `lydia native` or `/skills`.
