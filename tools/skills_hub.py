#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Lydia Skills Hub.
...
Used by lydia_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                lydia_meta = meta_block.get("lydia", {})
                if isinstance(lydia_meta, dict):
                    tags = lydia_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Lydia centralized index source
# ---------------------------------------------------------------------------

LYDIA_INDEX_URL = "https://lydia-agent.nousresearch.com/docs/api/skills-index.json"
LYDIA_INDEX_TTL = 6 * 3600  # 6 hours


def _lydia_index_cache_file() -> Path:
    return _index_cache_dir() / "lydia-index.json"
...
def _load_lydia_index() -> Optional[dict]:
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.debug("Lydia index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Lydia index fetch failed: %s", e)
        return _load_stale_index_cache()
...
class LydiaIndexSource(SkillSource):
    """Skill source backed by the centralized Lydia Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "lydia-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "lydia-index"),
            identifier=entry.get("identifier", ""),
