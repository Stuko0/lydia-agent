...
provider** — turn on a backend by enabling a plugin (``lydia plugins
enable video_gen/<name>``) and selecting it in ``lydia native`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/lydia-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `lydia native` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `lydia native` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `lydia native` → "
...
# `lydia native` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`lydia native` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `lydia native` → Video Generation."
        )
        return {"description": "\n".join(parts)}
