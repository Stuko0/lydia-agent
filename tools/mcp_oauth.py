    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Lydia Agent"
"""
...
from lydia_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__lydia_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses LYDIA_HOME so each profile gets its own OAuth tokens.
    Layout: ``LYDIA_HOME/mcp-tokens/``
    """
    try:
        from lydia_constants import get_lydia_home
        base = Path(get_lydia_home())
    except ImportError:
        base = Path(os.environ.get("LYDIA_HOME", str(Path.home() / ".lydia")))
...
class LydiaTokenStorage:
    ...
    File layout::
        ...
        LYDIA_HOME/mcp-tokens/<server_name>.json         -- tokens
        LYDIA_HOME/mcp-tokens/<server_name>.client.json   -- client info
        LYDIA_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Lydia records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Lydia.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://lydia-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `lydia mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Lydia Agent")
    scope = cfg.get("scope")
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`lydia mcp login {server_name}` interactively first to complete "
