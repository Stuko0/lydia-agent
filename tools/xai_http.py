...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``lydia native`` repaint, tool-registration scans,
    ...
    2. ``~/.lydia/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_lydia_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.lydia/.env`` first, then ``os.environ``.

    Wraps :func:`lydia_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from lydia_cli.config import get_env_value as _lydia_get_env_value
...
def lydia_xai_user_agent() -> str:
    """Return a stable Lydia-specific User-Agent for xAI HTTP calls."""
    try:
        from lydia_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Lydia-Agent/{__version__}"


def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Lydia config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Lydia home, then mark it seen."""
    ...
    try:
        ...
        marker_dir = get_lydia_home() / "state"
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    Prefers Lydia-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``lydia_cli.config.get_env_value`` so keys
    stored in ``~/.lydia/.env`` (the standard Lydia location) are honored —
