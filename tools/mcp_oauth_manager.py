...
`mcp_tool.py`, and `lydia_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# LydiaMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------
...
def _make_lydia_provider_class() -> Optional[type]:
    ...
    class LydiaMCPOAuthProvider(OAuthClientProvider):
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`LydiaTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, LydiaTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import LydiaTokenStorage
                        if isinstance(storage, LydiaTokenStorage):
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import LydiaTokenStorage
            if not isinstance(storage, LydiaTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``lydia mcp reauth``.
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`LydiaMCPOAuthProvider` directly using the helpers
        ...
        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `lydia mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``lydia mcp remove <name>`` and (indirectly) by
        ``lydia mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
