...
/**
 * In production the Python `lydia dashboard` server injects a one-shot
  ...
  return {
    ...
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[lydia] Could not find session token in ${BACKEND} — ` +
              `is \`lydia dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        console.warn(
          `[lydia] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`lydia dashboard\` or set LYDIA_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@lydia/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
  ...
  server: {
    proxy: {
      ...
      // Same host as `lydia dashboard` must serve these; Vite has no
