...
const SIDEBAR_COLLAPSED_KEY = "lydia-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="lydia-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="lydia-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'lydia update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "lydia update";
    return (
      t.status.updateLydiaConfirmMessage ??
      `This will run 'lydia update' (${cmd}) and restart the gateway when it finishes.`
    );
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Lydia gateway process. Connected channels and active sessions will reconnect afterward."
      }
