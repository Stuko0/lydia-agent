 ...
 *
 * Plugins call window.__LYDIA_PLUGINS__.register(name, Component)
 ...
 * Exposed at runtime as ``window.__LYDIA_PLUGIN_SDK__.sdkVersion`` so a
...
export function exposePluginSDK() {
  window.__LYDIA_PLUGINS__ = {
    register: registerPlugin,
    registerSlot,
  };

  window.__LYDIA_PLUGIN_SDK__ = {
    ...
    // window.__LYDIA_SESSION_TOKEN__ directly.
    authedFetch,
    ...
    // UI components — Stuko DS where available, shadcn/ui primitives elsewhere.
    components: {
      Card,
      CardHeader,
      CardTitle,
