...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      lydia-agent = self'.packages.default;
      lydiaVenv = lydia-agent.lydiaVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "lydia-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "lydia-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "lydia-build-package" { } ''
          echo "PASS: package built at ${lydia-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "lydia-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "lydia-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${lydia-agent}/bin/lydia || (echo "FAIL: lydia binary missing"; exit 1)
          test -x ${lydia-agent}/bin/lydia-agent || (echo "FAIL: lydia-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${lydia-agent}/bin/lydia version 2>&1 | grep -qi "lydia" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "lydia-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in lydia lydia-agent lydia-acp; do
            test -x ${lydia-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "lydia-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking lydia --help ==="
          ${lydia-agent}/bin/lydia --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${lydia-agent}/bin/lydia --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "lydia-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${lydia-agent}/share/lydia-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${lydia-agent}/share/lydia-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "LYDIA_BUNDLED_SKILLS" ${lydia-agent}/bin/lydia || \
        ...
        bundled-plugins = pkgs.runCommand "lydia-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${lydia-agent}/share/lydia-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${lydia-agent}/share/lydia-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "LYDIA_BUNDLED_PLUGINS" ${lydia-agent}/bin/lydia || \
        ...
        bundled-locales = pkgs.runCommand "lydia-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${lydia-agent}/share/lydia-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${lydia-agent}/share/lydia-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${lydia-agent}/share/lydia-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "LYDIA_BUNDLED_LOCALES" ${lydia-agent}/bin/lydia || \
          ...
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && LYDIA_BUNDLED_LOCALES=${lydia-agent}/share/lydia-agent/locales \
        ...
        bundled-tui = pkgs.runCommand "lydia-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${lydia-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${lydia-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "LYDIA_TUI_DIR" ${lydia-agent}/bin/lydia || \
        ...
        lydia-node = pkgs.runCommand "lydia-node-version" { } ''
          set -e
          echo "=== Checking LYDIA_NODE in wrapper ==="
          grep -q "LYDIA_NODE" ${lydia-agent}/bin/lydia || \
            (echo "FAIL: LYDIA_NODE not set in wrapper"; exit 1)
          echo "PASS: LYDIA_NODE present in wrapper"

          LYDIA_NODE=$(sed -n "s/^export LYDIA_NODE='\(.*\)'/\1/p" ${lydia-agent}/bin/lydia)
        ...
        managed-guard = pkgs.runCommand "lydia-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked "config set" ${lydia-agent}/bin/lydia config set model foo
          check_blocked "config edit" ${lydia-agent}/bin/lydia config edit
        ...
        extra-python-packages = let
          ...
          lydiaWithExtra = lydia-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "lydia-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${lydiaWithExtra}/bin/lydia || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${lydiaWithExtra}/bin/lydia || \
          ...
          if grep -q "PYTHONPATH" ${lydia-agent}/bin/lydia; then
        ...
        extra-dependency-groups = let
          lydiaWithGroups = lydia-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "lydia-extra-dependency-groups" { } ''
          set -e
        ...
        messaging-variant = pkgs.runCommand "lydia-messaging-variant" { } ''
          set -e
        ...
        in pkgs.runCommand "lydia-config-roundtrip" {
