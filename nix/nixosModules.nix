# nix/nixosModules.nix — NixOS module for lydia-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: lydia runs from /nix/store bind-mounted read-only into a
...
# and read by lydia at startup — no container recreation needed for env changes.
#
# Tool resolution: the lydia wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.lydia-agent;
    effectivePackage =
    ...
    lydia-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-lydia-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "lydia-config-attrs";
      description = "Lydia YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "lydia-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "lydia-documents" { } (
      ''
        mkdir -p $out
    ...
    # lydia user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "lydia-container-entrypoint" ''
      set -eu
      ...
      if [ ! -f /var/lib/lydia-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/lydia-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/lydia ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/lydia
        chmod 0440 /etc/sudoers.d/lydia
      fi
    ...
    # Default: /var/lib/lydia/workspace → /data/workspace.
  ...
  in {
    options.services.lydia-agent = with lib; {
      enable = mkEnableOption "Lydia Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = lydia-agent;
        description = "The lydia-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/lydia";
        description = "State directory. Contains .lydia/ subdir (LYDIA_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Lydia config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Lydia reads this file on every startup via load_lydia_dotenv().
        '';
      };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `lydia gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the lydia user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the lydia plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Lydia discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          with the same Python interpreter as lydia (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in lydia-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the lydia CLI to environment.systemPackages and export
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.lydia symlink to the service
            stateDir. These users are automatically added to the lydia group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.lydia-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the lydia CLI to system PATH and export LYDIA_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.lydia/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.LYDIA_HOME = "${cfg.stateDir}/.lydia";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.lydia-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the lydia user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.lydia-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed lydia
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure lydia is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.lydia        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.lydia/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.lydia/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.lydia/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.lydia/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.lydia/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."lydia-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.lydia
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.lydia ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.lydia ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.lydia -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.lydia/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.lydia/$_subdir"
            chmod 2770 "${cfg.stateDir}/.lydia/$_subdir"
            find "${cfg.stateDir}/.lydia/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # lydia-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.lydia/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.lydia/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.lydia/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.lydia/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.lydia/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.lydia/.managed
          chmod 0644 ${cfg.stateDir}/.lydia/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.lydia/.container-mode <<'LYDIA_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    lydia_bin=${containerDataDir}/current-package/bin/lydia
    LYDIA_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.lydia/.container-mode
            chmod 0644 ${cfg.stateDir}/.lydia/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.lydia/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.lydia";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.lydia" ]; then
                  rm -f "${symlinkPath}"
                  echo "lydia-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.lydia -> stateDir/.lydia for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.lydia";
                target = "${cfg.stateDir}/.lydia";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "lydia-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.lydia/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.lydia/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.lydia/auth.json
              fi
            ''}
          ''}
          ...
          # Lydia reads $LYDIA_HOME/.env at startup via load_lydia_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.lydia/.env"
    ...
    LYDIA_NIX_ENV_EOF
        ...
        find ${cfg.stateDir}/.lydia/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.lydia/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.lydia/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.lydia-agent = {
          description = "Lydia Agent Gateway";
          ...
          serviceConfig = {
            ...
            # so interactive users in the lydia group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.lydia-agent = {
          description = "Lydia Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ${containerBin} create \
                ...
                --env LYDIA_HOME=${containerDataDir}/.lydia \
                --env LYDIA_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/lydia gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
