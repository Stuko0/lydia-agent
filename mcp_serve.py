"""
Lydia MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Lydia-specific extra)

Usage:
    lydia mcp serve
    lydia mcp serve --verbose
...
logger = logging.getLogger("lydia.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using LYDIA_HOME."""
    try:
        from lydia_constants import get_lydia_home
        return get_lydia_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("LYDIA_HOME", Path.home() / ".lydia")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from lydia_constants import get_lydia_home
        directory_file = get_lydia_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("LYDIA_HOME", Path.home() / ".lydia")
...
class EventBridge:
    ...
    This is the Lydia equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from lydia_constants import get_lydia_home
            db_file = get_lydia_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("LYDIA_HOME", Path.home() / ".lydia")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Lydia MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "lydia",
        instructions=(
            "Lydia Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Lydia MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
