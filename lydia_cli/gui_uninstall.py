"""
Lydia Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``lydia desktop`` / ``lydia gui``)
     Built inside the agent checkout under ``$LYDIA_HOME/lydia-agent/``:
       ...
       - ``$LYDIA_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Lydia.app`` or ``~/Applications/Lydia.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Lydia`` (NSIS per-user)
...
the app name ("Lydia"), separate from ``$LYDIA_HOME``:
  - macOS:   ``~/Library/Application Support/Lydia``
  - Windows: ``%APPDATA%\\Lydia``
  - Linux:   ``$XDG_CONFIG_HOME/Lydia`` (default ``~/.config/Lydia``)
...
``lydia uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(lydia_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return lydia_home / "lydia-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Lydia"
...
def source_built_gui_artifacts(lydia_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``lydia desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``lydia-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(lydia_home)
    ...
    return [
        ...
        lydia_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Lydia" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        ...
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Lydia).
            local_base / "Programs" / "Lydia",
            # Older / alternate layout some builds used.
            local_base / "lydia-desktop",
        ]
...
def agent_is_installed(lydia_home: Path) -> bool:
    """Return True when a usable Python agent install exists under LYDIA_HOME.
    ...
    if (agent_root / "lydia_cli").is_dir():
        return True
...
def gui_is_installed(lydia_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(lydia_home):
        if p.exists():
            return True
...
def gui_install_summary(lydia_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = lydia_home if lydia_home is not None else get_lydia_home()
...
def uninstall_gui(lydia_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``lydia-agent/lydia_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$LYDIA_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove lydia' or "
            "'sudo dnf remove lydia'). AppImage builds are a single file you "
