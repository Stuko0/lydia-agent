"""`lydia checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.lydia/checkpoints/``.  Actions:

    lydia checkpoints               # same as `status`
    lydia checkpoints status        # total size, project count, breakdown
    lydia checkpoints list          # per-project checkpoint counts + workdir
    lydia checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    lydia checkpoints clear [-f]    # nuke the entire base (asks first)
    lydia checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    lydia checkpoints
    lydia checkpoints prune --retention-days 3 --max-size-mb 200
    lydia checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: lydia checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``lydia checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `lydia checkpoints` → status
