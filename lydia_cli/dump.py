"""
Dump command for lydia CLI.

Outputs a compact, plain-text summary of the user's Lydia setup
...
from lydia_cli.config import get_lydia_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    ``LYDIA_GIT_SHA`` build-arg (see ``lydia_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``lydia dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from lydia_cli.gateway import get_gateway_runtime_snapshot
...
def run_dump(args):
    ...
    lines.append(f"lydia_home:      {display_lydia_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `lydia auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `lydia auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["lydia-cli"])
