"""
Gateway subcommand for lydia CLI.

Handles: lydia gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_lydia_home is imported lazily at call sites to avoid ImportError
# when lydia_constants is cached from a pre-update version during `lydia update`.
...
from lydia_cli.colors import Colors, color
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "lydia CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``lydia gateway status`` from falsely counting the ``lydia`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``lydia gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_lydia_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # the command doesn't advertise *some other* profile. LYDIA_HOME
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``lydia update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Lydia profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Lydia profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from lydia_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "lydia_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m lydia_cli.main gateway
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose LYDIA_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``lydia update`` users
    # with no running gateway until they re-invoke ``lydia gateway``
    ...
    from lydia_cli._subprocess_compat import (
    ...
    if sys.platform == "win32":
        try:
            from lydia_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / LYDIA_HOME) the windowless
        # base interpreter needs to import lydia_cli.  Empty on POSIX, where
...
def _sync_lydia_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``LYDIA_HOME``.

    Under ``sudo``, ``LYDIA_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_lydia_home` falls back to ``/root/.lydia`` — the wrong
    profile. The unit file pins ``LYDIA_HOME`` for the actual gateway
    ...
    unit_home = env.get("LYDIA_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("LYDIA_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["LYDIA_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}lydia gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}lydia gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from lydia_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: lydia gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `lydia gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``lydia gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from lydia_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans() -> bool:
    ...
    in-process under a ``gateway restart`` argv (lydia_cli/gateway.py restart
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (LYDIA_HOME-scoped).
...
from lydia_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Lydia Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``lydia gateway run`` must remain interruptible from
...
_SERVICE_BASE = "lydia-gateway"
SERVICE_DESCRIPTION = "Lydia Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current LYDIA_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (LYDIA_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from lydia_constants import get_default_lydia_root

    home = get_lydia_home().resolve()
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary LYDIA_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(lydia_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when LYDIA_HOME is a named profile.

    For ``~/.lydia/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        lydia_home: Optional explicit LYDIA_HOME path. Defaults to the current
            ``get_lydia_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional Lydia root to compare against. Used when
    ...
    home = Path(lydia_home or str(get_lydia_home())).resolve()
...
def _profile_arg_for_target_user(lydia_home: str, target_home_dir: str) -> str:
    ...
    target_root = Path(target_home_dir) / ".lydia"
    try:
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this LYDIA_HOME.

    Default ``~/.lydia`` returns ``lydia-gateway`` (backward compatible).
    Profile ``~/.lydia/profiles/coder`` returns ``lydia-gateway-coder``.
    Any other LYDIA_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Lydia installs that predate the
# lydia-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (lydia-gateway-*.service) and unrelated third-party
# "lydia" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("lydia.service",)

# ExecStart content markers that identify a unit as running our gateway.
# A legacy unit is only flagged when its file contains one of these.
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "lydia_cli.main gateway",
    "lydia_cli/main.py gateway",
    "gateway/run.py",
    " lydia gateway ",
    "/lydia gateway ",
)
...
def _find_legacy_lydia_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Lydia gateway units.

    Detects unit files installed by older Lydia versions that used a
    different service name (e.g. ``lydia.service`` before the rename to
    ``lydia-gateway.service``). When both a legacy unit and the current
    ``lydia-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``lydia-gateway-coder.service`` and unrelated third-party
      ``lydia-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``lydia.service`` running an unrelated
...
def has_legacy_lydia_units() -> bool:
    """Return True when any legacy Lydia gateway unit files exist."""
    return bool(_find_legacy_lydia_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Lydia gateway unit files if any are installed.
    ...
    if not legacy:
        return
    print_warning("Legacy Lydia gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current lydia-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    lydia gateway migrate-legacy")
...
def remove_legacy_lydia_units(
    ...
    """Stop, disable, and remove legacy Lydia gateway unit files.
    ...
    legacy = _find_legacy_lydia_units()
    if not legacy:
        print("No legacy Lydia gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Lydia gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: lydia gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo lydia gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    lydia gateway uninstall")
    print_info("    sudo lydia gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: lydia gateway install"
            )
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Lydia often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.lydia`` → ``ai.lydia.gateway.plist`` (backward compatible).
    Profile ``~/.lydia/profiles/coder`` → ``ai.lydia.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.lydia.gateway-{suffix}" if suffix else "ai.lydia.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.lydia/lydia-agent  -> /home/alice/.lydia/lydia-agent
      /opt/lydia                 -> /opt/lydia  (kept as-is)
...
def _lydia_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current LYDIA_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_lydia_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.lydia                    → /home/alice/.lydia
      /root/.lydia/profiles/coder     → /home/alice/.lydia/profiles/coder
      /opt/custom-lydia               → /opt/custom-lydia  (kept as-is)
    """
    current_lydia = get_lydia_home().resolve()
    current_default = (Path.home() / ".lydia").resolve()
    target_default = Path(target_home_dir) / ".lydia"

    # Default ~/.lydia → remap to target user's default
    if current_lydia == current_default:
        return str(target_default)

    # Profile or subdir of ~/.lydia → preserve the relative structure
    try:
        relative = current_lydia.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.lydia) — keep as-is
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m lydia_cli.main``, so module
    ...
    dir, or a clone that ``lydia update`` later relocates/removes — the path
    ...
    ``LYDIA_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if LYDIA_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        # Anchor cwd to the target user's LYDIA_HOME (stable, always exists)
...
User={username}
Group={group_name}
ExecStart={python_path} -m lydia_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="LYDIA_HOME={lydia_home}"
Restart=always
RestartSec=5
...
"""
    ...
    lydia_home = str(get_lydia_home().resolve())
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir LYDIA_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved LYDIA_HOME
    ...
    exported a throwaway ``LYDIA_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``lydia gateway restart``
    with ``LYDIA_HOME=/tmp/lydia-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="LYDIA_HOME=..."`` lines and launchd
    ``<key>LYDIA_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'LYDIA_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>LYDIA_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp LYDIA_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: LYDIA_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported LYDIA_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only LYDIA_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp LYDIA_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../lydia_test`` path into
    # ``Environment="LYDIA_HOME=..."``. Writing that to the developer's
    ...
    # LYDIA_HOME into the unit (manual E2E homes like /tmp/lydia-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Lydia install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo lydia gateway uninstall --system")
    print_info("         lydia gateway install")
    print_info("         lydia gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (lydia.service from pre-rename installs)
    # before installing the new lydia-gateway.service. If both remain, they
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}lydia gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}lydia gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}lydia gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `lydia gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `lydia gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}lydia gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}lydia gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}lydia gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}lydia gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.lydia.gateway-{suffix}" if suffix else "ai.lydia.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``lydia gateway start/stop/status`` call).
...
# (the documented `nohup lydia gateway run` workaround). See #23387.
...
def _launchd_unsupported_marker_path() -> Path:
    return get_lydia_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m lydia_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active LYDIA_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "lydia_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup lydia gateway run --replace`
    ...
    """
    from lydia_cli._subprocess_compat import windows_detach_popen_kwargs

    log_dir = get_lydia_home() / "logs"
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup lydia gateway run --replace "
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Lydia install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  lydia gateway status             # Check status")
    from lydia_constants import display_lydia_home as _dhh
...
def launchd_stop():
    ...
    # `lydia gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    # Lydia PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Lydia PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Lydia install")
    else:
        print("⚠ Service definition is stale relative to the current Lydia install")
        print("  Run: lydia gateway start")
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: lydia gateway stop")
            else:
    ...
    if deep:
        log_file = get_lydia_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``lydia gateway run`` is always a misconfiguration, so we
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``lydia gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``lydia gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other LYDIA_HOME roots.
    """
    ...
    print("  Use 'lydia gateway restart' to replace it,")
    print("  or 'lydia gateway stop' first.")
    print("  Or use 'lydia gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print_error(
        "Refusing to run the Lydia gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'lydia' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/lydia/docker/entrypoint.sh before the Lydia command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$LYDIA_HOME and break later non-root dashboard/gateway runs."
    )
    print(
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `lydia gateway run` still
    ...
    # /restart) rather than through `lydia gateway restart` which already
    ...
    # `lydia gateway start/restart` — leaving the gateway vulnerable to
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("LYDIA_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from lydia_constants import get_lydia_home as _ghh
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. lydia) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where Lydia delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Lydia connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: lydia pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Lydia will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``lydia setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run lydia under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.lydia/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_lydia_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from lydia_cli.telegram_managed_bot import (
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'lydia pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: lydia pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'lydia gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Lydia will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Lydia will store the returned account_id/token in ~/.lydia/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `lydia gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_lydia_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `lydia pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `lydia pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "LydiaAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``lydia_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from lydia_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.lydia/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.lydia/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: lydia gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: lydia gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: lydia gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo lydia gateway install --system"
                        )
                    print_info("  Or run in foreground:  lydia gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: lydia gateway run")
                print_info(
                    "  For persistence:   tmux new -s lydia 'lydia gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: lydia gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup lydia gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'lydia gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`lydia_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from lydia_cli.service_manager import (
    ...
    if profile is None:
        ...
        # LYDIA_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``lydia gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``lydia gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    # `docker logs` and ${LYDIA_HOME}/logs/gateways/<profile>/current,
    ...
    #
    # Prefer `sleep infinity` (matches the static main-lydia service's
    # pattern in docker/s6-rc.d/main-lydia/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: lydia gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: lydia gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s lydia 'lydia gateway run'"
                )
                print()
            ...
            print()
            print(
                "  lydia gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s lydia 'lydia gateway run'         # persistent via tmux"
            )
            print(
                "  nohup lydia gateway run > ~/.lydia/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from lydia_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
                print("Per-profile gateways are auto-registered when you create a profile.")
                print()
                print("  lydia profile create <name>     # creates the s6 service slot")
                print("  lydia -p <name> gateway start   # bring it up via s6")
                print("  lydia status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: lydia gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: lydia gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: lydia gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  lydia profile delete <name>     # tears down the s6 service slot")
                print("  lydia -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `lydia -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: lydia gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_LYDIA_GATEWAY") == "1":
            print_error(
                ...
                "Use `lydia gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_LYDIA_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `lydia gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: lydia gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  lydia gateway install")
                else:
                    print("To install as a service:")
                    print("  lydia gateway install")
                    print("  sudo lydia gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  lydia gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup lydia gateway run > ~/.lydia/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s lydia 'lydia gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  lydia gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  lydia gateway install  # Install as user service")
                    print(
                        "  sudo lydia gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Lydia gateway unit files from
        # pre-rename installs (e.g. lydia.service). Profile units and
        # unrelated third-party services are never touched.
