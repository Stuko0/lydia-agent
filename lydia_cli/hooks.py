"""lydia hooks — inspect and manage shell-script hooks.

Usage::

    lydia hooks list
    lydia hooks test <event> [--for-tool X] [--payload-file F]
    lydia hooks revoke <command>
    lydia hooks doctor

Consent records live under ``~/.lydia/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.lydia/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``lydia hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: lydia hooks {list|test|revoke|doctor}")
        print("Run 'lydia hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.lydia/config.yaml.")
        print("See `lydia hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `lydia hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `lydia hooks test` and `lydia hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Lydia wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `lydia hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `lydia hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `lydia hooks doctor`.")
