"""MCP picker — interactive `lydia mcp picker` (also the default `lydia mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``lydia mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `lydia plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from lydia_cli.cli_output import prompt_yes_no
from lydia_cli.curses_ui import curses_single_select
from lydia_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Lydia session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from lydia_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `lydia mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: lydia mcp install <name>    Picker: lydia mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Lydia is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Lydia — run `lydia update` "
...
def show_catalog() -> None:
    """`lydia mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`lydia mcp picker` (and default `lydia mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`lydia mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `lydia mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
