...
billing screens drive. Companion to ``lydia_cli/nous_account.py`` (which owns
...
- **Auth** = the OAuth bearer JWT Lydia already holds for inference
...
# lydia_cli.auth.NOUS_BILLING_MANAGE_SCOPE (kept here too so this module has no
...
def _billing_not_logged_in(exc: Optional[BaseException] = None) -> "BillingAuthError":
    ...
    err = BillingAuthError(
        "Not logged into Nous Portal — run `lydia portal` to log in.",
        status=401,
        error="invalid_token",
    )
    if exc is not None:
