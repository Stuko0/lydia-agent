...
Profile-aware location: ``$LYDIA_HOME/logs/dashboard-auth.log``.
...
from ``lydia_constants`` or other lydia_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$LYDIA_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``lydia_constants.get_lydia_home`` semantics: env var wins,
    else ``~/.lydia``. A local copy avoids an import cycle with the
    middleware which lives below ``lydia_cli``.
    """
    home = os.environ.get("LYDIA_HOME") or str(Path.home() / ".lydia")
