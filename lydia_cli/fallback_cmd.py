"""
lydia fallback — manage the fallback provider chain.
...
Subcommands:
  lydia fallback [list]   Show the current fallback chain (default when no subcommand)
  lydia fallback add      Pick provider + model via the same picker as `lydia model`,
                           then append the selection to the chain
  lydia fallback remove   Pick an entry to delete from the chain
  lydia fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.lydia/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from lydia_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `lydia model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  lydia fallback add")
        print()
        return
    ...
    print("  Docs: https://lydia-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `lydia model`, then append the selection to the chain."""
    from lydia_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `lydia model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `lydia fallback list` to view, or `lydia fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``lydia fallback [subcommand]``."""
