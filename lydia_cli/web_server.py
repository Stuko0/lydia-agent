"""
Lydia Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m lydia_cli.main web          # Start on http://127.0.0.1:9119
    python -m lydia_cli.main web --port 8080
"""
...
from lydia_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
from lydia_cli.memory_providers import (
    MemoryProvider,
    ProviderField,
    get_memory_provider,
)
...
try:
    ...
    # running `lydia dashboard` needs fastapi+uvicorn; lazy install keeps
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``lydia gateway run`` — but the
    desktop app spawns a ``lydia dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same LYDIA_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    try:
        import lydia_cli.gateway  # noqa: F401
    except Exception:
        pass
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from lydia_cli.gateway import _get_restart_drain_timeout
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire lydia_cli.gateway import into a background thread so the event
    ...
    # since the app has no gateway running the scheduler. Server `lydia
...
app = FastAPI(title="Lydia Agent", version=__version__, lifespan=_lifespan)

# Memory-provider OAuth connect routes live in the memory layer, not here.
from lydia_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# Backward compat: the desktop build (app.asar) still sends X-Lydia-*.
_LYDIA_LEGACY_HEADER = "X-Lydia-Session-Token"
...
#
# This list is defined in ``lydia_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from lydia_cli.dashboard_auth.public_paths import (
...
def _has_valid_session_token(request: Request) -> bool:
    ...
    # Legacy: desktop build (app.asar) still sends X-Lydia-Session-Token.
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Lydia-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``lydia-0day``
...
async def _dashboard_auth_gate(request: Request, call_next):
    from lydia_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from lydia_cli.dashboard_auth.token_auth import token_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Lydia (no terminal prompt), "
...
class ModelAssignment(BaseModel):
    ...
    # ``lydia model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Lydia provider slug — fine.
    ...
    1. Vendor-name → Lydia-provider mapping: when the provider string is
       not a known Lydia provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from lydia_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from lydia_cli.model_normalize import normalize_model_for_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from lydia_cli.models import _AGGREGATOR_PROVIDERS
...
def _default_lydia_root_is_opt_data() -> bool:
    raw = os.environ.get("LYDIA_HOME", "").strip()
    if not raw:
        return False
    try:
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``lydia update``.
    ...
    a container — e.g. the lydia-webui image sharing the Lydia source tree),
    the dashboard's ``lydia update`` button is the correct update path and
    ...
    if _default_lydia_root_is_opt_data():
        return True
    try:
        from lydia_constants import is_container
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Lydia root is actually /opt/data
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``lydia:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.lydia-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``lydia_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from lydia_cli import web_git as _web_git  # noqa: E402
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_lydia_home() at call time
    ...
    try:
        ...
        # Windows install the first import of lydia_cli.gateway blocks the
        ...
        # and which providers are registered so ``lydia status`` and the
        # SPA's StatusPage can show "OAuth gate ON via Nous Research" or
        # "loopback only — no auth gate" with no extra round trips.
        auth_required = bool(getattr(app.state, "auth_required", False))
        auth_providers: list[str] = []
        try:
            from lydia_cli.dashboard_auth import list_providers as _list_providers
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the lydia home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_lydia_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from lydia_cli.auth import get_nous_auth_status
    ...
    features = []
    try:
        from lydia_cli.nous_subscription import get_nous_subscription_features
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from lydia_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.lydia/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_lydia_home() / "logs"
...
def _spawn_lydia_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``lydia <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``lydia_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "lydia_cli.main", *subcommand]
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["lydia", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``lydia gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``lydia gateway restart`` children race each other on the
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``lydia gateway restart`` in the background."""
    try:
...
@app.post("/api/lydia/update")
async def update_lydia():
    """Kick off ``lydia update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Lydia updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("lydia-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "lydia-update",
    ...
    try:
        proc = _spawn_lydia_action(["update"], "lydia-update")
    except Exception as exc:
        _log.exception("Failed to spawn lydia update")
...
@app.get("/api/lydia/update/check")
async def check_lydia_update(force: bool = False):
    """Report whether a Lydia update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/lydia/update`` actually runs ``lydia update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Lydia version string
    ...
    try:
        ...
        if force:
            try:
                (get_lydia_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.lydia/config.yaml`` under ``tts.``.
    """
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Lydia surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Lydia supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _memory_provider_config_path(provider: MemoryProvider) -> Path:
    return get_lydia_home() / provider.name / "config.json"
...
# in lydia_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options(profile: Optional[str] = None, refresh: bool = False):
    ...
    try:
        ...
        # onboarding picker) sees the SAME full provider universe `lydia model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `lydia model` does so GUI onboarding lands on a
    ...
    if slug == "nous":
        try:
            ...
            from lydia_cli.auth import get_provider_auth_state
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from lydia_cli.moa_config import normalize_moa_config
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.lydia/config.yaml`` — applies to **new** sessions only.
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (lydia_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from lydia_cli.nous_subscription import apply_nous_managed_defaults
        ...
        # endpoint, mirroring the ``lydia model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from lydia_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``lydia model`` universe). This is what lets the desktop Keys tab
    ...
    """
    try:
        from lydia_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from lydia_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `lydia model` provider — would be
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `lydia model` picker uses (not desktop-only prefix guesses).
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Lydia from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Lydia to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Lydia from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Lydia to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Lydia in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Lydia through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Lydia via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Lydia through an IMAP/SMTP mailbox.",
        "docs_url": "https://lydia-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Lydia to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Lydia inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Lydia to Google Chat via Cloud Pub/Sub.",
        "docs_url": "https://lydia-agent.nousresearch.com/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://lydia-agent.nousresearch.com/docs/user-guide/messaging/weixin/",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use Lydia through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Lydia to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "docs_url": "https://lydia-agent.nousresearch.com/docs/user-guide/messaging/teams",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Lydia to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Lydia as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://lydia-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://lydia-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in lydia gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in lydia gateway setup",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.lydia-agent.nousresearch.com"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Lydia Agent").strip() or "Lydia Agent"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Lydia appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's LYDIA_HOME.
...
# the canonical ``lydia auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.lydia/.anthropic_oauth.json`` — Lydia-managed PKCE flow (what
    ...
    if lydia_creds and lydia_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "lydia_pkce",
            "source_label": f"Lydia PKCE ({_LYDIA_OAUTH_FILE})",
    ...
    try:
        ...
        pass
    try:
        from lydia_cli.config import get_env_value
    except ImportError:
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Lydia even
    when they also have a separate Lydia-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Lydia never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "lydia auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "lydia auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "lydia auth add minimax-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "lydia auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from lydia_cli import auth as hauth
        if provider_id == "nous":
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Lydia, so the disconnect
    ...
    user sees exactly what executes, and Lydia then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Lydia — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``lydia model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `lydia model` universe (zero-edit for new plugins).
    try:
        from lydia_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"lydia auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    sync with the `lydia model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Lydia-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            ...
            try:
                from lydia_cli.auth import clear_provider_auth
        ...
        try:
            from lydia_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.lydia/.anthropic_oauth.json AND credential pool
...
# Guarded so lydia web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Lydia file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``lydia auth add anthropic``.
    """
    from agent.anthropic_adapter import _LYDIA_OAUTH_FILE
    payload = {
    ...
    for _endpoint in _ANTHROPIC_OAUTH_TOKEN_URLS:
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "lydia-dashboard/1.0",
            },
            method="POST",
        )
        try:
...
# the tokens exactly like `lydia auth add xai-oauth`.
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `lydia auth add xai-oauth`'s credential-pool insert.
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from lydia_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from lydia_cli.auth import _save_codex_tokens
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `lydia sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `lydia sessions prune`)."""
    if body.older_than_days < 1:
        raise HTTPException(status_code=400, detail="older_than_days must be >= 1")
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_lydia_home()
    db = _open_session_db_for_profile(body.profile)
    try:
...
async def get_logs(
    ...
    from lydia_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_lydia_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from lydia_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, LYDIA_HOME)."""
    from lydia_cli import profiles as profiles_mod
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process LYDIA_HOME at import time. The dashboard is a single
...
#
# Wraps the same config data layer the CLI uses (lydia_cli.mcp_config), so
# servers managed here show up under `lydia mcp list` and vice versa.  Secrets
...
async def list_mcp_servers(profile: Optional[str] = None):
    from lydia_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from lydia_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from lydia_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from lydia_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    def _probe_scoped():
        ...
        # LYDIA_HOME override (see mcp_tool._wrap_with_home_override), so
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds LYDIA_HOME-derived paths in the child.
...
#
# Wraps the same JSON store the CLI uses (lydia_cli.webhook); the webhook
...
async def list_webhooks():
    import lydia_cli.webhook as wh
...
# spawn the real `lydia gateway <verb>` so behaviour matches the CLI exactly.
...
async def list_credential_pool():
    ...
    from lydia_cli.auth import read_credential_pool
...
# `lydia memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `lydia memory setup` to configure a new one.",
            )
...
def _dashboard_backup_dir() -> Path:
    return get_lydia_home() / "backups"


def _new_dashboard_backup_path() -> Path:
    stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
    return _dashboard_backup_dir() / f"lydia-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `lydia import`. The spawned action runs with
...
async def list_hooks():
    ...
    try:
        from lydia_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    cp_dir = get_lydia_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``lydia`` subprocess, and
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from lydia_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`lydia -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the LYDIA_HOME
...
def _profile_setup_command(name: str) -> str:
    ...
    return "lydia setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local LYDIA_HOME override so the write lands in the target
    ...
    """
    from lydia_constants import set_lydia_home_override, reset_lydia_home_override
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local LYDIA_HOME override (same mechanism as
    ...
    """
    from lydia_constants import set_lydia_home_override, reset_lydia_home_override
    from lydia_cli.mcp_security import validate_mcp_server_entry
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the LYDIA_HOME override. Returns the
    number of skills newly disabled.
    """
    from lydia_constants import set_lydia_home_override, reset_lydia_home_override
    from lydia_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's LYDIA_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``lydia profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from LYDIA_HOME).
    """
    from lydia_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``lydia profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the LYDIA_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``lydia profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from lydia_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_lydia_home()`` at call
    ...
    retargeted to the *current* ``get_lydia_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a LYDIA_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    which is all endpoints that resolve ``get_lydia_home()`` at call time
...
async def get_skills(profile: Optional[str] = None):
    ...
    from lydia_cli.skills_config import get_disabled_skills
    with _profile_scope(profile):
        config = load_config()
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``lydia tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``lydia tools`` picker shows
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``lydia tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.lydia/.env`` via ``save_env_value`` —
    the same store ``lydia tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``lydia tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``lydia -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same LYDIA_HOME the rest of the
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Lydia,
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``lydia computer-use permissions grant`` as a background action.
...
#
# The endpoint spawns the same ``lydia --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from lydia_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from lydia_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from lydia_cli.dashboard_auth.audit import AuditEvent, audit_log
        from lydia_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``lydia --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``LYDIA_TUI_RESUME`` env var —
    matching what ``lydia_cli.main._launch_tui`` does for the CLI path.
    ...
    ``LYDIA_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_lydia_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``lydia -p <name>`` performs. The in-process
    ...
    """
    from lydia_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    if profile_dir is not None:
        env["LYDIA_HOME"] = str(profile_dir)
    ...
    # inherits the profile LYDIA_HOME set above.
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from lydia_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="lydia-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Lydia inside WSL2 to use the dashboard's /chat "
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`lydia_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from lydia_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/lydia/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /lydia`` on every request. We
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/lydia``)
    ...
    # under ``/lydia`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Lydia backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Lydia Teal",         "description": "Classic dark teal — the canonical Lydia look"},
    {"name": "default-large", "label": "Lydia Teal (Large)", "description": "Lydia Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.lydia/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.lydia/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_lydia_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.lydia/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as lydia_cli.plugins):
    1. User plugins:    ~/.lydia/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.lydia/plugins/  (only if LYDIA_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from lydia_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_lydia_home() / "plugins", "user"),
    ...
    # ``lydia_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("LYDIA_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".lydia" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_lydia_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"lydia auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from lydia_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from lydia_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from lydia_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.lydia/plugins/``) ship with the CWD and are
...
from lydia_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    try:
        from lydia_cli.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    # the lydia-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # Each provider plugin that ships with Lydia Agent exposes a
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `lydia dashboard register` (Nous Portal) or "
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"  Lydia Web UI → http://{host}:{actual_port}")
