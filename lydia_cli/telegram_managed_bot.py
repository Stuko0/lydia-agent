...
manual BotFather token copy-paste. Lydia talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.lydia-agent.nousresearch.com"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``lydia_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``lydia_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"lydia_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Lydia Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Lydia Telegram onboarding service.")
