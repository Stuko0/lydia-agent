...
on Japanese locales, etc.).  Lydia's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``lydia_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Lydia is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``LYDIA_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("LYDIA_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Lydia-managed Git install directories so
    ...
    # already-running shells don't see the change — which means lydia
    ...
    # install without Lydia-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Lydia's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Lydia's
    ``lydia config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Lydia (User env var in Windows Settings, or
    export in a PowerShell profile) and Lydia picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Lydia-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\lydia\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``lydia`` from right after install) retain their old PATH.

    Any subprocess Lydia spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Lydia-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "lydia", "git", "cmd"),
        os.path.join(local_appdata, "lydia", "git", "bin"),
        os.path.join(local_appdata, "lydia", "git", "usr", "bin"),
        # Lydia venv Scripts directory — host of the lydia.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes lydia, but harmless to include.
        os.path.join(local_appdata, "lydia", "lydia-agent", "venv", "Scripts"),
