...
Used by ``lydia profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``lydia_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Lydia Agent kanban board.
...
Rules:
  ...
  - Never write "Lydia Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_lydia_home())
        else:
