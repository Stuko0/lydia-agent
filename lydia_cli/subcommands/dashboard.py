"""``lydia dashboard`` / ``lydia serve`` subcommand parsers.
...
``lydia_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `lydia dashboard` / `lydia serve` cmdlines and SIGTERM them directly —
    # the same path `lydia update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Lydia web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Lydia web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    # Backward-compat shim: older Lydia desktop app shells (<= 0.15.x) spawn the
    # backend as `lydia dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Lydia couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Lydia Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Lydia backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Lydia backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # `lydia dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.lydia/.env.
    # Nested subparser so bare `lydia dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "LYDIA_DASHBOARD_OAUTH_CLIENT_ID into ~/.lydia/.env, and prints "
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://lydia.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via LYDIA_DASHBOARD_PORTAL_URL. Mainly for "
