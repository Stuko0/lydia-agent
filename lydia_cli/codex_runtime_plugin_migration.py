"""Migrate Lydia' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Lydia' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Lydia mcp_servers.<n>.command/args/env  → codex stdio transport
  Lydia mcp_servers.<n>.url/headers       → codex streamable_http transport
  Lydia mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Lydia mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Lydia-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Lydia' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Lydia config.")
...
# Lydia keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Lydia' sampling subsection — codex MCP has no equivalent
    "sampling",
}
...
def _translate_one_server(
    ...
    """Translate one Lydia MCP server config to the codex inline-table dict
    ...
    if has_command:
        ...
        # Lydia' transport: sse hint is informational; codex auto-negotiates
    ...
    for key in lydia_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            ...
            skipped.append(f"{key} (unknown Lydia key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (LYDIA_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Lydia)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Lydia' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Lydia' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Lydia
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="lydia-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a LYDIA_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed lydia-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.lydia`` symlink that happens to live under ``/private/var/folders``
...
def _build_lydia_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Lydia' own
    ...
    so a lydia installed under /opt/, /usr/local/, or a venv all work.
    LYDIA_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # LYDIA_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_lydia_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever LYDIA_HOME its launcher sets
    ...
    # — that would override the launcher's LYDIA_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("LYDIA_HOME", tmp_path) would
    ...
    lydia_home = os.environ.get("LYDIA_HOME") or ""
    if lydia_home and _looks_like_test_tempdir(lydia_home):
        lydia_home = ""
    if lydia_home:
        env["LYDIA_HOME"] = lydia_home
    # PYTHONPATH passes through so a worktree-launched lydia finds the
...
def migrate(
    ...
    *,
    ...
    """Translate Lydia mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        lydia_config: full ~/.lydia/config.yaml dict
        ...
        expose_lydia_tools: when True (default), register Lydia' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Lydia for tools
    ...
    if not isinstance(lydia_servers, dict):
        report.errors.append(
            "mcp_servers in Lydia config is not a dict; cannot migrate."
        )
        return report
    ...
    # Inject Lydia' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Lydia for the tools codex
    ...
    if expose_lydia_tools:
        translated["lydia-tools"] = _build_lydia_tools_mcp_entry()
        if "lydia-tools" not in report.migrated:
            report.migrated.append("lydia-tools")
