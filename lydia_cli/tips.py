...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or lydia -c.",
    ...
    "/skin changes the CLI theme — try mono, slate, alice, or dragon.",
    ...
    "Ctrl+Z suspends Lydia to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "lydia -c resumes your most recent CLI session. lydia -c \"project name\" resumes by title.",
    "lydia -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "lydia -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    ...
    "lydia --yolo bypasses all dangerous command approval prompts for the entire session.",
    "lydia chat --source telegram tags the session for filtering in lydia sessions list.",
    "lydia -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "lydia doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "lydia dump outputs a compact setup summary — great for bug reports.",
    ...
    "lydia sessions browse opens an interactive session picker with search.",
    "lydia sessions stats shows session counts by platform and database size.",
    "lydia sessions prune --older-than 30 cleans up old sessions.",
    "lydia skills search react --source skills-sh searches the skills.sh public directory.",
    "lydia skills check scans installed hub skills for upstream updates.",
    "lydia skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "lydia skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "lydia mcp add github --command npx adds MCP servers from the command line.",
    "lydia mcp serve runs Lydia itself as an MCP server for other agents.",
    "lydia auth add lets you add multiple API keys for credential pool rotation.",
    "lydia completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "lydia logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "lydia backup creates a zip backup of your entire Lydia home directory.",
    "lydia profile create coder creates an isolated profile that becomes its own command.",
    "lydia profile create work --clone copies your current config and keys to a new profile.",
    "lydia update syncs new bundled skills to ALL profiles automatically.",
    "lydia gateway install sets up Lydia as a system service (systemd/launchd).",
    "lydia memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "lydia webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: lydia native disables unused tools, lydia skills config trims skills down.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as lydia -w).",
    ...
    "LYDIA_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    "Profile names become shell commands — 'lydia profile create coder' creates the 'coder' command.",
    "lydia profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Lydia prints a resume command with session ID and stats.",
    "lydia sessions export backup.jsonl exports all sessions for backup or analysis.",
    "lydia -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "lydia skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .lydia/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: lydia cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Lydia runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "lydia gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Lydia via a local macOS server.",
    ...
    "Context files (.lydia.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Lydia loads project context from .lydia.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "lydia mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "lydia mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "lydia mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "Checkpoints use shadow repos in ~/.lydia/checkpoints/ — your project's .git is never touched.",
    ...
    "lydia plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "lydia acp runs Lydia as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "LYDIA_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "lydia auth add nous or lydia auth add openai-codex sets up OAuth-based providers.",
    ...
    "lydia status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.lydia/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Lydia your own.",
    ...
    "Before a gateway session reset, Lydia auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.lydia/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Lydia state via LYDIA_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "LYDIA_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in LYDIA_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.lydia/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'lydia profile create ls' would be rejected.",
    "lydia profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "LYDIA_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via lydia native.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.lydia/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.lydia/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Lydia auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Lydia config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Lydia is working.',
    ...
    '/reload reloads ~/.lydia/.env into the running session — pick up new API keys without restarting.',
    ...
    "LYDIA_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.lydia/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.lydia/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'lydia curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "lydia curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'lydia curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'lydia auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'LYDIA_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "LYDIA_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'lydia dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'lydia dashboard embeds the full Lydia TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.lydia/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.lydia/dashboard-plugins/ — no npm build required.',
    ...
    'LYDIA_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'LYDIA_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'LYDIA_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'LYDIA_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.lydia/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.lydia/skills/ so drop-in skills appear without restarting the session.',
    ...
    'lydia -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    ...
    "lydia chat --source tool tags programmatic chats so they don't clutter lydia sessions list.",
    'lydia dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'lydia sessions rename <ID> "new title" renames any past session; lydia sessions delete <ID> removes one.',
    'lydia import restores a session export or profile archive produced by sessions export or profile export.',
    'lydia fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'lydia pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'lydia setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'lydia status --deep runs the full health sweep across every component; plain lydia status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'LYDIA_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'LYDIA_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.lydia/plugins/ — trust-gated by design.',
    "LYDIA_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'LYDIA_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'LYDIA_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'LYDIA_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'LYDIA_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'LYDIA_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'LYDIA_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'LYDIA_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'LYDIA_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'LYDIA_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'LYDIA_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Lydia block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from lydia sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.lydia/dashboard-plugins/.',
]
