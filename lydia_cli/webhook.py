"""lydia webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.lydia/webhook_subscriptions.json and are
...
def _setup_hint() -> str:
  ...
  Then start the gateway: lydia gateway run
"""
...
def webhook_command(args):
    """Entry point for 'lydia webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: lydia webhook {subscribe|list|remove|test}")
        print("Run 'lydia webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (lydia gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: lydia webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from lydia webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (lydia gateway run)")
