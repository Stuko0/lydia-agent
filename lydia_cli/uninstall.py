"""
Lydia Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.lydia/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Lydia PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing lydia-agent or lydia PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Lydia Agent" comment and following line
                if '# Lydia Agent' in line or '# lydia-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('lydia' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing lydia
                if 'lydia' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the lydia wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "lydia",
        Path("/usr/local/bin/lydia"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains lydia_cli reference)
                content = wrapper.read_text()
                if 'lydia_cli' in content or 'lydia-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(lydia_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``lydia`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Lydia home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (lydia_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``lydia gateway run`` processes
    ...
    try:
        from lydia_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from lydia_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``LYDIA_HOME`` and ``LYDIA_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\lydia\git\cmd``, ``%LOCALAPPDATA%\lydia\git\bin``,
#      ``%LOCALAPPDATA%\lydia\git\usr\bin``, ``%LOCALAPPDATA%\lydia\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\lydia\git\`` and Node to
#      ``%LOCALAPPDATA%\lydia\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``lydia dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\lydia\gateway-service\`` and sometimes
...
def _lydia_path_markers(lydia_home: Path) -> list[str]:
    """Path-entry substrings that identify Lydia-owned User-PATH entries."""
    root = str(lydia_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare lydia-agent install dir.
    markers = [root + "\\lydia-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if LYDIA_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "lydia".  We don't want to catch
    # unrelated entries like "clydia-foo" or "ephermeral", so we look for
    # backslash-lydia as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(lydia_home: Path) -> list[str]:
    """Strip Lydia-owned entries from User-scope PATH in the registry.
...
def remove_lydia_env_vars_windows() -> list[str]:
    """Delete LYDIA_HOME and LYDIA_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("LYDIA_HOME", "LYDIA_GIT_BASH_PATH"):
                try:
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its LYDIA_HOME directory.

    We shell out to ``lydia -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current LYDIA_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m lydia_cli.main` so we don't depend on a `lydia`
    #    wrapper that may be half-removed mid-uninstall.
    lydia_invocation = [_sys.executable, "-m", "lydia_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                lydia_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's LYDIA_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``lydia uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$LYDIA_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from lydia_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(lydia_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ✦ Lydia Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Lydia Chat GUI installation was found.")
        print(f"  Checked: {lydia_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Lydia Agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(lydia_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Lydia Agent at {lydia_home / 'lydia-agent'}")
        print(f"  • Your config, sessions, and secrets under {lydia_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Lydia Agent is still installed. Run 'lydia' to use the CLI,")
    print("or 'lydia uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.lydia/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.lydia/ for future reinstall
    """
    ...
    # full wipe (code + ~/.lydia data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ✦ Lydia Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {lydia_home / 'config.yaml'}")
    print(f"  Secrets: {lydia_home / '.env'}")
    print(f"  Data:    {lydia_home / 'cron/'}, {lydia_home / 'sessions/'}, {lydia_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their LYDIA_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Lydia data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Lydia code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``lydia`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$LYDIA_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in lydia_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\lydia\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(lydia_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Lydia-owned PATH entries in User environment")

        log_info("Removing LYDIA_HOME / LYDIA_GIT_BASH_PATH User env vars...")
        removed_env = remove_lydia_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Lydia-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing lydia command...")
    ...
    #     (only when they still point into this Lydia home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Lydia-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(lydia_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Lydia-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(lydia_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE LYDIA_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from lydia_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(lydia_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.lydia/, just remove the lydia-agent subdir
            if lydia_home in project_root.parents or project_root.parent == lydia_home:
    ...
    #     under LYDIA_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(lydia_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(lydia_home)
    ...
    # 5. Optionally remove ~/.lydia/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile LYDIA_HOME dirs live under
        ...
        try:
            if lydia_home.exists():
                shutil.rmtree(lydia_home)
                log_success(f"Removed {lydia_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {lydia_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {lydia_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://lydia-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://lydia-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Lydia Agent! ✦")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m lydia_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import lydia_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``lydia_constants`` + ``lydia_cli.colors``
    (and lazily ``lydia_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m lydia_cli.uninstall")
    parser.add_argument(
        "--mode",
