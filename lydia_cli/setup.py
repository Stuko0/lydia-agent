"""
Interactive setup wizard for Lydia Agent.
...
_DOCS_BASE = "https://lydia-agent.nousresearch.com/docs"
...
# display_lydia_home imported lazily at call sites (stale-module safety during lydia update)
...
from lydia_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from lydia_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print_info("Configure Lydia using environment variables or config commands:")
    print_info("  lydia config set model.provider custom")
    print_info("  lydia config set model.base_url http://localhost:8080/v1")
    print_info("  lydia config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'lydia setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``LYDIA_NONINTERACTIVE=1`` (see ``lydia_cli/web_server.py``). In that
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from lydia_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'lydia setup')")
...
def _print_setup_summary(config: dict, lydia_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'lydia setup' to configure"))
    ...
    # Video generation — opt-in via `lydia native` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from lydia_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'lydia setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'lydia setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'lydia setup terminal'"))
    ...
    # Spotify (OAuth via lydia auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'lydia setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('lydia setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('lydia setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('lydia setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('lydia setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('lydia setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('lydia config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('lydia config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('lydia config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('lydia', Colors.GREEN)}              Start chatting")
    print(f"   {color('lydia gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('lydia doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `lydia native` and `lydia setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``lydia model``)
    ...
    provider added to ``lydia model`` is automatically available here.
    ...
    # Delegate to the shared lydia model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from lydia_cli.main import select_provider_and_model
    try:
    ...
    # on demand via `lydia auth add`, `lydia setup` vision, and
    # `lydia setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``lydia model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from lydia_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.lydia/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Lydia auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via lydia setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'lydia setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Lydia runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `lydia setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `lydia setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `lydia setup agent` later to customize.")
...
def _setup_telegram_auto_result():
    ...
    try:
        from lydia_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_lydia_home(Path(get_lydia_home()))
    except Exception:
        pass
...
def _profile_name_from_lydia_home(lydia_home) -> str | None:
    """Return the active profile name when LYDIA_HOME is a profile dir."""
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Lydia delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Lydia to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from lydia_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://lydia-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://lydia-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  lydia config edit")
    print_info("   Open config in your editor:  lydia config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from lydia_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Lydia from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'lydia setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     lydia config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_lydia_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: lydia gateway install --system")
                print_info("  Or run in foreground:  lydia gateway")
        else:
            from lydia_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   lydia gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   lydia gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `lydia setup tools` and `lydia native` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``lydia_cli.auth`` — lists every supported
    ...
    """
    try:
        from lydia_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from lydia_cli.config
    ...
    if section_key == "model":
        ...
        from lydia_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_lydia.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_lydia migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_lydia", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Lydia.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Lydia to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Lydia at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Lydia at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Lydia at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Lydia at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Lydia equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Lydia config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Lydia.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Lydia's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(lydia_home: Path) -> bool:
    ...
    print_info("Lydia can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: lydia claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: lydia claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Lydia configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Lydia config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Lydia (use lydia claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``lydia setup --portal`` and ``lydia portal``. This is the
    ...
    Lydia session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``lydia setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``lydia model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``lydia portal`` always offers a model picker,
    ...
    # setup calls, so `lydia portal` == quick setup's Nous step.
    try:
        ...
        print_info("  You can retry later with `lydia portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `lydia portal`: %s", exc)
        print()
    ...
    print_info("  Run `lydia portal info` to inspect routing.")
    print_info("  Run `lydia` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      lydia setup           — full or quick (auto-detected)
      lydia setup model     — just model/provider
      lydia setup tts       — just text-to-speech
      lydia setup terminal  — just terminal backend
      lydia setup gateway   — just messaging platforms
      lydia setup tools     — just tool configuration
      lydia setup agent     — just agent settings
    """
    from lydia_cli.config import is_managed, managed_error
    if is_managed():
    ...
    print(
        color(
            "│  Let's configure your Lydia Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Lydia configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'lydia setup model|terminal|")
    ...
    print()
    print_info("You can edit these files directly or use 'lydia config edit'")
    ...
    # Tune later with `lydia setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, lydia_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'lydia setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    lydia setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  lydia setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``lydia native`` (which
    ...
    try:
        ...
        from lydia_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``lydia setup agent`` /
    ``lydia config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, lydia_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `lydia update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        lydia native")
        print_info("  Seed skills:         lydia skills opt-in --sync")
        print_info("  Add MCP servers:     lydia mcp add")
        print_info("  Enable plugins:      lydia plugins")
        print_info("  Tune agent settings: lydia setup agent")
        print()
        _print_setup_summary(config, lydia_home)
        return
...
def _blank_slate_walkthrough(config: dict, lydia_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`lydia update` runs from re-injecting them. Opt back in any")
            print_info("time with `lydia skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `lydia native`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `lydia plugins list` / `lydia plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `lydia plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `lydia mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `lydia mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   lydia native")
    print_info("  Seed skills:         lydia skills opt-in --sync")
    print_info("  Add MCP servers:     lydia mcp add")
    print_info("  Tune agent settings: lydia setup agent")
    print()
...
def _run_quick_setup(config: dict, lydia_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'lydia setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Lydia to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'lydia setup gateway'.")
