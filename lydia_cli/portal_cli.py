"""``lydia portal`` — the human-readable entry point for Nous Portal.

Running ``lydia portal`` with no subcommand performs the one-shot Portal
...
``lydia auth add nous --type oauth`` (which still works), is identical to
...
already in ``lydia auth`` or ``lydia native``. It's the onboarding + discovery
...
DOCS_URL = "https://lydia-agent.nousresearch.com/docs/user-guide/features/tool-gateway"
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `lydia portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `lydia auth add nous --type
    ...
    """
    from lydia_cli.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    """Top-level dispatch for `lydia portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `lydia portal` is the
        # human-readable alias for `lydia auth add nous --type oauth` /
        # `lydia setup --portal`.
    ...
    print("Run `lydia portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `lydia portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `lydia portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `lydia auth add "
