"""Profile distributions — shareable, packaged Lydia profiles via git.

A distribution is a Lydia profile published as a git repository (or
...
* ``lydia profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``lydia skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``lydia profile``, not a parallel tree):

    lydia profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "lydia-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            lydia_requires=str(data.get("lydia_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.lydia_requires:
            out["lydia_requires"] = self.lydia_requires
        if self.author:
...
def check_lydia_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Lydia {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``lydia profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Lydia profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Lydia distribution."
        )
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.lydia).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="lydia_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `lydia profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`lydia profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="lydia_dist_update_") as tmp:
        plan = plan_install(
