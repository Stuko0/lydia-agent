...
the tool registry or launching Lydia (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``lydia-cli`` to the non-existent ``lydia``. ``resolve_toolset('lydia')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``lydia-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``lydia`` instead of ``lydia-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `lydia native` to reconfigure."
        )
    return warnings
