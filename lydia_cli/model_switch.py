...
- ``lydia_cli.providers``        -- canonical provider identity + overlays
- ``lydia_cli.model_normalize``  -- per-provider name formatting
...
_LYDIA_MODEL_WARNING = (
    ...
    "for use with Lydia Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research __PROT_LYDIA_MODEL__ / Lydia 4 chat families.
# The previous substring check (`"lydia" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``lydia-brain:qwen3-14b-ctx16k`` that just
# happen to carry "lydia" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   __PROT_NR_LYDIA__-3-Llama-3.1-70B, lydia-4-405b, openrouter/lydia3:70b
# Negative examples it must NOT match:
#   lydia-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
...
def is___PROT_NOUS_LYDIA___non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Lydia 3/4 chat model.
...
def _check_lydia_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Lydia 3/4 chat model."""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``lydia config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from lydia_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'lydia model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'lydia doctor' — config issues detected:"
        ...
        from lydia_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from lydia_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from lydia_cli.config import load_config
            from lydia_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from lydia_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in lydia_cli.runtime_provider.resolve_runtime_provider;
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from lydia_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from lydia_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from lydia_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by lydia provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://lydia-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Lydia release. Falls back to the in-repo
    ...
    # --- 1. Check Lydia-mapped providers ---
    from lydia_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from lydia_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for lydia_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if not has_creds:
            try:
                from lydia_cli.auth import _load_auth_store
        ...
        # /model picker sees the SAME list `lydia model` would build, with
    ...
    # --- 2. Check Lydia-only providers (nous, openai-codex, copilot, opencode-go) ---
    from lydia_cli.providers import LYDIA_OVERLAYS
    from lydia_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Lydia provider ID.
    # LYDIA_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Lydia IDs ("copilot").
    ...
    for pid, overlay in LYDIA_OVERLAYS.items():
        ...
        # Resolve Lydia slug — e.g. "github-copilot" → "copilot"
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            # `lydia model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from lydia_cli.auth import get_provider_auth_state as _nous_state
    ...
    # with `lydia model`).
    try:
        from lydia_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Lydia's canonical write key (matches
            ...
            # Lydia writes ``models:`` as a dict keyed by model id
            # (see lydia_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Lydia's own writer uses; a hyphen variant is accepted
            ...
            # active model. Lydia's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, lydia_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from lydia_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`lydia_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from lydia_cli.models import fetch_openrouter_models
