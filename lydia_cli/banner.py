...
Pure display functions with no LydiaCLI state dependency.
"""
...
_IRIS = "\033[1;38;2;196;167;231m"  # True-color #c4a7e7 bold (Iris — Rose Pine Moon)
_BOLD = "\033[1m"
_DIM = "\033[2m"
_RST = "\033[0m"
...
from lydia_cli import __release_date__ as RELEASE_DATE
from lydia_cli import __version__ as VERSION
...
# (e.g. nix-built lydia — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://10.1.200.116:3000/arquant-admin/NewLydia.git"
_OFFICIAL_REPO_CANONICAL = "github.com/stuk0o/lydia-agent"
...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    if not origin_url:
        # No remote configured (offline fork) — no updates possible
        return 0
...
def _fetch_pypi_latest(package: str = "lydia-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Lydia update is available.
    ...
    # and `lydia update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/lydia/update/check`
    ...
    if embedded_rev:
        ...
        # $LYDIA_HOME/lydia-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = lydia_home / "lydia-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Lydia git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$LYDIA_HOME/lydia-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``lydia_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from lydia_cli.build_info import get_build_sha
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Lydia checkout. Cached per-process. Release URL always points at the
...
def format_banner_version_label() -> str:
    ...
    base = f"Lydia Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(
    ...
    accent = _skin_color("banner_accent", "#ea9a97")
    dim = _skin_color("banner_dim", "#6e6a86")
    text = _skin_color("banner_text", "#e0def4")
    session_color = _skin_color("session_border", "#6e6a86")

    # Use skin's custom gothic L art if provided
    try:
        from lydia_cli.skin_engine import get_active_skin
    ...
    try:
        from lydia_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            ...
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built lydia; we know an update
    ...
    # Pip-install warning — `pip install lydia-agent` is not the supported
