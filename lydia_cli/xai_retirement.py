...
Pure logic: walks a Lydia config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `lydia doctor` and a future `lydia migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Lydia config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Lydia config and return retirement issues.
