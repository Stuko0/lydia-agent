"""Slash command definitions and autocomplete for the Lydia CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Lydia config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Lydia works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Lydia is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.lydia/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Lydia Agent to the latest version", "Info"),
    CommandDef("version", "Show Lydia Agent version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`lydia_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/lydia`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Lydia ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Lydia
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``lydia skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``lydia skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``lydia skills list`` and
...
# unrelated command landed. These claim their slots right after /lydia,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /lydia <command>).
...
# native slot, the alias spelling stays reachable via /lydia reset).
...
# ``/lydia <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/lydia" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /lydia credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /lydia billing.
#   - moa: high-cost slash mode, available through /lydia moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /lydia debug on Slack.
_SLACK_VIA_LYDIA_ONLY = frozenset({"credits", "billing", "moa", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/lydia <verb>`` subcommand.
    ...
    can still reach them via ``/lydia <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/lydia`` is always reserved as the first entry so the
    legacy ``/lydia <subcommand>`` form keeps working for anything that
    ...
    # Reserve /lydia as the catch-all top-level command.
    entries.append(("lydia", "Talk to Lydia or run a subcommand", "[subcommand] [args]"))
    seen.add("lydia")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_LYDIA_ONLY:
            # Intentionally Slack-via-/lydia only (see _SLACK_VIA_LYDIA_ONLY).
            return
    ...
    # everything except /lydia, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://lydia-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /lydia handler.

    Maps both canonical names and aliases so /lydia bg do stuff works
    the same as /lydia background do stuff.

    Plugin-registered slash commands are included so ``/lydia <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from lydia_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
