"""Shell completion script generation for lydia CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# Lydia Agent bash completion
# Add to ~/.bashrc:
#   eval "$(lydia completion bash)"

_lydia_profiles() {{
    local profiles_dir="$HOME/.lydia/profiles"
...
complete -F _lydia_completion lydia
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef lydia
# Lydia Agent zsh completion
# Add to ~/.zshrc:
...
_lydia_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.lydia/profiles" ]]; then
        profiles+=($HOME/.lydia/profiles/*(N/:t))
    fi
...
{top_cmds_str}
            ...
            _describe 'lydia command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Lydia Agent fish completion",
        "# Add to your config:",
        "#   lydia completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __lydia_profiles",
        "    echo default",
        "    if test -d $HOME/.lydia/profiles",
        "        for d in $HOME/.lydia/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c lydia -f -s p -l profile"
