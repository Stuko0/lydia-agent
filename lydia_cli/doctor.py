"""
Doctor command for lydia CLI.

Diagnoses issues with Lydia Agent setup.
"""
...
from lydia_cli.config import get_project_root, get_lydia_home, get_env_path
from lydia_cli.env_loader import load_lydia_dotenv
from lydia_constants import display_lydia_home
from lydia_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
LYDIA_HOME = get_lydia_home()
_DHH = display_lydia_home()  # user-facing display path (e.g. ~/.lydia or ~/.lydia/profiles/coder)

# Load environment variables from ~/.lydia/.env so API key checks work
_env_path = get_env_path()
load_lydia_dotenv(lydia_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from lydia_cli.colors import Colors, color
from lydia_cli.models import _LYDIA_USER_AGENT
from lydia_constants import OPENROUTER_MODELS_URL
...
from lydia_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.lydia/.env contains provider auth/base URL settings."""
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from lydia_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches lydia_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``lydia --version`` reporting a stale version while
    ...
    """
    try:
        from lydia_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != lydia_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'lydia update', or set "
            "lydia_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-lydia and dashboard static services are up
    ...
    for static in ("main-lydia", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `lydia profile create <name>`")
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from lydia_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `lydia`.
    os.environ.setdefault("LYDIA_INTERACTIVE", "1")

    # Handle `lydia doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.lydia/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Lydia Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`lydia doctor --ack {hit.advisory.id}`."
                )
    ...
    # Detect drift between pyproject.toml and lydia_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.lydia/.env (primary location for user config)
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'lydia setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'lydia setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'lydia setup' to create one")
                issues.append("Run 'lydia setup' to create .env")
    
    # Check ~/.lydia/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from lydia_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from lydia_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'lydia config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from lydia_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.lydia/.env or run 'lydia setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'lydia setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'lydia config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from lydia_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from lydia_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'lydia setup' to migrate config")
                else:
                    issues.append("Run 'lydia doctor --fix' or 'lydia setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'lydia doctor --fix'")
        except Exception:
            pass
        ...
        try:
            import yaml
            from lydia_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
            ...
            if drift:
                check_warn(
                    ...
                    "(stale ghost from an earlier `lydia setup` run)",
                )
                if should_fix:
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Lydia' own device-code flow — the Codex CLI is
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Lydia a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Lydia Agent Persona\n\n"
                "<!-- Edit this file to customize how Lydia communicates. -->\n\n"
                "You are Lydia, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    if state_db_path.exists():
        try:
            ...
            from lydia_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'lydia doctor --fix' "
                        "(or 'lydia sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from lydia_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'lydia doctor --fix' "
                        "(or 'lydia sessions repair') to recover hidden sessions"
                    )
            else:
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'lydia doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "lydia"
        ...
        _cmd_link = _cmd_link_dir / "lydia"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(lydia not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/lydia → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/lydia points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/lydia → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/lydia — run 'lydia doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/lydia exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/lydia not found",
                    "(lydia command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/lydia → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/lydia symlink — run 'lydia doctor --fix'")
    ...
    try:
        from lydia_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_path.exists():
            ...
            # `lydia update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `lydia doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable LYDIA_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Lydia one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'lydia config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``lydia doctor``.
    ...
    # never the legitimate source for `lydia doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'lydia setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = LYDIA_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: lydia skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = LYDIA_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: lydia memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: lydia memory setup",
                    "No Honcho API key — run 'lydia memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run lydia memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: lydia memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: lydia memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from lydia_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "lydia -p" in content:
                            _m = _re.search(r"lydia -p (\S+)", content)
