"""``lydia debug`` debug tools for Lydia Agent.

Currently supports:
    lydia debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.lydia/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[lydia debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.lydia/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``lydia debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``lydia debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Lydia version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `lydia debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "lydia-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``lydia debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.lydia/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``lydia debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "lydia-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from lydia_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_lydia_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``lydia dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``lydia dump`` is run
        internally.
...
_NOUS_BUNDLE_FORMAT = "lydia-debug-share/1"
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``lydia debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "lydia debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `lydia debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print(f"To delete now:  lydia debug delete <url>")

    print(f"\nShare these links with the Lydia team for support.")
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/Lydia version, provider, which API keys are
...
def _run_debug_share_nous(*, log_lines: int, redact: bool) -> None:
    """Handle ``lydia debug share --nous``: upload the bundle to Nous-S3.
    ...
    suggests ``--local``.
    """
    from lydia_cli.diagnostics_upload import share_to_nous
    ...
    if redact:
        logger.info(
            "lydia debug share --nous: applied force-mode redaction before upload"
        )
    ...
    try:
        ...
        print(
            ...
            "provisioned.\n"
            "Run `lydia debug share --local` to print the report instead, "
            "or `lydia debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: lydia debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'lydia debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``lydia debug`` call.
    ...
    # best-effort — any failure is swallowed so ``lydia debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: lydia debug <command>")
        print()
        print("Commands:")
