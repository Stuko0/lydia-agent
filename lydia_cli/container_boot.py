...
``$LYDIA_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-lydia-setup
(the stage2 hook) has chowned the volume and seeded $LYDIA_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$LYDIA_HOME``,
    not under ``profiles/``). The dispatcher in ``lydia_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``lydia gateway start`` (no ``-p``) targets. Without it,
    bare ``lydia gateway start`` inside the container would land on
    ...
    ``$LYDIA_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        lydia_home: The container's LYDIA_HOME (typically /opt/data).
    ...
    # ``lydia gateway start`` (no ``-p``) has somewhere to land;
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `lydia profile create` (config.yaml
            ...
            # via ``lydia -p default-named gateway start`` if they
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the lydia args.

    Two container-command argv shapes are handled:

    * **s6-overlay v2 / tini:** PID 1 argv is
      ``/init /opt/lydia/docker/main-wrapper.sh <subcommand> [args...]``.
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
      rc.init-launched process as ``/bin/sh -e
      /run/s6/basedir/scripts/rc.init top /opt/lydia/docker/main-wrapper.sh
    ...
    ``lydia`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``lydia <subcommand>``, so an
    explicit leading ``lydia`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `lydia <subcommand>`; peel an explicit lydia.
    if args and Path(args[0]).name == "lydia":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``lydia dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted LYDIA_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `lydia -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit lydia
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the lydia user) won't hit EACCES. See
...
def _write_reconcile_log(
    ...
    """Append one line per profile to $LYDIA_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted LYDIA_HOME, both race to flock() the same s6-log lock
