"""``lydia logs`` — view and filter Lydia log files.
...
Usage examples::
    ...
    lydia logs                    # last 50 lines of agent.log
    lydia logs -f                 # follow agent.log in real time
    lydia logs errors             # last 50 lines of errors.log
    lydia logs gateway -n 100    # last 100 lines of gateway.log
    lydia logs gui -f            # follow gui.log (dashboard/pty/ws)
    lydia logs desktop -f        # follow desktop.log (Electron app boot/backend)
    lydia logs --level WARNING    # only WARNING+ lines
    lydia logs --session abc123   # filter by session ID substring
    lydia logs --component tools  # only tool-related lines
    lydia logs --since 1h         # lines from the last hour
    lydia logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_lydia_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Lydia runs — try 'lydia chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_lydia_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_lydia_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_lydia_home()}/logs/")
        return

    print(f"Log files in {display_lydia_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'lydia chat' to generate logs)")
