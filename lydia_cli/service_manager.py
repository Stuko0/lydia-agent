...
the existing module-level functions in lydia_cli.gateway and
lydia_cli.gateway_windows directly. This protocol is a thin facade
...
``lydia gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``lydia gateway start/stop/restart``).
    """
    ...
    # `lydia gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged lydia
    ...
    detection always fails. Since every Lydia runtime call inside the
    container drops to lydia via ``s6-setuidgid``, that silent failure
...
# in ``lydia_cli.gateway`` (systemd/launchd) and ``lydia_cli.gateway_windows``
...
# active (set via the ``lydia -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in lydia_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from lydia_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from lydia_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from lydia_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from lydia_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in lydia_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from lydia_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from lydia_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from lydia_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from lydia_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``lydia_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `lydia profile create`
# runs inside the container (Phase 4). Static services (main-lydia, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's LYDIA_HOME blindly;
    derive the shared profile root from the current LYDIA_HOME and map the
    ...
    lydia_home = Path(os.environ.get("LYDIA_HOME", "/opt/data"))
...
# UID/GID of the in-image ``lydia`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the lydia user.
    ...
    (the ``lydia`` user — UID 10000 — running every Lydia runtime
    ...
    is dispatched as the lydia user.
    ...
    with lydia ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           lydia:lydia, 0755 (parent must already exist)
    ``svc_dir/event/``                     lydia:lydia, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 lydia:lydia, 0755
    ``svc_dir/supervise/event/``           lydia:lydia, 03730
    ``svc_dir/supervise/control``          lydia:lydia, 0660    (FIFO)
    ...
    slot's supervise/ was lydia-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        try:
            ...
            # Running as the lydia user already — directory is lydia-
    ...
    # when the parent slot's supervise/ is lydia-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``lydia -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`lydia profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-lydia, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources LYDIA_HOME (and any extra env) via with-contenv —
             so e.g. ``-e LYDIA_HOME=/data/lydia`` is honored at run
          ...
          4. Drops to the lydia user and exec's
             ``lydia -p <profile> gateway run`` (or just ``lydia
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``lydia gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root LYDIA_HOME profile" (the implicit profile that exists at
        the top of $LYDIA_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``lydia_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$LYDIA_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``LYDIA_HOME``,
        ...
        allocator in ``lydia_cli.profiles._allocate_gateway_port``).
        ...
        # profile's LYDIA_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``lydia gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-LYDIA_HOME PID lock first;
        ...
        # Each profile is scoped to its own LYDIA_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "lydia gateway run --replace"
        else:
            gateway_cmd = f"lydia -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid lydia {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${LYDIA_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the LYDIA_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e LYDIA_HOME=/data/lydia``
        gets its logs under /data/lydia/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``lydia logs`` reads and what
        ...
        return (
            ...
            f': "${{LYDIA_HOME:=/opt/data}}"\n'
            f'log_dir="$LYDIA_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            ...
            # later then runs its log service as lydia and crash-loops on
            ...
            f'chown lydia:lydia "$LYDIA_HOME/logs/gateways" 2>/dev/null || true\n'
            f'chown -R lydia:lydia "$log_dir" 2>/dev/null || true\n'
            f'rm -f "$log_dir/lock"\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid lydia s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``lydia gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``lydia -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with lydia ownership
            ...
            # the lydia user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is lydia-owned (see ``_seed_supervise_skeleton``), and on
