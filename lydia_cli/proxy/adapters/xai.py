...
# xAI's public API is OpenAI-compatible for the endpoints Lydia commonly
# uses. The Responses endpoint is included because Lydia' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Lydia-managed OAuth credentials."""

    auth_hint = "lydia auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`lydia auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`lydia auth reset xai-oauth` or re-authenticate with "
                    "`lydia auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `lydia auth add xai-oauth --type oauth`."
            )
