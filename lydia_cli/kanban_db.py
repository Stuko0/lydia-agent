...
``<root>`` is the **shared Lydia root** (the parent of any active
...
``lydia -p <profile>`` joins the same board as the dispatcher that
...
the profile's Lydia home but are otherwise isolated: a worker spawned
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``lydia kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.lydia``. In Docker / custom
deployments where ``LYDIA_HOME`` points outside ``~/.lydia`` (e.g.
``/opt/lydia``), ``<root>`` is ``LYDIA_HOME``. Legacy env-var
...
coding workloads.  See ``docs/lydia-kanban-v1-spec.pdf`` for the full
...
from lydia_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active LYDIA_HOME so dispatcher- and
...
# enough that kebab-case names like ``atm10-server`` or ``lydia-agent``
...
def kanban_home() -> Path:
    """Return the shared Lydia root that anchors the kanban board.
    ...
    2. ``get_default_lydia_root()``, which already returns ``<root>``
       when ``LYDIA_HOME`` is ``<root>/profiles/<name>``, and returns
       ``LYDIA_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``LYDIA_HOME`` would silently fork the board per profile,
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``lydia kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    2. ``<root>/kanban/current`` on disk (set by ``lydia kanban boards
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``lydia kanban boards switch <typo>`` returns an error
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``lydia kanban log`` unambiguous even when multiple
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (lydia_cli/projects_db). When set,
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``lydia gateway run --replace`` /
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Lydia treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
    ...
    # holder (e.g. an external `lydia kanban list` mid-integrity-probe) block
    ...
    if resolved in _INITIALIZED_PATHS:
        ...
        try:
            ...
            with _INIT_LOCK:
                from lydia_state import apply_wal_with_fallback
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See lydia_state._WAL_INCOMPAT_MARKERS for detection logic.
                from lydia_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``lydia kanban init`` and the
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from lydia_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``lydia --skills ...``. Use this to pin a task to a
    ...
    if project_id:
        try:
            from lydia_cli import projects_db as _pdb
    ...
    # `lydia --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``lydia kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `lydia kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``lydia kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``lydia kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``lydia kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``lydia kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Lydia' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Lydia-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``lydia kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Lydia checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Lydia treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Lydia creates/reuses that linked worktree. With
      no ``workspace_path``, Lydia anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Lydia uses
      ``wt/<task-id>``.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Lydia
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``lydia kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``lydia kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Lydia profile.
    ...
    if not rows:
        return False
    try:
        from lydia_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from lydia_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Lydia profile.
        # `_default_spawn` invokes ``lydia -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Lydia
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `lydia kanban tail` — without
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from lydia_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _module_lydia_argv() -> list[str]:
    """Return the interpreter-bound Lydia CLI invocation."""
    # ``lydia_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``lydia`` package — there is no
    # ``lydia`` package to import.
    return [sys.executable, "-m", "lydia_cli.main"]


def _absolute_lydia_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Lydia shim."""
...
def _lydia_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Lydia executable path.
...
def _resolve_lydia_argv() -> list[str]:
    """Resolve the ``lydia`` invocation as argv parts for ``Popen``.
    ...
    2. ``shutil.which("lydia")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\lydia.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m lydia_cli.main`` — fallback for setups where
       Lydia is launched from a venv and the ``lydia`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_lydia_bin`` for the same reason. Kept
    local (not imported from gateway) because ``lydia_cli`` sits below
    ...
    lydia_bin = _safe_which_no_cwd("lydia") if _IS_WINDOWS else shutil.which("lydia")
    if lydia_bin:
...
def _resolve_worker_cli_toolsets(lydia_home: Optional[str]) -> Optional[list[str]]:
    ...
    if not lydia_home:
        return None
    try:
        from lydia_constants import reset_lydia_home_override, set_lydia_home_override
        from lydia_cli.config import load_config
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for LYDIA_HOME=%r (%s)",
            lydia_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``lydia -p <profile> chat -q ...`` subprocess.
    ...
    # Inject LYDIA_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `lydia -p <name>` the
    # _apply_profile_override() runs *before* lydia_constants is imported.
    # If LYDIA_HOME is absent from the child's env, get_lydia_home() falls
    # back to Path.home() / ".lydia" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from lydia_cli.profiles import resolve_profile_env
    try:
        env["LYDIA_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via LYDIA_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # LYDIA_HOME never had profiles created.
        pass
    if task.tenant:
    ...
    # that even when the worker activates a profile (`lydia -p <name>`
    # rewrites LYDIA_HOME), its kanban paths still match the
    ...
    # `lydia -p <assignee>` activates the profile, but the env var is
    ...
    cmd = [
        *_resolve_lydia_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped LYDIA_HOME above,
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("LYDIA_HOME"))
    if worker_toolsets:
    ...
    # `lydia kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`lydia` executable not found on PATH. "
            "Install Lydia Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``lydia kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled LYDIA_PROFILE values like "lydia-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Lydia root exists

    Reads profile paths directly so this module has no import dependency on
    ``lydia_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from lydia_constants import get_default_lydia_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``lydia kanban assignees`` for the terminal.
