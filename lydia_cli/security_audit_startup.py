...
3. Running inside a container with no persistent volume mount over the
   LYDIA_HOME data dir (state is ephemeral — lost on container restart).
...
logger = logging.getLogger("lydia.security_audit")
...
def _running_as_root() -> Optional[str]:
    ...
    return (
        ...
        "full host compromise. Run Lydia as an unprivileged user (or in a "
...
def _container_no_volume_mount(lydia_home: Optional[Path]) -> Optional[str]:
    ...
    home = lydia_home or Path(
        os.environ.get("LYDIA_HOME", os.path.expanduser("~/.lydia"))
    )
    try:
