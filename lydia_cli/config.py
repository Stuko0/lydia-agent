"""
Configuration management for Lydia Agent.

Config files are stored in ~/.lydia/ for easy access:
- ~/.lydia/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.lydia/.env         - API keys and secrets

This module provides:
- lydia config          - Show current configuration
- lydia config edit     - Open config in editor
- lydia config set      - Set a specific value
- lydia config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``lydia config set`` (which rewrites
    ...
    ``config.yaml`` in place: lydia never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.lydia/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``lydia logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  lydia config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Lydia makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Lydia itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``lydia update``, the TUI build.
...
#   on every plugin install / ``lydia update``.
...
# * ``LYDIA_HOME`` / ``LYDIA_PROFILE`` / ``LYDIA_CONFIG`` /
#   ``LYDIA_ENV`` — Lydia runtime location flags. Writing these into
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Lydia runtime location — never via dashboard env writer.
    # NOT a LYDIA_* blanket: integration credentials (LYDIA_GEMINI_*,
    # LYDIA_LANGFUSE_*, LYDIA_SPOTIFY_*, ...) ARE allowed.
    "LYDIA_HOME", "LYDIA_PROFILE", "LYDIA_CONFIG", "LYDIA_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Lydia runtime location "
            "(LYDIA_HOME, LYDIA_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `lydia plugins enable
    # observability/langfuse` or `lydia native → Langfuse`); credentials gate
...
from lydia_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_lydia_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Lydia is running in package-manager-managed mode.

    Two signals: the LYDIA_MANAGED env var (set by the systemd service),
    or a .managed marker file in LYDIA_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade lydia-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``lydia_cli/`` — i.e. the git checkout for source
    installs, ``/opt/lydia`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$LYDIA_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Lydia was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$LYDIA_HOME/.install_method`` — read for
    ...
    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.lydia``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$LYDIA_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.lydia:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$LYDIA_HOME``, a
    ...
    ``docker`` and ``lydia update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``stuk0o/lydia-agent`` image bakes a ``docker``
        stamp into ``/opt/lydia`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $LYDIA_HOME.
    try:
    ...
    #    host install whose shared $LYDIA_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``lydia update``.
    try:
        method = (
            (get_lydia_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``lydia_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$LYDIA_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/lydia`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Lydia lives in a ``uv tool`` layout.

    ``uv tool install lydia-agent`` places the install at
    ``.../uv/tools/lydia-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``lydia-agent`` happens to be uv-tool-installed on the machine while
    the *active* Lydia is a regular pip/venv install, causing
    ``lydia update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/lydia-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade lydia-agent"
    if method == "docker":
        return "docker pull stuk0o/lydia-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade lydia-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade lydia-agent"
        return "pip install --upgrade lydia-agent"
    return "lydia update"
...
# Long-form text for ``lydia update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# lydia_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Lydia, it doesn't update
...
✗ ``lydia update`` doesn't apply inside the Docker container.

Lydia Agent runs as a published image (stuk0o/lydia-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull stuk0o/lydia-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate lydia-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm stuk0o/lydia-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/stuk0o/lydia-agent/tags
  • Your config and session history live under ``$LYDIA_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``lydia update`` inside Docker.
...
def format_managed_message(action: str = "modify this Lydia installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this Lydia installation is managed by NixOS "
            f"(LYDIA_MANAGED={env_hint}).\n"
            "Edit services.lydia-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Lydia installation is managed by Homebrew "
            f"(LYDIA_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade lydia-agent"
        )

    return (
        f"Cannot {action}: this Lydia installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Lydia."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from LYDIA_HOME/.container-mode.
    ...
    container_mode_file = get_lydia_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "lydia-agent")
    exec_user = info.get("exec_user", "lydia")
    lydia_bin = info.get("lydia_bin", "/data/current-package/bin/lydia")
...
# Re-export from lydia_constants — canonical definition lives there.
from lydia_constants import get_lydia_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_lydia_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_lydia_home() / ".env"
...
def _resolve_lydia_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    Docker containers running Lydia commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level LYDIA_HOME once, but
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the lydia group can
    ...
    caddy, etc.) needs to traverse LYDIA_HOME to reach a served subdirectory.
...
def _is_container() -> bool:
    ...
    When Lydia runs in a container with volume-mounted config files, forcing
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into LYDIA_HOME, upgrading legacy empty templates.
...
def ensure_lydia_home():
    """Ensure ~/.lydia directory structure exists with secure permissions.
...
def _ensure_lydia_home_managed(home: Path):
    ...
    if not home.is_dir():
        raise RuntimeError(
            f"LYDIA_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Lydia-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Lydia
        ...
        # app, ACP) in a code workspace, Lydia adds a coding operating brief
    ...
    "terminal": {
        ...
        #             LYDIA_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force LYDIA_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Lydia auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Lydia sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Lydia
        ...
        # bundled Lydia image, which drops to the `lydia` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Lydia sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Lydia should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``lydia chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.lydia/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: lydia sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .lydia.md, or .cursorrules before Lydia applies
    ...
    # single read_file page exceeds these limits, Lydia truncates the
    ...
    # See tools/kanban_tools.py and lydia_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``lydia dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``lydia kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``lydia kanban decompose`` and the kanban
        ...
        # ``lydia profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `lydia model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `lydia` (and `lydia chat`) launches by default:
        ...
        "interface": "cli",
        # When true, `lydia --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `lydia -c` muscle memory.  Default off so existing
        # users aren't surprised.  LYDIA_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `lydia --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/lydia`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `lydia pets`. Disabled until
        ...
        "pet": {
            ...
            # get_lydia_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # optional path prefix (e.g. ``https://example.com/lydia``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Lydia appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.lydia/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.lydia/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.lydia/cache/delegation/
    ...
    # response. If not, Lydia feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Lydia auto-pauses the goal and
    ...
    # cron daemon.
    #
    # See `lydia curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `lydia curator run --consolidate` overrides
        ...
        # normally restored on every `lydia update`, so pruning them only
        ...
        # skipped), snapshot ~/.lydia/skills/ into
        # ~/.lydia/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `lydia curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for lydia-specific overrides; everything else
    ...
    # replace Lydia' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.lydia/shell-hooks-allowlist.json.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `lydia doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``lydia_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Lydia to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $LYDIA_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
    ...
    # todos to ready, and fires `lydia -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `lydia` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `lydia kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     lydia-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Lydia native (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.lydia/logs/.
    ...
    # update model picker lists without shipping a lydia-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://lydia-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `lydia model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.lydia/.env,
        ...
        # behavior: files must live under the Lydia cache, under
        ...
        # uploaded as native gateway attachments. Files inside the Lydia
        # cache (~/.lydia/cache/{documents,images,audio,video,screenshots})
    ...
    # Session storage — controls automatic cleanup of ~/.lydia/state.db.
    ...
    "sessions": {
        ...
        # default of ``lydia sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.lydia/sessions/session_{sid}.json`` on every turn
    ...
    # ``lydia update`` behaviour.
    "updates": {
        # Run a full ``lydia backup``-style zip of LYDIA_HOME before every
        # ``lydia update``.  Backups land in ``<LYDIA_HOME>/backups/`` and
        # can be restored with ``lydia import <path>``.  Off by default:
        # zipping a large LYDIA_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``lydia update
        ...
        "backup_keep": 5,
        # What `lydia update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<LYDIA_HOME>/lsp/bin/`` on first use.
    ...
    # `lydia native`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.lydia/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.lydia/.env (or your shell) and never in config.yaml.
            ...
            # ~/.lydia/bin/ on first use.  When False you must install
    ...
    "computer_use": {
        ...
        # by default upstream. Lydia disables it for our users unless they
        # explicitly opt in here. When false (default), Lydia sets
    ...
    # Lydia Desktop (Electron app) launch options. These only affect
    # `lydia desktop`; they do not touch the CLI/gateway.
    "desktop": {
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'lydia model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Lydia auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Lydia, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @lydia:example.org)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #lydia)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: lydia-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'lydia-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Lydia API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Lydia API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Lydia API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.lydia.config`` entries, then checks
    ...
    try:
        ...
        # should never break `lydia update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Lydia versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``lydia_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'lydia setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Lydia won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'lydia doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("LYDIA_HOME", "~/.lydia")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "lydia update / lydia -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$LYDIA_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_lydia_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `lydia plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `lydia curator status` has no
    ...
    #   3. Creates `~/.lydia/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_lydia_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_lydia_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `lydia config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `lydia config set`"
                    )
    ...
    # behavior set it to true explicitly via `lydia config set`.
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from lydia_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: lydia config set <key> <value>")
    ...
    # `lydia update`, but only the version bump is persisted.
    ...
    # metadata.lydia.config in their SKILL.md frontmatter.
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``lydia config set``
    ...
    backends) — while display paths (``lydia status``/``dump``) read ``name``
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.lydia/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.lydia/config.yaml.
    ...
    (which change ``LYDIA_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/lydia/config.yaml invalidates the
...
#   openai-codex (OAuth — lydia auth) — OpenAI Codex
#   nous         (OAuth — lydia auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.lydia/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Lydia writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.lydia/.env.
    ...
    interactive menu render (`lydia native`, `lydia setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `lydia native`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Lydia env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.lydia/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.lydia/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.lydia/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.lydia/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Lydia — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Lydia vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.lydia/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.lydia/.env`` over ``os.environ``.

    Used for Lydia-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    from lydia_cli.auth import get_anthropic_key
    ...
    try:
        ...
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                ...
                f"(run 'lydia doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  lydia config edit     # Edit config file", Colors.DIM))
    print(color("  lydia config set <key> <value>", Colors.DIM))
    print(color("  lydia setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `lydia config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `lydia config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: lydia config set <key> <value>")
            print()
            print("Examples:")
            print("  lydia config set model anthropic/claude-sonnet-4")
            print("  lydia config set terminal.backend docker")
            print("  lydia config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'lydia config migrate' to add them")
        ...
        print("  lydia config           Show current configuration")
        print("  lydia config edit      Open config in editor")
        print("  lydia config set <key> <value>   Set a config value")
        print("  lydia config check     Check for missing/outdated config")
        print("  lydia config migrate   Update config with new options")
        print("  lydia config path      Show config file path")
        print("  lydia config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``lydia config`` UI so users
