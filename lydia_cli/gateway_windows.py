...
_TASK_NAME_DEFAULT = "Lydia_Gateway"
_TASK_DESCRIPTION = "Lydia Agent Gateway - Messaging Platform Integration"
...
def _preserve_lydia_home_path(path: str | Path) -> str:
    """Render Lydia-owned paths under the configured LYDIA_HOME spelling.

    Windows installs may keep ``%LOCALAPPDATA%\\lydia`` as a symlink/junction to
    ...
    path lives under LYDIA_HOME.
    """
    candidate = Path(path)
    try:
        from lydia_cli.config import get_lydia_home

        home = Path(get_lydia_home())
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Lydia profile."""
    from lydia_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "lydia_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def get_task_name() -> str:
    ...
    Default profile: ``Lydia_Gateway``
    Named profile X: ``Lydia_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during lydia_cli boot.
    from lydia_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\lydia\\gateway-service\\<task_name>.cmd``
    (or ``<LYDIA_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Lydia installs stay self-contained).
    """
    _assert_windows()
    from lydia_cli.config import get_lydia_home

    script_dir = Path(get_lydia_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``LYDIA_HOME`` whenever it
    ...
    source checkout only if ``LYDIA_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports LYDIA_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m lydia_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "LYDIA_HOME={lydia_home}"')
    ...
    # if someone imports lydia_constants-based logic during startup.
    ...
    prog_args = [pythonw_path, "-m", "lydia_cli.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('LYDIA_HOME')}) = {_quote_vbs_string(lydia_home)}",
...
def _write_task_script() -> Path:
    ...
    lydia_home = str(Path(get_lydia_home()))
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Lydia builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "lydia_cli.main"]
    if profile_arg:
...
def windowless_gateway_restart_spec(
    ...
    ``lydia_cli`` package without the venv launcher's site config.

    Returns ``(new_argv, working_dir, env_overlay)``.  ``new_argv``
    preserves every argument after the interpreter (``-m lydia_cli.main
    ...
    from lydia_cli.config import get_lydia_home
    from lydia_cli.gateway import PROJECT_ROOT
    ...
    try:
        lydia_home = str(Path(get_lydia_home()).resolve())
    except Exception:
        lydia_home = ""
    ...
    if lydia_home:
        env_overlay["LYDIA_HOME"] = lydia_home
    _prepend_pythonpath(
        env_overlay,
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m lydia_cli.main gateway run``
    ...
    log_dir = Path(get_lydia_home()) / "logs"
...
def _prompt_install_choices(
    ...
    from lydia_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `lydia -p <profile> gateway install` must be safe.
    ...
    from lydia_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"lydia {profile_arg} gateway start" if profile_arg else "lydia gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: lydia gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Lydia gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: lydia gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: lydia gateway start")
        _print_next_steps()
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from lydia_cli.config import get_lydia_home
        print(f"    type {Path(get_lydia_home())}\\logs\\gateway.log")
        print(f"    type {Path(get_lydia_home())}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from lydia_cli.config import get_lydia_home

    lydia_home = Path(get_lydia_home())
    print()
    print("Next steps:")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Lydia gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: lydia gateway status")
                    return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: lydia gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: lydia gateway start")
            return
...
def restart() -> None:
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `lydia gateway status`."
        )
