"""
Backup and import commands for lydia CLI.

`lydia backup` creates a zip archive of the entire ~/.lydia/ directory
(excluding the lydia-agent repo and transient files).

`lydia import` restores from a backup zip, overlaying onto the current
LYDIA_HOME root.
"""
...
from lydia_constants import get_default_lydia_root, get_lydia_home, display_lydia_home
...
# ``lydia-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/lydia-agent/``
...
# LYDIA_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "lydia-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under LYDIA_HOME) —
...
# File names that ``lydia import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE LYDIA_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside LYDIA_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to lydia root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``lydia-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/lydia-agent/`` — must be preserved.
        if part == "lydia-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside LYDIA_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Lydia home directory."""
    lydia_root = get_default_lydia_root()

    if not lydia_root.is_dir():
        print(f"Error: Lydia home directory not found at {lydia_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"lydia-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"lydia-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_lydia_home()} ...")
    ...
    for dirpath, dirnames, filenames in os.walk(lydia_root, followlinks=False):
        ...
        # ``lydia-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "lydia-agent" and not is_root)
        ]
    ...
    # outside LYDIA_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_lydia_home()}."
        )
    ...
    print(f"\nRestore with: lydia import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Lydia backup.
    ...
    # Look for telltale files that a lydia home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Lydia backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.lydia/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a lydia dir name
        if prefix in {".lydia", "lydia"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_lydia_home()}")
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Lydia configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under LYDIA_HOME.
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_lydia_home()}."
            )
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # lydia_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: lydia profile list  (after installing lydia)")

        # Guidance
        print()
        if not (lydia_root / "lydia-agent").is_dir():
            print("Note: The lydia-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: lydia update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  lydia -p {pname} gateway install")

        print("Done. Your Lydia configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and lydia backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to LYDIA_HOME).
...
# — `lydia update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $LYDIA_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside LYDIA_HOME and the entry is silently
...
# Relative path of the cron job database inside LYDIA_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``lydia update``.
    ...
    Args:
        ...
        lydia_home: Override for the Lydia home directory (tests).
...
def run_quick_backup(args) -> None:
    """CLI entry point for lydia backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_lydia_home()}/state-snapshots/")
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `lydia claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of LYDIA_HOME under ``backups/`` before a
    ``lydia claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``lydia import <archive>``.  Writes to
    ``<LYDIA_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `lydia import` and the
