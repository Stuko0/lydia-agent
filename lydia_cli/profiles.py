"""
Profile management for multiple isolated Lydia instances.

Each profile is a fully independent LYDIA_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.lydia/profiles/<name>/`` by default.

The "default" profile is ``~/.lydia`` itself — backward compatible,
zero migration needed.

Usage::

    lydia profile create coder          # fresh profile + bundled skills
    lydia profile create coder --clone  # also copy config, .env, SOUL.md, skills
    lydia profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    lydia -p coder chat                 # or via flag
    lydia profile use coder             # set as sticky default
    lydia profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See lydia_constants.get_subprocess_home().
    "home",
]
...
# Marker file written by `lydia profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `lydia
...
# `lydia skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Lydia infrastructure directories that only the default profile
         (``~/.lydia``) ever contains.  Gated on ``source_dir`` actually
...
# Directories/files to exclude when exporting the default (~/.lydia) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "lydia-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "lydia", "default", "test", "tmp", "root", "sudo",
})

# Lydia subcommands that cannot be used as profile names/aliases
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the lydia root, NOT to the current LYDIA_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where LYDIA_HOME points outside
    ``~/.lydia``, profiles live under ``LYDIA_HOME/profiles/`` so
...
def _get_default_lydia_home() -> Path:
    """Return the default (pre-profile) LYDIA_HOME path.

    In standard deployments this is ``~/.lydia``.
    In Docker/custom deployments where LYDIA_HOME is outside ``~/.lydia``
    (e.g. ``/opt/data``), returns LYDIA_HOME directly.
    """
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Lydia installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its LYDIA_HOME directory."""
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, lydia subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _LYDIA_SUBCOMMANDS:
        return f"'{canon}' conflicts with a lydia subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            lydia_exe = shutil.which("lydia") or "lydia"
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Lydia. If we
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``lydia -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``lydia profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``lydia -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``lydia profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``LYDIA_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Lydia config
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own LYDIA_HOME.
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``lydia update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.lydia)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.lydia/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `lydia -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Lydia profile.\n"
    ...
    if not soul_path.exists():
        try:
            from lydia_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `lydia update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`lydia profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Lydia (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `lydia profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `lydia -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches LYDIA_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``lydia profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "LYDIA_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.lydia).\n"
            "To remove everything, use: lydia uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``LYDIA_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from lydia_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set LYDIA_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("LYDIA_HOME")
    try:
        os.environ["LYDIA_HOME"] = str(profile_dir)
        from lydia_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["LYDIA_HOME"] = old_home
        elif "LYDIA_HOME" in os.environ:
            del os.environ["LYDIA_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.lydia/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: lydia profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from LYDIA_HOME.

    Returns ``"default"`` if LYDIA_HOME is not set or points to ``~/.lydia``.
    Returns the profile name if LYDIA_HOME points into ``~/.lydia/profiles/<name>``.
    Returns ``"custom"`` if LYDIA_HOME is set to an unrecognized path.
    """
    from lydia_constants import get_lydia_home
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.lydia itself — its parent is ~/ and its
        # directory name is ".lydia", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: lydia profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.lydia itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.lydia). "
            "Specify a different name: lydia profile import <archive> --name <name>"
        )
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    legacy_old_host = f"lydia.{old_name}"
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a LYDIA_HOME path string.

    Called early in the CLI entry point, before any lydia modules
    are imported, to set the LYDIA_HOME environment variable.
    """
