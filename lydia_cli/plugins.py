...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with lydia-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.lydia/plugins/<name>/``
3. **Project plugins** – ``./.lydia/plugins/<name>/`` (opt-in via
   ``LYDIA_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``lydia_agent.plugins``
   entry-point group.
...
from lydia_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
...
# stderr in addition to ~/.lydia/logs/agent.log. Aimed at plugin authors
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    ...
    is passed. Does not touch the root logger or other Lydia loggers.
    """
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Lydia' shipped guidance lives in the evidence-based
    ...
    # Kanban task lifecycle hooks. Fired by lydia_cli.kanban_db when a task
    ...
    # separate `lydia -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `lydia kanban dispatch`),
...
ENTRY_POINTS_GROUP = "lydia_agent.plugins"
...
class PluginManifest:
    ...
    #              in ~/.lydia/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``lydia plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active Lydia profile name (e.g. ``"default"``).

        Derived from ``LYDIA_HOME`` via
        :func:`lydia_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.lydia/profiles/<name>``, or ``"custom"`` when
        ``LYDIA_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Lydia core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``lydia honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``lydia <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`lydia_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from lydia_cli.dashboard_auth import (
    ...
    def register_slack_action_handler(
        ...
        Lydia' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``lydia model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: lydia_cli.main imports plugins indirectly
        from lydia_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.lydia/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to lydia_cli/. Two layouts are
        ...
        # 2. User plugins (~/.lydia/plugins/)
        user_dir = get_lydia_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.lydia/plugins/)
        if _env_enabled("LYDIA_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".lydia" / "plugins"
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with lydia and must
            ...
            # all ~20 of them added several seconds to every `lydia`
            # invocation — including plain `lydia chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Lydia ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `lydia plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``lydia plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`lydia plugins
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `lydia plugins list`.
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="lydia-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``lydia native`` TUI so plugin-provided toolsets appear
