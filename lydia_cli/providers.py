"""
Single source of truth for provider identity in Lydia Agent.
...
2. **Lydia overlays** — transport type, auth patterns, aggregator flags,
...
# -- Lydia overlay ----------------------------------------------------------
# Lydia-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class LydiaOverlay:
    """Lydia-specific provider metadata layered on top of models.dev."""
...
LYDIA_OVERLAYS: Dict[str, LydiaOverlay] = {
    ...
    "openai-codex": LydiaOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": LydiaOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": LydiaOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": LydiaOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
    ...
    "copilot-acp": LydiaOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": LydiaOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": LydiaOverlay(
    ...
    "kimi-for-coding": LydiaOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": LydiaOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": LydiaOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": LydiaOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": LydiaOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": LydiaOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": LydiaOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": LydiaOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "opencode": LydiaOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": LydiaOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "azure-foundry": LydiaOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": LydiaOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "lydia", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Lydia overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Lydia overlay
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + lydia extra
    ...
    if overlay is not None:
        # Lydia-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
