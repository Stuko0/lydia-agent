"""``lydia slack ...`` CLI subcommands.

Today only ``lydia slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ lydia slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Lydia. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Lydia deployment — users can tweak them in the Slack UI after
    pasting.
    ...
    ``/new``, etc. work inline.
    """
    from lydia_cli.commands import slack_app_manifest
    ...
    if include_assistant:
        features["assistant_view"] = {
            "assistant_description": "Chat with Lydia in threads and DMs.",
        }
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``lydia_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$LYDIA_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Lydia")
    ...
    """
    name = getattr(args, "name", None) or "Lydia"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("LYDIA_HOME") or str(Path.home() / ".lydia")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Lydia app\n"
