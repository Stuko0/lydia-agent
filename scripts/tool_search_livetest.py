#!/usr/bin/env python3
"""Live test harness for Lydia Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any lydia imports.
ORIGINAL_HOME = os.environ.get("LYDIA_HOME")
ORIGINAL_AUTH = Path.home() / ".lydia" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.lydia/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.lydia/.env`` so
    ...
    real_env_file = Path.home() / ".lydia" / ".env"
    if real_env_file.exists():
        ...
        load_lydia_dotenv(lydia_home=str(Path.home() / ".lydia"))
...
def reset_module_state():
    """Drop cached modules so the new LYDIA_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "lydia_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["LYDIA_HOME"] = str(home)
...
def main():
    ...
    # Restore original LYDIA_HOME
    if ORIGINAL_HOME is not None:
        os.environ["LYDIA_HOME"] = ORIGINAL_HOME
    else:
