...
#   2. ~/.lydia/node/ from a prior Lydia-managed install
#   3. fnm, proto, nvm (in that order) if the user already uses a version manager
#   4. Termux `pkg`, macOS Homebrew
#   5. pinned nodejs.org tarball into ~/.lydia/node/ (always works, zero shell rc edits)
#
# Usage:
...
# Redirect a Lydia-managed Node's `npm install -g` to the command link dir
...
# ---------------------------------------------------------------------------
# Heal a broken Lydia-managed Node tree (partial upgrade / missing lib/)
# ---------------------------------------------------------------------------
...
# absent. Used by lydia_constants.find_lydia_node_executable() and safe
...
heal_managed_node() {
    ...
    _nb_log "Lydia-managed Node is broken — redownloading to $LYDIA_HOME/node/..."
    _nb_install_bundled_node
}
...
ensure_node() {
    ...
    if [ -x "$LYDIA_HOME/node/bin/node" ]; then
        export PATH="$LYDIA_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (Lydia-managed)"
            LYDIA_NODE_AVAILABLE=true
            return 0
        fi
    fi
