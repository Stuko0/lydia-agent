#!/usr/bin/env python3
"""Drive the Lydia TUI under LYDIA_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.lydia/perf.log on exit.
...
bypasses the lydia_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  LYDIA_PERF_LOG     (default ~/.lydia/perf.log)
...
DEFAULT_LOG = Path(os.environ.get("LYDIA_PERF_LOG", str(get_lydia_home() / "perf.log")))
DEFAULT_STATE_DB = get_lydia_home() / "state.db"
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / "packages" / "lydia-ink" / "src"
