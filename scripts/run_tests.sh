#!/usr/bin/env bash
# Canonical test runner for lydia-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.lydia/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.lydia/lydia-agent/venv"; do
...
if [ -f "$HOME/.lydia/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.lydia"
...
exec env -i \
  ...
  ${LYDIA_RUN_SLOW_PET_TESTS:+LYDIA_RUN_SLOW_PET_TESTS="$LYDIA_RUN_SLOW_PET_TESTS"} \
