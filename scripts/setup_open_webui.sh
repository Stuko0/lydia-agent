...
# Bootstrap Open WebUI against Lydia Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.lydia/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-lydia.sh
...
#   LYDIA_API_MODEL_NAME='Lydia Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Lydia Agent WebUI}"
...
LYDIA_ENV_FILE="${LYDIA_ENV_FILE:-$HOME/.lydia/.env}"
...
LYDIA_API_MODEL_NAME="${LYDIA_API_MODEL_NAME:-Lydia Agent}"
LYDIA_API_BASE_URL="http://${LYDIA_API_CONNECT_HOST}:${LYDIA_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-lydia.sh"
LOG_DIR="$HOME/.lydia/logs"
...
install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.lydia.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.lydia</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.lydia"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.lydia"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-lydia.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Lydia Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-lydia.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.lydia/logs/openwebui.log
StandardError=append:%h/.lydia/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-lydia.service
}
...
main() {
  ...
  log 'Ensuring Lydia API server is configured...'
  ...
  log 'Restarting Lydia gateway so API server settings take effect...'
  lydia gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${LYDIA_API_CONNECT_HOST}:${LYDIA_API_PORT}/health" >/dev/null; then
    log 'Lydia API server did not answer on the first check. Trying to start gateway in the background...'
    nohup lydia gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  ...
  log "Lydia API endpoint: ${LYDIA_API_BASE_URL}"
