...
VERSION_FILE = REPO_ROOT / "lydia_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows lydia-shim resolution + prefer --update on recovery; #52378)
    ...
    "274096618+lydia-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "lydia@nousresearch.com": "NousResearch",
    ...
    "lydia@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "github@nadyalydia.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (LYDIA_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (LYDIA_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: lydia-eval community link)
    ...
    "lydiaagent26@gmail.com": "lydiaagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+lydiaagent26@users.noreply.github.com": "lydiaagent26",
    ...
    "vigo@lydia": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Lydia runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level LYDIA_HOME state files)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Lydia
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"lydia-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "lydia@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://10.1.200.116:3000/arquant-admin/NewLydia",
    ...
    lines.append(f"# Lydia Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Lydia Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Lydia Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Lydia Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Lydia Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Lydia Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Lydia Agent v{new_version} ({calver_date})' "
